...
Todas las variables se configuran en `config.yaml`.
...
**Valores:**
- `noble` - Ubuntu 24.04 LTS (único codename probado y soportado actualmente)
...
**Seguridad:** Se guarda en texto plano en config.yaml (eliminar después)
...
```bash
# Verificar sintaxis
# Validar YAML (requiere yq o verificación manual)

# Ver variables
# config.yaml se parsea automáticamente por install.sh
echo $HOSTNAME
echo $INSTALL_GNOME
```
...
**Solución:** Definir todas las variables obligatorias en config.yaml
