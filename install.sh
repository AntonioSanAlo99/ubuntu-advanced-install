...
run_module() {
    ...
    # Ejecutar módulo y capturar salida
    local module_output
    local module_exit_code
    
    # NO usar set -e aquí para que no interrumpa
    set +e
    module_output=$(bash "$module_path" 2>&1)
    module_exit_code=$?
    set -e
    
    # Mostrar salida del módulo
    echo "$module_output"
    ...
    if [ $module_exit_code -eq 0 ]; then
        ...
        log_error "Módulo falló: $module_name (exit code: $module_exit_code)"
        echo "[$module_name] FAILED (code: $module_exit_code)" >> "$LOG_DIR/module-summary.log"
        
        # Mostrar opciones de error
        echo ""
        echo "════════════════════════════════════════════════════════════════"
        echo -e "${RED}  ✗ ERROR EN MÓDULO: $module_name${NC}"
        echo "════════════════════════════════════════════════════════════════"
        echo ""
        echo "Opciones:"
        echo "  1) Continuar con siguiente módulo"
        echo "  2) Reintentar este módulo"
        echo "  3) Saltar al siguiente y continuar"
        echo "  4) Abrir shell de depuración"
        echo "  5) Abortar instalación"
        echo ""
        read -p "Selecciona opción (1-5) [1]: " error_choice
        error_choice=${error_choice:-1}
        
        case $error_choice in
            1)
                echo "Continuando con siguiente módulo..."
                return 1
                ;;
            2)
                echo "Reintentando módulo..."
                run_module "$module_name"
                return $?
                ;;
            3)
                echo "Saltando al siguiente módulo..."
                return 1
                ;;
            4)
                echo ""
                echo "════════════════════════════════════════════════════════════════"
                echo "  SHELL DE DEPURACIÓN"
                echo "════════════════════════════════════════════════════════════════"
                echo ""
                ...
                echo ""
                echo "Escribe 'exit' para volver al instalador"
                echo ""
                bash
                echo ""
                read -p "¿Reintentar el módulo ahora? (s/n): " retry
...
full_interactive_install() {
    ...
    if grep -q "Intel" /proc/cpuinfo; then
        ...
        echo -e "  ${YELLOW}⚠ CPU no reconocida${NC}"
        ((HW_WARNINGS++))
    fi
    
    CPU_CORES=$(nproc)
    echo "  ✓ Cores: $CPU_CORES"
    [ "$CPU_CORES" -lt 2 ] && echo -e "  ${YELLOW}⚠ Pocos cores para GNOME (recomendado: 4+)${NC}" && ((HW_WARNINGS++))
    ...
    if [ "$RAM_GB" -lt 4 ]; then
        echo -e "  ${RED}✗ RAM insuficiente (mínimo 4GB)${NC}"
        ((HW_ERRORS++))
    elif [ "$RAM_GB" -lt 8 ]; then
        echo -e "  ${YELLOW}⚠ RAM por debajo de lo recomendado (8GB)${NC}"
        ((HW_WARNINGS++))
    fi
    ...
    echo ""
    echo "════════════════════════════════════════════════════════════════"
    if [ $HW_ERRORS -eq 0 ] && [ $HW_WARNINGS -eq 0 ]; then
        echo -e "${GREEN}✓ Hardware compatible${NC}"
    else
        [ $HW_ERRORS -gt 0 ] && echo -e "${RED}Errores: $HW_ERRORS${NC}"
        [ $HW_WARNINGS -gt 0 ] && echo -e "${YELLOW}Advertencias: $HW_WARNINGS${NC}"
    fi
    echo "════════════════════════════════════════════════════════════════"
    echo ""
    
    if [ $HW_ERRORS -gt 0 ]; then
        read -p "¿Continuar de todas formas? (s/n): " hw_continue
        [ "$hw_continue" != "s" ] && [ "$hw_continue" != "S" ] && exit 1
    fi
...
if [ $# -gt 0 ]; then
    ...
    case "$1" in
        ...
        --help|-h)
            ...
            echo "  --debug              Modo depuración (muestra comandos)"
