...
interactive_config() {
    ...
    if [ "$INSTALL_GNOME" = "true" ]; then
        ...
        echo -e "${GREEN}  - AppManager GUI: sí (AppImage vía AM)${NC}"
    else
        GDM_AUTOLOGIN="false"
    ...
    if [ "$INSTALL_GNOME" = "true" ]; then
        ...
        echo "    - AppManager GUI: sí (AppImage vía AM)"
    fi
...
show_menu() {
    ...
    echo "  25) Desarrollo"
    echo "  26) Gaming"
    echo "  27) AM / AppMan (gestor de AppImages)"
    echo ""
...
full_automatic_install() {
    ...
    [ "${INSTALL_AM:-false}"          = "true" ] && run_module "17-install-am"
