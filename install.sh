...
interactive_config() {
    ...
    if [ "$INSTALL_GNOME" = "true" ]; then
        ...
        read -p "  ¿Optimizar memoria? (deshabilita Tracker, Evolution, etc.) (s/n) [s]: " opt_mem
        [[ ${opt_mem:-s} =~ ^[SsYy]$ ]] && GNOME_OPTIMIZE_MEMORY="true" || GNOME_OPTIMIZE_MEMORY="false"
        
        read -p "  ¿Aplicar tema transparente? (s/n) [s]: " opt_theme
        [[ ${opt_theme:-s} =~ ^[SsYy]$ ]] && GNOME_TRANSPARENT_THEME="true" || GNOME_TRANSPARENT_THEME="false"
