...
map_yaml_to_legacy() {
    ...
    [ -n "${DEVELOPMENT_RUST:-}" ]          && INSTALL_RUST="$DEVELOPMENT_RUST"
    ...
    [ -n "${DEVELOPMENT_BUN:-}" ]           && INSTALL_BUN="$DEVELOPMENT_BUN"
    [ -n "${DEVELOPMENT_DENO:-}" ]          && INSTALL_DENO="$DEVELOPMENT_DENO"
    ...
    [ -n "${EXTRAS_SYS_MANAGERS:-}" ]       && INSTALL_SYS_MANAGERS="$EXTRAS_SYS_MANAGERS"
...
export_config_vars() {
    ...
    export INSTALL_RUST="${INSTALL_RUST:-true}"
    ...
    export INSTALL_BUN="${INSTALL_BUN:-false}"
    export INSTALL_DENO="${INSTALL_DENO:-false}"
    ...
    export INSTALL_FALCOND="${INSTALL_FALCOND:-false}"
    export INSTALL_OPTISCALER="${INSTALL_OPTISCALER:-false}"
    ...
    export INSTALL_STREAMING_WEBAPPS="${INSTALL_STREAMING_WEBAPPS:-false}"
    export INSTALL_SYS_MANAGERS="${INSTALL_SYS_MANAGERS:-false}"
    ...
    # ── Laptop ───────────────────────────────────────────────────────────────
    export POWER_MANAGER="${POWER_MANAGER:-1}"
...
# Ejemplo: INSTALL_RUST=$(_yn "$opt_rust" "false")
#   "" + default "false" → "false"
#   "s"/"S"/"y"/"Y" → "true"
...
interactive_config() {
    ...
    if [ "$INSTALL_GNOME" = "false" ]; then
        ...
        GPU_MANUAL="9"
        INSTALL_ONLYOFFICE="false"; INSTALL_QBITTORRENT="false"; INSTALL_MULLVAD="false"
        INSTALL_OBS="false"; INSTALL_OBSIDIAN="false"
        ...
        if [ "$INSTALL_DEVELOPMENT" = "true" ]; then
            ...
            INSTALL_BUN=$(_yn "$opt_bun" "false")
            read -p "    ¿Deno runtime? (s/n) [n]: " opt_deno
            INSTALL_DENO=$(_yn "$opt_deno" "false")
            ...
            read -p "    ¿n8n (automatización de workflows)? (s/n) [n]: " opt_n8n
            INSTALL_N8N=$(_yn "$opt_n8n" "false")
            INSTALL_MELD="false"; INSTALL_POSTMAN="false"  # Sin GUI en servidor
            read -p "    ¿Red y cloud (nmap, AWS CLI, httpie, jq, tshark)? (s/n) [n]: " opt_nettools
            INSTALL_NETTOOLS=$(_yn "$opt_nettools" "false")
        else
            INSTALL_VSCODE="false"; NODEJS_OPTION="1"; INSTALL_RUST="true"; INSTALL_TOPGRADE="false"; INSTALL_BOXES="false"; INSTALL_LAZY_TOOLS="false"
            INSTALL_BUN="false"; INSTALL_DENO="false"; INSTALL_DOCKER_TOOLS="false"; INSTALL_N8N="false"; INSTALL_MELD="false"; INSTALL_POSTMAN="false"; INSTALL_NETTOOLS="false"
        fi
        ...
        if [ "$INSTALL_DEVELOPMENT" = "true" ]; then
            ...
            INSTALL_TOPGRADE=$(_yn "$opt_topgrade" "true")
            read -p "    ¿GNOME Boxes (máquinas virtuales)? (s/n) [n]: " opt_boxes
            INSTALL_BOXES=$(_yn "$opt_boxes" "false")
            read -p "    ¿Lazy TUI tools (lazygit + lazydocker + LazyVim)? (s/n) [n]: " opt_lazy
            INSTALL_LAZY_TOOLS=$(_yn "$opt_lazy" "false")
            read -p "    ¿Bun runtime? (s/n) [n]: " opt_bun
            INSTALL_BUN=$(_yn "$opt_bun" "false")
            ...
            INSTALL_MELD=$(_yn "$opt_meld" "false")
            read -p "    ¿Postman (API testing)? (s/n) [n]: " opt_postman
            INSTALL_POSTMAN=$(_yn "$opt_postman" "false")
...
save_config() {
    ...
    local _power_str
    case "${POWER_MANAGER:-1}" in 2) _power_str="tlp" ;; *) _power_str="ppd" ;; esac
...
development:
  ...
  bun:          ${INSTALL_BUN:-false}
  deno:         ${INSTALL_DENO:-false}
...
gaming:
  ...
  falcond:            ${INSTALL_FALCOND:-false}
  optiscaler:         ${INSTALL_OPTISCALER:-false}
...
extras:
  ...
  streaming_webapps: ${INSTALL_STREAMING_WEBAPPS:-false}
  sys_managers:      ${INSTALL_SYS_MANAGERS:-false}
...
if [ -n "${1:-}" ]; then
    ...
    case "$1" in
        ...
        --dry-run|-n)
            ...
            [ "${INSTALL_GAMING}" = "true" ] && echo "      CachyOS kernel: ${INSTALL_CACHYOS_KERNEL}, Falcond: ${INSTALL_FALCOND}, OptiScaler: ${INSTALL_OPTISCALER}"
