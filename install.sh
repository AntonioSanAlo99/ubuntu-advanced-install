...
chroot_umount() {
    local target="$1"
    [ -z "$target" ] && return 0
    # Orden inverso al montaje; lazy umount para rbind
    mountpoint -q "${target}/run"  2>/dev/null && umount -l "${target}/run"  2>/dev/null || true
    mountpoint -q "${target}/dev"  2>/dev/null && umount -l "${target}/dev"  2>/dev/null || true
    mountpoint -q "${target}/sys"  2>/dev/null && umount -l "${target}/sys"  2>/dev/null || true
