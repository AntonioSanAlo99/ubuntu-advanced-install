...
interactive_config() {
    ...
    # 1. Versión Ubuntu
    echo -e "${YELLOW}[1/9] Versión de Ubuntu${NC}"
    echo ""
    ...
    # 2. Hostname
    clear
    echo -e "${YELLOW}[2/9] Nombre del equipo (hostname)${NC}"
    echo ""
    ...
    # 3. Usuario
    clear
    echo -e "${YELLOW}[3/9] Nombre de usuario${NC}"
    echo ""
    ...
    # 4. Tipo de hardware
    clear
    echo -e "${YELLOW}[4/9] Tipo de hardware${NC}"
    echo ""
    ...
    # 5. Conectividad
    clear
    echo -e "${YELLOW}[5/9] Conectividad${NC}"
    echo ""
    ...
    clear
    echo -e "${YELLOW}[6/9] Componentes a instalar${NC}"
    echo ""
    ...
    echo -e "${CYAN}ℹ  Multimedia (códecs, thumbnailers): incluido por defecto${NC}"
    read -p "  ¿Instalar Spotify? (s/n) [s]: " opt_spotify
    [[ ${opt_spotify:-s} =~ ^[SsYy]$ ]] && INSTALL_SPOTIFY="s" || INSTALL_SPOTIFY="n"
    read -p "  ¿Instalar Cine? (reproductor GTK4 + mpv, desde fuente) (s/n) [s]: " opt_cine
    [[ ${opt_cine:-s} =~ ^[SsYy]$ ]] && INSTALL_CINE="true" || INSTALL_CINE="false"
    ...
    if [ "$INSTALL_GAMING" = "true" ]; then
        read -p "  ¿Compilar ProtonPlus desde fuente? (gestor de Proton/Wine-GE) (s/n) [s]: " inst_pp
    ...
    echo -e "${GREEN}✓ Multimedia: $INSTALL_MULTIMEDIA$([ "$INSTALL_SPOTIFY" = "s" ] && echo " (+Spotify)")$([ "$INSTALL_CINE" = "true" ] && echo " (+Cine)")${NC}"
    echo -e "${GREEN}✓ Desarrollo: $INSTALL_DEVELOPMENT${NC}"
    echo -e "${GREEN}✓ Gaming: $INSTALL_GAMING$([ "$INSTALL_PROTONPLUS" = "true" ] && echo " (+ProtonPlus)")${NC}"
    echo ""
    read -p "Presiona Enter para continuar..."

    # 6b. Personalización de GNOME (solo si GNOME está activado)
    if [ "$INSTALL_GNOME" = "true" ]; then
        clear
        echo -e "${YELLOW}[6b/9] Personalización de GNOME${NC}"
        echo ""

        GNOME_OPTIMIZE_MEMORY="true"
        echo -e "${CYAN}ℹ  Optimización de memoria GNOME: activada por defecto${NC}"

        GNOME_TRANSPARENT_THEME="true"
        echo -e "${CYAN}ℹ  Tema transparente: activado por defecto${NC}"

        echo ""
        echo -e "${CYAN}Panel/Dock:${NC}"
        echo "  1) Ubuntu Dock (barra lateral, estilo macOS)"
        echo "  2) Dash to Panel (barra inferior integrada, estilo Windows)"
        read -p "Opción [1]: " opt_dock
        case "${opt_dock:-1}" in
            2) GNOME_DOCK="dash-to-panel" ;;
            *) GNOME_DOCK="ubuntu-dock" ;;
        esac
        ...
        echo -e "${GREEN}✓ Panel/Dock: $GNOME_DOCK${NC}"
    ...
    # 7. Optimizaciones
    clear
    echo -e "${YELLOW}[7/9] Optimizaciones del sistema${NC}"
    echo ""

    MINIMIZE_SYSTEMD="true"
    echo -e "${CYAN}ℹ  Minimización de systemd: activada por defecto${NC}"
    echo ""
...
run_module() {
    ...
    # Los módulos invocan arch-chroot que hereda estas variables.
    ...
    # arch-chroot se encarga de montar/desmontar pseudofilesystems automáticamente
...
show_menu() {
    ...
    echo "  15) Almacenamiento       → módulo 11-configure-storage"
    echo "  16) Audio plug and play  → módulo 11-configure-audio"
    ...
    case $choice in
        ...
        15) run_module "11-configure-storage" ;;
        16) run_module "11-configure-audio" ;;
...
full_automatic_install() {
    ...
    # arch-chroot monta/desmonta pseudofilesystems automáticamente en cada invocación.
    # No necesitamos montar manualmente — cada módulo usa arch-chroot directamente.
    ...
    run_module "11-configure-storage"  || exit 1
    run_module "11-configure-audio"    || exit 1
...
full_interactive_install() {
    ...
    MODULES_TO_RUN+=("11-configure-storage");      MODULES_LABELS+=("Optimización de almacenamiento"); MODULES_REQUIRED+=("1")
    MODULES_TO_RUN+=("11-configure-audio");        MODULES_LABELS+=("Audio plug and play");             MODULES_REQUIRED+=("1")
