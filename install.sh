...
save_config() {
    # ── Hashear contraseñas (como Subiquity/archinstall) ─────────────────────
    # Se usa SHA-512 ($6$) via openssl o mkpasswd. El hash se guarda en
    # config.env en lugar del plaintext. Los módulos que crean usuarios
    # (03-configure-base) usan chpasswd -e para aplicar el hash directamente.
    ...
    if command -v openssl &>/dev/null; then
        ...
        # Fallback: guardar plaintext con aviso
        log_warning "openssl/mkpasswd no disponible — contraseñas en texto plano"
        user_hash=""
        root_hash=""
    fi
...
# === CREDENCIALES (hash SHA-512, como Subiquity autoinstall) ===
# Generadas con: openssl passwd -6 "contraseña"
# Los módulos usan chpasswd -e para aplicar el hash directamente.
EOF
...
EOF
        ...
        cat >> "$CONFIG_FILE" << EOF
# AVISO: contraseñas en texto plano (openssl no disponible al generar)
# Elimina este fichero tras instalar: rm config.env
...
EOF
    ...
    if [ -n "$user_hash" ]; then
        echo -e "${GREEN}✓ Credenciales almacenadas como hash SHA-512${NC}"
    else
        echo -e "${YELLOW}⚠ Credenciales en texto plano — elimina config.env tras instalar${NC}"
    fi
}
...
export_config_vars() {
    ...
    # Credenciales: hash tiene prioridad sobre plaintext
...
validate_config() {
    ...
    # ── Contraseñas: deben existir (hash o plaintext) ────────────────────────
    if [ -z "${USER_PASSWORD:-}" ] && [ -z "${USER_PASSWORD_HASH:-}" ]; then
        _vfail "USER_PASSWORD o USER_PASSWORD_HASH requerido"
    fi
...
full_automatic_install() {
    ...
    run_module "00-check-dependencies" || { log_error "Error al verificar dependencias"; exit 1; }

    build_module_list
    run_all_modules "automatic" || exit 1

    run_module "91-generate-report"
    cleanup_config "auto"
    post_install_menu
}
