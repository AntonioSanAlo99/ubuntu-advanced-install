...
# Si existe config.yaml, tiene prioridad sobre config.env.
parse_yaml() {
...
map_yaml_to_legacy() {
    ...
    [ "${MULTIMEDIA_SPOTIFY:-}" = "true" ]  && INSTALL_SPOTIFY="s" || INSTALL_SPOTIFY="${INSTALL_SPOTIFY:-n}"
    [ -n "${DEVELOPMENT_ENABLED:-}" ]       && INSTALL_DEVELOPMENT="$DEVELOPMENT_ENABLED"
    [ "${DEVELOPMENT_VSCODE:-}" = "true" ]  && INSTALL_VSCODE="s"
    case "${DEVELOPMENT_NODEJS:-}" in lts) NODEJS_OPTION="2" ;; none) NODEJS_OPTION="1" ;; esac
    [ "${DEVELOPMENT_RUST:-}" = "true" ]    && INSTALL_RUST="s"
    [ "${DEVELOPMENT_TOPGRADE:-}" = "true" ] && INSTALL_TOPGRADE="s"
    [ "${DEVELOPMENT_GNOME_BOXES:-}" = "true" ] && INSTALL_BOXES="s"
    [ "${DEVELOPMENT_LAZY_TOOLS:-}" = "true" ] && INSTALL_LAZY_TOOLS="s"
    [ "${DEVELOPMENT_BUN:-}" = "true" ]     && INSTALL_BUN="s"
    [ "${DEVELOPMENT_DENO:-}" = "true" ]    && INSTALL_DENO="s"
    [ "${DEVELOPMENT_DOCKER_TOOLS:-}" = "true" ] && INSTALL_DOCKER_TOOLS="s"
    [ "${DEVELOPMENT_MELD:-}" = "true" ]    && INSTALL_MELD="s"
    [ "${DEVELOPMENT_POSTMAN:-}" = "true" ] && INSTALL_POSTMAN="s"
    [ "${DEVELOPMENT_NETTOOLS:-}" = "true" ] && INSTALL_NETTOOLS="s"
    ...
    [ -n "${GAMING_FALCOND:-}" ]            && INSTALL_FALCOND="$GAMING_FALCOND"
    [ -n "${GAMING_OPTISCALER:-}" ]         && INSTALL_OPTISCALER="$GAMING_OPTISCALER"
    [ -n "${GAMING_DISCORD:-}" ]            && INSTALL_DISCORD="$GAMING_DISCORD"
    [ -n "${EXTRAS_ONLYOFFICE:-}" ]         && { [ "$EXTRAS_ONLYOFFICE" = "true" ] && INSTALL_ONLYOFFICE="s" || INSTALL_ONLYOFFICE="n"; }
    [ -n "${EXTRAS_QBITTORRENT:-}" ]        && { [ "$EXTRAS_QBITTORRENT" = "true" ] && INSTALL_QBITTORRENT="s" || INSTALL_QBITTORRENT="n"; }
    [ -n "${EXTRAS_MULLVAD_VPN:-}" ]        && { [ "$EXTRAS_MULLVAD_VPN" = "true" ] && INSTALL_MULLVAD="s" || INSTALL_MULLVAD="n"; }
    [ -n "${EXTRAS_OBS_STUDIO:-}" ]         && { [ "$EXTRAS_OBS_STUDIO" = "true" ] && INSTALL_OBS="s" || INSTALL_OBS="n"; }
    [ -n "${EXTRAS_OBSIDIAN:-}" ]           && { [ "$EXTRAS_OBSIDIAN" = "true" ] && INSTALL_OBSIDIAN="s" || INSTALL_OBSIDIAN="n"; }
    [ -n "${EXTRAS_STREAMING_WEBAPPS:-}" ]  && INSTALL_STREAMING_WEBAPPS="$EXTRAS_STREAMING_WEBAPPS"
    ...
    [ -n "${PERFORMANCE_ZRAM:-}" ]             && PERF_ZRAM="$PERFORMANCE_ZRAM"
    [ -n "${PERFORMANCE_ZRAM_SWAPPINESS:-}" ]  && PERF_ZRAM_SWAPPINESS="$PERFORMANCE_ZRAM_SWAPPINESS"
    ...
    case "${LAPTOP_POWER_MANAGER:-ppd}" in ppd) POWER_MANAGER="1" ;; tlp) POWER_MANAGER="2" ;; esac
}
...
BANNER
    ...
    if [ "$INSTALL_GNOME" = "false" ]; then
        ...
        STEAM_METHOD="1"
        INSTALL_CACHYOS_KERNEL="false"; PSYCACHY_SCHEDULER="bore"
        INSTALL_FALCOND="false"; INSTALL_OPTISCALER="false"
        GPU_MANUAL="9"
        INSTALL_ONLYOFFICE="n"; INSTALL_QBITTORRENT="n"; INSTALL_MULLVAD="n"
        ...
        if [ "$INSTALL_DEVELOPMENT" = "true" ]; then
            ...
            read -p "    ¿Lazy TUI tools (lazygit + lazydocker + LazyVim)? (s/n) [n]: " opt_lazy
            INSTALL_LAZY_TOOLS="${opt_lazy:-n}"
            read -p "    ¿Bun runtime? (s/n) [n]: " opt_bun
            INSTALL_BUN="${opt_bun:-n}"
            read -p "    ¿Deno runtime? (s/n) [n]: " opt_deno
            INSTALL_DENO="${opt_deno:-n}"
            read -p "    ¿Docker tools (unregistry + docker-pussh + uncloud)? (s/n) [n]: " opt_docker_tools
            INSTALL_DOCKER_TOOLS="${opt_docker_tools:-n}"
            INSTALL_MELD="n"; INSTALL_POSTMAN="n"  # Sin GUI en servidor
            read -p "    ¿Red y cloud (nmap, AWS CLI, httpie, jq, tshark)? (s/n) [n]: " opt_nettools
            INSTALL_NETTOOLS="${opt_nettools:-n}"
        else
            INSTALL_VSCODE="n"; NODEJS_OPTION="1"; INSTALL_RUST="n"; INSTALL_TOPGRADE="n"; INSTALL_BOXES="n"; INSTALL_LAZY_TOOLS="n"
            INSTALL_BUN="n"; INSTALL_DENO="n"; INSTALL_DOCKER_TOOLS="n"; INSTALL_MELD="n"; INSTALL_POSTMAN="n"; INSTALL_NETTOOLS="n"
        fi
        ...
        if [ "$INSTALL_DEVELOPMENT" = "true" ]; then
            ...
            read -p "    ¿GNOME Boxes (máquinas virtuales)? (s/n) [n]: " opt_boxes
            INSTALL_BOXES="${opt_boxes:-n}"
            read -p "    ¿Lazy TUI tools (lazygit + lazydocker + LazyVim)? (s/n) [n]: " opt_lazy
            INSTALL_LAZY_TOOLS="${opt_lazy:-n}"
            read -p "    ¿Bun runtime? (s/n) [n]: " opt_bun
            INSTALL_BUN="${opt_bun:-n}"
            ...
            read -p "    ¿Docker tools (unregistry + docker-pussh + uncloud)? (s/n) [n]: " opt_docker_tools
            INSTALL_DOCKER_TOOLS="${opt_docker_tools:-n}"
            read -p "    ¿Meld (diff/merge visual)? (s/n) [n]: " opt_meld
            INSTALL_MELD="${opt_meld:-n}"
            read -p "    ¿Postman (API testing)? (s/n) [n]: " opt_postman
            INSTALL_POSTMAN="${opt_postman:-n}"
            read -p "    ¿Red y cloud (Wireshark, nmap, AWS CLI, httpie, jq)? (s/n) [n]: " opt_nettools
            INSTALL_NETTOOLS="${opt_nettools:-n}"
        else
            INSTALL_VSCODE="n"; NODEJS_OPTION="1"; INSTALL_RUST="n"; INSTALL_TOPGRADE="n"; INSTALL_BOXES="n"; INSTALL_LAZY_TOOLS="n"
            INSTALL_BUN="n"; INSTALL_DENO="n"; INSTALL_DOCKER_TOOLS="n"; INSTALL_MELD="n"; INSTALL_POSTMAN="n"; INSTALL_NETTOOLS="n"
        ...
        if [ "$INSTALL_GAMING" = "true" ]; then
            ...
            echo ""
            read -p "    ¿Kernel PsyCachy (CachyOS gaming optimizado)? (s/n) [n]: " inst_cachy
            if [[ ${inst_cachy:-n} =~ ^[SsYy]$ ]]; then
                INSTALL_CACHYOS_KERNEL="true"
                echo ""
                echo "      Scheduler del kernel PsyCachy:"
                echo "        1) BORE  — Burst-Oriented Response Enhancer (mejor gaming, default)"
                echo "        2) EEVDF — Earliest Eligible Virtual Deadline First (allround)"
                read -p "      Opción [1]: " opt_sched
                [[ "${opt_sched:-1}" = "2" ]] && PSYCACHY_SCHEDULER="eevdf" || PSYCACHY_SCHEDULER="bore"
            else
                INSTALL_CACHYOS_KERNEL="false"
                PSYCACHY_SCHEDULER="bore"
            fi
            echo ""
        ...
        read -p "  ¿qBittorrent (cliente torrent)? (s/n) [n]: " INSTALL_QBITTORRENT
    ...
    echo "  Extras: OnlyOffice: $INSTALL_ONLYOFFICE, qBittorrent: $INSTALL_QBITTORRENT, Mullvad: $INSTALL_MULLVAD"
    echo ""
...
PERF_ZRAM="${PERF_ZRAM:-true}"
PERF_ZRAM_SWAPPINESS="${PERF_ZRAM_SWAPPINESS:-180}"
PERF_OOMD_AGGRESSIVE="${PERF_OOMD_AGGRESSIVE:-true}"
PERF_TMPFILES_CLEANUP="${PERF_TMPFILES_CLEANUP:-true}"
PERF_DNS_OVER_TLS="${PERF_DNS_OVER_TLS:-true}"
...
export_config_vars() {
    ...
    export UBUNTU_VERSION TARGET_DISK TARGET HOSTNAME USERNAME \
        ...
        INSTALL_ONLYOFFICE INSTALL_QBITTORRENT INSTALL_MULLVAD \
        ...
        UBUNTU_SIZE_GB \
        PERF_ZRAM PERF_ZRAM_SWAPPINESS PERF_OOMD_AGGRESSIVE PERF_TMPFILES_CLEANUP PERF_DNS_OVER_TLS
...
load_or_create_config() {
    # config.yaml tiene prioridad sobre config.env (formato legacy)
    if [ -f "$CONFIG_YAML" ]; then
        echo -e "${GREEN}✓ Configuración encontrada: $CONFIG_YAML${NC}"
        echo ""
        read -p "¿Usar configuración existente? (s/n/e=editar) [s]: " use_existing
        
        case $use_existing in
            [Ee])
                ${EDITOR:-nano} "$CONFIG_YAML"
            ...
            *)
                ...
                map_yaml_to_legacy
                export_config_vars
                echo -e "${GREEN}✓ Configuración cargada (YAML)${NC}"
                ;;
        esac
    elif [ -f "$CONFIG_FILE" ]; then
        echo -e "${GREEN}✓ Configuración encontrada: $CONFIG_FILE${NC}"
        echo -e "${YELLOW}  (considera migrar a config.yaml para formato moderno)${NC}"
        echo ""
        read -p "¿Usar configuración existente? (s/n/e=editar) [s]: " use_existing
        ...
        case $use_existing in
            ...
            *)
                source "$CONFIG_FILE"
                export_config_vars
                echo -e "${GREEN}✓ Configuración cargada (legacy env)${NC}"
                ;;
        esac
    else
...
run_module() {
...

    {
        ...
        err_count=$(grep -ciE 'error|fail|unable|cannot|not found|E:' "$module_log" 2>/dev/null || echo 0)
...
build_module_list() {
    ...
    [[ "${INSTALL_QBITTORRENT:-n}" =~ ^[SsYy]$ ]] && _needs_extras=true
...
run_all_modules() {
    ...
    [[ "${INSTALL_QBITTORRENT:-n}" =~ ^[SsYy]$ ]] && extras="${extras}qBittorrent "
