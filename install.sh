...
BANNER
    ...
    if [ "$INSTALL_GAMING" = "true" ]; then
        ...
        if [[ ${inst_cachy:-n} =~ ^[SsYy]$ ]]; then
            ...
            echo "      Scheduler: 1) BORE (mejor respuesta interactiva)  2) EEVDF (stock optimizado)"
        ...
        echo ""
        read -p "    ¿Parámetros GRUB gaming? (mitigations=off, etc — MÁS RENDIMIENTO, MENOS SEGURIDAD) (s/n) [n]: " inst_grub_gaming
        [[ ${inst_grub_gaming:-n} =~ ^[SsYy]$ ]] && GRUB_GAMING_PARAMS="true" || GRUB_GAMING_PARAMS="false"
        echo ""
        ...
        GRUB_GAMING_PARAMS="false"; INSTALL_FALCOND="false"
    ...
    echo "  Gaming: $INSTALL_GAMING$([ "$INSTALL_GAMING" = "true" ] && echo " (GPU: $GPU_MANUAL, CachyOS kernel: $INSTALL_CACHYOS_KERNEL, GRUB gaming: $GRUB_GAMING_PARAMS)")"
...
CACHYOS_SCHEDULER="${CACHYOS_SCHEDULER:-1}"
GRUB_GAMING_PARAMS="${GRUB_GAMING_PARAMS:-false}"
...
export_config_vars() {
    ...
    export UBUNTU_VERSION TARGET_DISK TARGET HOSTNAME USERNAME \
        ...
        INSTALL_CACHYOS_KERNEL CACHYOS_SCHEDULER GRUB_GAMING_PARAMS \
        INSTALL_FALCOND INSTALL_OPTISCALER \
        INSTALL_ONLYOFFICE INSTALL_AMULE INSTALL_MULLVAD \
        INSTALL_STORAGE_OPT STORAGE_DISK_TYPE \
        AUTO_UPDATE_CHOICE POWER_MANAGER INSTALL_NOTHROTTLE DUAL_BOOT \
        UBUNTU_SIZE_GB
...
build_module_list() {
    ...
    [[ "${INSTALL_ONLYOFFICE:-n}" =~ ^[SsYy]$ || "${INSTALL_AMULE:-n}" =~ ^[SsYy]$ || "${INSTALL_MULLVAD:-n}" =~ ^[SsYy]$ ]] && {
        MODULES_TO_RUN+=("25-install-extras"); MODULES_LABELS+=("Apps extras (OnlyOffice, aMule, Mullvad)"); MODULES_REQUIRED+=("0")
    }
