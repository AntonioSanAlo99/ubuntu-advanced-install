...
chroot_mount() {
    ...
    #
    # /proc  → mount --types proc (instancia nueva, NO bind)
    # /sys   → mount --rbind + --make-rslave
    # /dev   → mount --rbind + --make-rslave
    # /run   → mount --bind  + --make-slave
    #
    # --make-rslave es NECESARIO para systemd: evita que eventos de mount
    # dentro del chroot se propaguen al host y viceversa.
    # Sin --make-rslave, systemd-tmpfiles y udev fallan.

    mkdir -p "${target}/proc" "${target}/sys" "${target}/dev" "${target}/run"

    mountpoint -q "${target}/proc" || mount --types proc /proc "${target}/proc"
    ...
    mountpoint -q "${target}/run"  || { mount --bind /run "${target}/run"  && mount --make-slave "${target}/run"; }
    ...
    if ! mountpoint -q "${target}/dev"; then
        echo "ERROR CRÍTICO: No se pudo montar /dev en ${target}/dev"
        return 1
    fi
...
chroot_umount() {
    local target="$1"
    [ -z "$target" ] && return 0
    # Orden inverso; -R para rbind mounts (recursivo)
    mountpoint -q "${target}/dev/shm" && umount "${target}/dev/shm" 2>/dev/null || true
    mountpoint -q "${target}/run"     && umount -R "${target}/run"  2>/dev/null || true
    mountpoint -q "${target}/dev"     && umount -R "${target}/dev"  2>/dev/null || true
    mountpoint -q "${target}/sys"     && umount -R "${target}/sys"  2>/dev/null || true
    mountpoint -q "${target}/proc"    && umount -l "${target}/proc" 2>/dev/null || true
...
full_automatic_install() {
    ...
    if ! mountpoint -q "${TARGET}/proc"; then
        ...
        log_error "  /proc del host: $(mountpoint -q /proc && echo 'OK' || echo 'NO MONTADO')"
        log_error "  Contenido ${TARGET}/proc: $(ls ${TARGET}/proc/ 2>&1 | head -3)"
        log_error "Los módulos chroot (kernel, grub, dracut) no funcionarán."
        exit 1
    fi
    log "SUCCESS" "Verificado: /proc montado en ${TARGET}/proc"
