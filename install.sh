...
chroot_mount() {
    ...
    # Patrón estándar Debian/Ubuntu para debootstrap + chroot
    ...
    # Verificar que /proc se montó (crítico para dpkg/grub/dracut)
...
chroot_umount() {
    ...
    mountpoint -q "${target}/dev/pts" && umount -l "${target}/dev/pts" 2>/dev/null || true
    mountpoint -q "${target}/run"     && umount -l "${target}/run"     2>/dev/null || true
    mountpoint -q "${target}/dev"     && umount -l "${target}/dev"     2>/dev/null || true
    mountpoint -q "${target}/sys"     && umount -l "${target}/sys"     2>/dev/null || true
