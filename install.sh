...
# ============================================================================
# PROGRESO — header al inicio de cada módulo
# ============================================================================
...
run_module() {
    ...
    # Progreso
    local pct=0 header_text="$module_label"
    if [ -n "$module_num" ] && [ -n "$module_total" ]; then
        header_text="[$module_num/$module_total] $module_label"
        pct=$(( (module_num - 1) * 100 / module_total ))
    fi
    progress_header "$header_text" "$pct"
