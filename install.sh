...
map_yaml_to_legacy() {
    ...
    [ -n "${SYSTEM_USE_LUKS:-}" ]           && USE_LUKS="$SYSTEM_USE_LUKS"
...
export_config_vars() {
    ...
    export USE_LUKS="${USE_LUKS:-false}"
    export LUKS_PASSWORD="${LUKS_PASSWORD:-}"
    export DUAL_BOOT="${DUAL_BOOT:-false}"
    export UBUNTU_SIZE_GB="${UBUNTU_SIZE_GB:-50}"

    # ── GNOME ────────────────────────────────────────────────────────────────
    export INSTALL_GNOME="${INSTALL_GNOME:-false}"
    export GNOME_DOCK="${GNOME_DOCK:-dash-to-panel}"
    export GDM_AUTOLOGIN="${GDM_AUTOLOGIN:-true}"
    export GNOME_OPTIMIZE_MEMORY="${GNOME_OPTIMIZE_MEMORY:-false}"
    export GNOME_TRANSPARENT_THEME="${GNOME_TRANSPARENT_THEME:-false}"

    # ── Multimedia ───────────────────────────────────────────────────────────
    export INSTALL_MULTIMEDIA="${INSTALL_MULTIMEDIA:-false}"
    export INSTALL_SPOTIFY="${INSTALL_SPOTIFY:-false}"

    # ── Desarrollo ───────────────────────────────────────────────────────────
    export INSTALL_DEVELOPMENT="${INSTALL_DEVELOPMENT:-false}"
    export INSTALL_VSCODE="${INSTALL_VSCODE:-false}"
    export NODEJS_OPTION="${NODEJS_OPTION:-1}"
    ...
    export STEAM_METHOD="${STEAM_METHOD:-2}"
...
##############################################################################
# DEFAULTS — inicializa TODAS las variables a un estado seguro (todo OFF)
##############################################################################
# Se llama UNA vez antes de aplicar el perfil. Cualquier variable nueva que
# se añada al proyecto SOLO necesita añadirse aquí + en export_config_vars +
# en save_config. Los perfiles solo sobreescriben lo que activan.
##############################################################################
# DEFAULTS — estado base seguro (todo OFF)
##############################################################################
# Punto único de inicialización. Variable nueva → SOLO añadirla aquí +
# export_config_vars + save_config. Los perfiles solo sobreescriben lo que
# activan. Los módulos usan ${VAR:-default} como segunda red de seguridad.
#
# CATEGORÍAS:
#   build_module_list: INSTALL_GNOME, GNOME_OPTIMIZE_MEMORY, GNOME_TRANSPARENT_THEME,
#     INSTALL_MULTIMEDIA, INSTALL_DEVELOPMENT, INSTALL_GAMING, IS_LAPTOP,
#     MINIMIZE_SYSTEMD, INSTALL_ONLYOFFICE/QBITTORRENT/OBS/OBSIDIAN/MULLVAD
#   Módulos directamente: GNOME_DOCK, GDM_AUTOLOGIN, INSTALL_SPOTIFY, INSTALL_VSCODE,
#     NODEJS_OPTION, INSTALL_TOPGRADE, etc. (pasadas via export)
#   Solo save_config/yaml: GPU_MANUAL, STEAM_METHOD, PSYCACHY_SCHEDULER, etc.
##############################################################################

_defaults_all_off() {
    # ── Activan/desactivan módulos (build_module_list) ────────────────────────
    INSTALL_GNOME="false"
    ...
    # Extras (cada uno activa módulo 25 si es true)
    INSTALL_ONLYOFFICE="false"; INSTALL_QBITTORRENT="false"; INSTALL_OBS="false"
    INSTALL_OBSIDIAN="false"; INSTALL_MULLVAD="false"

    # ── Configuración de módulos (pasadas via export) ─────────────────────────
    GNOME_DOCK="dash-to-panel"; GDM_AUTOLOGIN="true"
    INSTALL_SPOTIFY="false"
    # Desarrollo
    INSTALL_VSCODE="false"; NODEJS_OPTION="1"
    ...
    # Gaming
    INSTALL_PROTONPLUS="false"; INSTALL_DISCORD="false"
    STEAM_METHOD="2"; INSTALL_CACHYOS_KERNEL="false"; PSYCACHY_SCHEDULER="bore"
    GPU_MANUAL="9"
    # Sistema
    USE_LUKS="false"
    INSTALL_NOTHROTTLE="false"; KERNEL_PARAMS_LEVEL="1"
}

##############################################################################
# PERFILES — cada función activa SOLO lo que cambia respecto a _defaults_all_off
##############################################################################
...
_profile_server() {
    # Server no tiene GNOME — los defaults de _defaults_all_off ya son correctos
    # Solo ajustar GDM_AUTOLOGIN porque no aplica sin GUI
    GDM_AUTOLOGIN="false"
}

##############################################################################
# PREGUNTAS MODULARES — cada bloque es una función reutilizable
##############################################################################
...
_ask_system_base() {
    ...
    echo "  ¿Cifrar disco con LUKS? (cifrado completo de la partición root)"
    echo "    Esto añade una capa de seguridad que protege los datos si el"
    echo "    equipo se pierde o es robado. Reduce ligeramente el rendimiento"
    echo "    I/O (~2-5%) y requiere introducir la contraseña al arrancar."
    echo "    Con LUKS, el keyring de GNOME se desbloquea automáticamente"
    echo "    sin contraseña adicional (como en Fedora)."
    read -p "  ¿LUKS? (s/n) [n]: " USE_LUKS
    USE_LUKS=$(_yn "$USE_LUKS" "false")
    if [ "$USE_LUKS" = "true" ]; then
        echo ""
        echo "  Contraseña LUKS (cifrado de disco):"
        echo "  (se pedirá en cada arranque antes de GDM)"
        while true; do
            read -s -p "  Contraseña LUKS: " LUKS_PASSWORD; echo ""
            read -s -p "  Confirmar:       " LUKS_PASSWORD_CONFIRM; echo ""
            [ "$LUKS_PASSWORD" = "$LUKS_PASSWORD_CONFIRM" ] && [ -n "$LUKS_PASSWORD" ] && break
...
##############################################################################
# FLUJO PERSONALIZADO (perfil 6 — solo componentes, sin extras/ajustes)
##############################################################################

_interactive_custom() {
    read -p "  ¿Escritorio GNOME? (s/n) [s]: " inst_gnome
    [[ ${inst_gnome:-s} =~ ^[SsYy]$ ]] && { INSTALL_GNOME="true"; GNOME_OPTIMIZE_MEMORY="true"; GNOME_TRANSPARENT_THEME="true"; INSTALL_MULTIMEDIA="true"; }
    ...
    if [ "$INSTALL_GNOME" = "true" ]; then
        ...
        [[ ${inst_gaming:-n} =~ ^[SsYy]$ ]] && { INSTALL_GAMING="true"; INSTALL_PROTONPLUS="true"; }
    fi
}

##############################################################################
# INTERACTIVE_CONFIG — orquestador principal
##############################################################################
...
interactive_config() {
    ...
    # Inicializar todo OFF y luego aplicar perfil
    _defaults_all_off

    case $profile_choice in
        1) _profile_desktop;     echo -e "  ${GREEN}✓ Perfil Escritorio${NC}" ;;
        2) _profile_development; echo -e "  ${GREEN}✓ Perfil Desarrollo${NC}" ;;
        3) _profile_gaming;      echo -e "  ${GREEN}✓ Perfil Gaming${NC}" ;;
        4) _profile_complete;    echo -e "  ${GREEN}✓ Perfil Completo${NC}" ;;
        5) _profile_server;      echo -e "  ${GREEN}✓ Perfil Servidor${NC}"
           echo ""
           read -p "  ¿Desarrollo CLI? (s/n) [n]: " inst_dev
           if [[ ${inst_dev:-n} =~ ^[SsYy]$ ]]; then
               INSTALL_DEVELOPMENT="true"; INSTALL_TOPGRADE="true"; INSTALL_LAZY_TOOLS="true"
               INSTALL_DOCKER_TOOLS="true"; INSTALL_N8N="true"; INSTALL_NETTOOLS="true"
               echo -e "  ${GREEN}✓ Desarrollo CLI activado${NC}"
           fi ;;
        6) echo -e "  ${CYAN}✓ Modo personalizado${NC}"; echo ""; _interactive_custom ;;
    esac
    echo ""
    ...
    # ── PASO 6: Resumen ──────────────────────────────────────────────────────
    _show_summary "$profile_choice"

    read -p "¿Guardar configuración? (s/n) [s]: " save_conf
    [[ ${save_conf:-s} =~ ^[SsYy]$ ]] && save_config
    echo ""
...
show_menu() {
    ...
    echo "  32) optimize-laptop        33) minimize-systemd"
...
run_all_modules() {
    ...
    if [ "${INSTALL_GNOME:-false}" = "true" ]; then
        ...
        echo "    Extensiones: V-Shell, caffeine, user-theme"
        echo ""
    fi
...
Estructura de módulos:
  ...
  3x = Optimización (laptop, systemd)
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════════════════╗"
echo -e "║              INSTALADOR DE UBUNTU                        ║"
echo -e "╠══════════════════════════════════════════════════════════╣"
echo -e "║                                                          ║"
echo -e "║  ${NC}${BOLD}1)${NC}${CYAN}  Instalación interactiva guiada                       ║"
echo -e "║      ${NC}${DIM}Configura paso a paso y ejecuta${NC}${CYAN}                     ║"
echo -e "║                                                          ║"
echo -e "║  ${NC}${BOLD}2)${NC}${CYAN}  Instalación automática                                ║"
echo -e "║      ${NC}${DIM}Usa config.yaml existente${NC}${CYAN}                           ║"
echo -e "║                                                          ║"
echo -e "╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
