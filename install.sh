...
chroot_mount() {
    ...
    if [ -z "$target" ] || [ ! -d "$target" ]; then
        echo "ERROR: chroot_mount: directorio no válido: '$target'"
        return 1
    fi

    # Replicación exacta de arch-chroot (arch-install-scripts/common.in)
    # arch-chroot monta filesystems NUEVOS, no bind mounts del host.
    # Esto funciona en cualquier entorno (VM, contenedor, live USB).

    mkdir -p "${target}/proc" "${target}/sys" "${target}/dev/pts" \
             "${target}/dev/shm" "${target}/run" "${target}/tmp"

    # Cada mount se encadena con && — si uno falla, no continúa
    mount -t proc proc "${target}/proc" -o nosuid,noexec,nodev &&
    mount -t sysfs sys "${target}/sys" -o nosuid,noexec,nodev,ro &&
    mount -t devtmpfs udev "${target}/dev" -o mode=0755,nosuid &&
    mount -t devpts devpts "${target}/dev/pts" -o mode=0620,gid=5,nosuid,noexec &&
    mount -t tmpfs shm "${target}/dev/shm" -o mode=1777,nosuid,nodev &&
    mount --bind /run "${target}/run" &&
    mount -t tmpfs tmp "${target}/tmp" -o mode=1777,strictatime,nodev,nosuid || {
        echo "ERROR: Fallo al montar pseudofilesystems en ${target}"
        # Limpiar lo que se haya montado parcialmente
    ...
    echo "✓  Pseudofilesystems montados en $target"
}

chroot_umount() {
    local target="$1"
    [ -z "$target" ] && return 0
    umount "${target}/tmp"     2>/dev/null || true
