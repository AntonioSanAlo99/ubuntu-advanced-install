...
run_module() {
    ...
    if [ "$VERBOSE_MODE" = "true" ]; then
        # Modo verbose: mostrar comandos ejecutados
        if bash -x "$module_path" 2>&1 | while IFS= read -r line; do
            # Filtrar líneas de bash -x (comenzan con +)
            if [[ "$line" =~ ^\+\+ ]]; then
                # Comando interno, mostrar en gris oscuro
                echo -e "\033[0;90m  → ${line#++}\033[0m"
            elif [[ "$line" =~ ^\+ ]]; then
                # Comando principal, mostrar en cyan
                echo -e "${CYAN}[CMD]${NC} ${line#+}"
            else
        ...
        # Modo normal: solo salida del módulo
