...
trap - ERR  # Se configura después en el bloque de traps
...
# ── Logging: funciones log_*() escriben a $LOG_FILE y terminal ────────────
# run_module() captura output de módulos con tee por módulo.
...
# ============================================================================
# STATUS BAR — barra fija en la última línea del terminal
# ============================================================================
# Un solo proceso controla el terminal: el módulo escribe su output a un
# fichero temporal. run_module() lee ese fichero con tail -f, imprime cada
# línea dentro de la scroll region (filas 1..N-1) y repinta la barra en
# la fila N. Cero race conditions porque solo hay un escritor al terminal.
...
_bar_setup() {
    _BAR_ROWS=$(tput lines 2>/dev/null) || _BAR_ROWS=24
    _BAR_COLS=$(tput cols 2>/dev/null) || _BAR_COLS=80
    # Scroll region: filas 1 a N-1 (la fila N queda fuera, reservada para la barra)
    printf '\033[1;%dr' "$(( _BAR_ROWS - 1 ))"
    # Mover cursor a la última fila del scroll (N-1) para que el output empiece ahí
    printf '\033[%d;1H' "$(( _BAR_ROWS - 1 ))"
    _bar_draw
}
...
_bar_draw() {
    ...
    local text_len=${#_BAR_TEXT}
    local overhead=$(( 12 + text_len ))
    local bar_size=$(( _BAR_COLS - overhead ))
...
trap '_bar_teardown 2>/dev/null; error_handler $LINENO' ERR
trap '_bar_teardown 2>/dev/null' EXIT
...
run_module() {
    ...
    if [ -n "$module_num" ] && [ -n "$module_total" ]; then
        _BAR_TEXT="[$module_num/$module_total] $module_label"
        start_pct=$(( (module_num - 1) * 100 / module_total ))
        end_pct=$(( module_num * 100 / module_total ))
    else
        _BAR_TEXT="$module_label"
    fi
    _BAR_PCT="$start_pct"
    local range=$(( end_pct - start_pct ))

    # ── Ejecutar módulo: output a fichero, lectura controlada ─────────────────
    # El módulo escribe a un FIFO. Este proceso lee cada línea del FIFO,
    # la imprime dentro de la scroll region y repinta la barra. Un solo
    # escritor al terminal = cero mezcla de output.
    local exit_code=0
    local fifo exit_file
    fifo=$(mktemp -u /tmp/.module-fifo.XXXXXX)
    exit_file=$(mktemp /tmp/.module-exit.XXXXXX)
    mkfifo "$fifo"

    # Activar scroll region y pintar barra inicial
    _bar_setup

    # Lanzar módulo en background, output al FIFO + log
    # El exit code se escribe directamente a exit_file (fd 3), no por stdout
    # stdin viene de /dev/tty para que módulos interactivos (read) funcionen
    {
        if [ "$VERBOSE_MODE" = "true" ]; then
            bash -x "$module_path" 2>&1
        else
            bash "$module_path" 2>&1
        fi
        echo $? >&3
    } 3>"$exit_file" < /dev/tty | tee -a "$module_log" > "$fifo" &
    local module_bg_pid=$!

    # Leer FIFO línea a línea — único proceso que escribe al terminal
    local line_num=0
    while IFS= read -r line; do
        printf '%s\n' "$line"
        line_num=$(( line_num + 1 ))
        # Interpolar porcentaje: curva logarítmica basada en líneas
        if [ "$range" -gt 0 ]; then
            local progress=$(( range * line_num / (line_num + 30) ))
            _BAR_PCT=$(( start_pct + progress ))
            [ "$_BAR_PCT" -ge "$end_pct" ] && _BAR_PCT=$(( end_pct - 1 ))
        fi
        _bar_draw
    done < "$fifo"

    wait "$module_bg_pid" 2>/dev/null
    if [ -f "$exit_file" ]; then
    ...
    # Barra final al 100% del rango del módulo
    _BAR_PCT="$end_pct"
    _bar_draw

    # Desactivar scroll region
    _bar_teardown
