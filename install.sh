...
# Reserva la última fila con scroll region. Todo el output de la barra va
# directamente a /dev/tty para no mezclarse con pipes (tee, etc.).
#
# El porcentaje avanza independientemente dentro de cada módulo:
#   - status_start recibe start_pct y end_pct (rango del módulo)
#   - El loop background interpola progresivamente en el tiempo
#   - Al finalizar el módulo, status_stop salta al end_pct
...
_setup_scroll_region() {
    ...
    # Scroll region: filas 1..(rows-1); la fila rows queda fuera (reservada)
    printf "\n" >/dev/tty
    printf "\0337" >/dev/tty
    printf "\033[1;%dr" "$(( rows - 1 ))" >/dev/tty
    printf "\0338" >/dev/tty
...
_restore_scroll_region() {
    ...
    printf "\033[1;%dr" "$rows" >/dev/tty
    # Ir a la última línea y limpiarla
    printf "\033[%d;1H" "$rows" >/dev/tty
    printf "\033[2K" >/dev/tty
    printf "\0338" >/dev/tty
}

_draw_status_bar() {
    # $1=texto  $2=porcentaje (0-100)
    # Todo el output va a /dev/tty para no interferir con pipes
    local text="$1"
    local percent="${2:-0}"
    ...
    # Guardar cursor → última fila → pintar → restaurar — todo a /dev/tty
    {
        printf "\0337"
...
_status_loop() {
    # $1=texto  $2=start_pct  $3=end_pct  $4=pct_file
    local text="$1"
    local start_pct="${2:-0}"
    local end_pct="${3:-0}"
    local pct_file="$4"
    ...
    while true; do
        local pct="$start_pct"
        if [ "$range" -gt 0 ]; then
            # Curva logarítmica: avanza rápido al inicio, se frena al 90% del rango
            # Fórmula: pct = start + range * (1 - 1/(1 + tick/20))
            # Esto llega a ~90% del rango en ~180 ticks (54s) y nunca al 100%
            local progress=$(( range * tick / (tick + 20) ))
            pct=$(( start_pct + progress ))
            [ "$pct" -ge "$end_pct" ] && pct=$(( end_pct - 1 ))
        fi
        echo "$pct" > "$pct_file" 2>/dev/null
        _draw_status_bar "$text" "$pct"
        sleep 0.3
        tick=$(( tick + 1 ))
    done
}
...
status_start() {
    # $1 = texto  $2 = start_pct (0-100)  $3 = end_pct (0-100)
    status_stop 2>/dev/null
    local start_pct="${2:-0}"
    local end_pct="${3:-$start_pct}"
    _STATUS_PCT_FILE=$(mktemp /tmp/.status_pct.XXXXXX)
    echo "$start_pct" > "$_STATUS_PCT_FILE"
    _setup_scroll_region
    _draw_status_bar "$1" "$start_pct"
    _status_loop "$1" "$start_pct" "$end_pct" "$_STATUS_PCT_FILE" &
    _SPINNER_PID=$!
    _STATUS_ACTIVE=true
...
status_stop() {
    ...
    [ -n "$_STATUS_PCT_FILE" ] && rm -f "$_STATUS_PCT_FILE" 2>/dev/null
    _STATUS_PCT_FILE=""
}
...
run_module() {
    ...
    # Status bar: barra estilo apt con porcentaje progresivo por módulo
    ...
    status_start "$status_text" "$start_pct" "$end_pct"
