...
# ============================================================================
# El módulo se ejecuta en background con stdout/stderr a un FIFO.
# El proceso principal lee el FIFO línea a línea, imprime dentro de la
# scroll region (filas 1..N-1) y repinta la barra en la fila N.
# Un solo proceso escribe al terminal → cero mezcla de output.
#
# Para módulos interactivos: stdin del módulo viene de /dev/tty,
# así que read -p funciona. El prompt del read llega por el FIFO
# (vía stderr→stdout) y se imprime en la scroll region. El usuario
# escribe su respuesta que va directo al read del módulo.

_BAR_ACTIVE=false

_bar_init() {
    # Activar scroll region y pintar barra
    local rows
...
_bar_cleanup() {
    ...
    printf '\0337\033[%d;1H\033[2K\0338' "$rows"
    _BAR_ACTIVE=false
}

# Ejecutar módulo con barra de progreso fija al fondo
run_module_with_bar() {
    ...
    # - stdout al FIFO vía tee (para log + barra de progreso)
    # - stderr directo al terminal (para prompts de read -p sin buffering)
    # - stdin de /dev/tty (para read interactivos)
    # En modo verbose: stderr también al pipe (para capturar bash -x traces)
    {
    ...
    # Leer FIFO — único escritor al terminal
    local n=0
...
run_module() {
    ...
    # ── Ejecutar módulo con barra fija al fondo ──────────────────────────────
    local exit_code=0
