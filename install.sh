...
# ============================================================================
# La barra se pinta directamente a /dev/tty tras cada línea de output del
# módulo, usando un filtro en el pipe. Sin proceso background, sin scroll
# region, sin race conditions.
...
status_start() {
    ...
    _STATUS_TEXT="$1"
    _STATUS_START_PCT="${2:-0}"
    _STATUS_END_PCT="${3:-$_STATUS_START_PCT}"
    _STATUS_ACTIVE=true
    # Reservar una línea vacía al fondo para la barra
    local rows
    rows=$(tput lines 2>/dev/null) || rows=24
    printf "\033[1;%dr" "$(( rows - 1 ))" >/dev/tty
    _draw_status_bar "$_STATUS_TEXT" "$_STATUS_START_PCT"
}

status_stop() {
    if [ "$_STATUS_ACTIVE" = true ]; then
        # Restaurar scroll region completa y limpiar barra
        local rows
        rows=$(tput lines 2>/dev/null) || rows=24
...
# Filtro de output: repinta la barra tras cada línea del módulo.
# El porcentaje interpola entre start_pct y end_pct con curva logarítmica.
# Uso: bash module.sh 2>&1 | _status_filter | tee -a logfile
_status_filter() {
    local line_num=0
    local text="$_STATUS_TEXT"
    local start="$_STATUS_START_PCT"
    local range=$(( _STATUS_END_PCT - _STATUS_START_PCT ))

    while IFS= read -r line; do
        printf '%s\n' "$line"
        line_num=$(( line_num + 1 ))
        # Curva logarítmica: progress = range * n / (n + 20)
        local pct="$start"
        if [ "$range" -gt 0 ]; then
            local progress=$(( range * line_num / (line_num + 20) ))
            pct=$(( start + progress ))
            [ "$pct" -ge "$_STATUS_END_PCT" ] && pct=$(( _STATUS_END_PCT - 1 ))
        fi
...
run_module() {
    ...
    if [ "$VERBOSE_MODE" = "true" ]; then
        bash -x "$module_path" 2>&1 | _status_filter | tee -a "$module_log" || exit_code=${PIPESTATUS[0]}
    else
        bash "$module_path" 2>&1 | _status_filter | tee -a "$module_log" || exit_code=${PIPESTATUS[0]}
    fi
