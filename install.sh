...
chroot_mount() {
    local target="$1"
    if [ -z "$target" ]; then
        echo "ERROR: chroot_mount llamado sin argumento TARGET"
        return 1
    fi
    if [ ! -d "$target" ]; then
        echo "ERROR: directorio TARGET no existe: $target"
        return 1
    fi

    echo "── Montando pseudofilesystems en $target ──"

    # Patrón Gentoo Handbook — referencia canónica para chroot con systemd
    # Ref: https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Base
    mkdir -p "${target}/proc" "${target}/sys" "${target}/dev" "${target}/run"

    # /proc — instancia nueva (mount --types proc), NO bind
    if ! mountpoint -q "${target}/proc" 2>/dev/null; then
        mount --types proc /proc "${target}/proc" || { echo "FALLO: mount proc"; return 1; }
        echo "  ✓ /proc montado"
    fi

    # /sys — rbind + rslave (systemd necesita rslave)
    if ! mountpoint -q "${target}/sys" 2>/dev/null; then
        mount --rbind /sys "${target}/sys"             || { echo "FALLO: mount sys"; return 1; }
    ...
    if ! mountpoint -q "${target}/dev" 2>/dev/null; then
        mount --rbind /dev "${target}/dev"             || { echo "FALLO: mount dev"; return 1; }
    ...
    if ! mountpoint -q "${target}/run" 2>/dev/null; then
        mount --bind /run "${target}/run"              || { echo "FALLO: mount run"; return 1; }
        mount --make-slave "${target}/run" 2>/dev/null || true
        echo "  ✓ /run montado"
    fi

    # /dev/shm — necesario si es symlink (caso non-Gentoo media)
    if test -L /dev/shm; then
        rm -f "${target}/dev/shm"
        mkdir -p "${target}/dev/shm"
        mount --types tmpfs --options nosuid,nodev,noexec shm "${target}/dev/shm"
        chmod 1777 "${target}/dev/shm"
        echo "  ✓ /dev/shm montado (tmpfs)"
    fi
    ...
    if ! mountpoint -q "${target}/proc"; then
        echo "ERROR CRÍTICO: /proc NO quedó montado en ${target}/proc"
        return 1
    fi
    if ! mountpoint -q "${target}/dev"; then
        echo "ERROR CRÍTICO: /dev NO quedó montado en ${target}/dev"
        return 1
    fi

    echo "── Pseudofilesystems OK ──"
}

chroot_umount() {
    local target="$1"
    [ -z "$target" ] && return 0
    echo "── Desmontando pseudofilesystems de $target ──"
    # Orden inverso; -R para rbind, -l para lazy
    mountpoint -q "${target}/dev/shm" 2>/dev/null && umount "${target}/dev/shm" 2>/dev/null || true
    mountpoint -q "${target}/run"     2>/dev/null && umount -R "${target}/run"  2>/dev/null || true
    mountpoint -q "${target}/dev"     2>/dev/null && umount -R "${target}/dev"  2>/dev/null || true
    mountpoint -q "${target}/sys"     2>/dev/null && umount -R "${target}/sys"  2>/dev/null || true
    mountpoint -q "${target}/proc"    2>/dev/null && umount -l "${target}/proc" 2>/dev/null || true
    echo "── Pseudofilesystems desmontados ──"
}
...
full_automatic_install() {
    ...
    chroot_mount "$TARGET" || { log_error "No se pudieron montar pseudofilesystems en $TARGET"; exit 1; }
    trap 'chroot_umount "$TARGET"' EXIT

    # Verificación explícita — si /proc no está montado, todo fallará
    if ! mountpoint -q "${TARGET}/proc"; then
        log_error "FALLO CRÍTICO: /proc no está montado en ${TARGET}/proc"
        exit 1
    fi
