...
5. Aplica optimizaciones de rendimiento (oomd, DNS-over-TLS, minimize-systemd)
6. Verifica la instalación y genera un informe

Todo es interactivo con perfiles predefinidos, o automático via `config.yaml`.
...
```
install.sh                    Orquestador principal (perfiles + ejecución)
...
modules/
  ...
  04-install-bootloader.sh    GRUB + parámetros del kernel (3 niveles)
  05-configure-network.sh     NetworkManager + systemd-resolved (DNS-over-TLS)
  10-install-gnome-core.sh    GNOME Shell + extensiones + dconf + GDM + rtkit + PipeWire
  11-configure-gnome-user.sh  Primer login, wallpaper, V-Shell, carpetas app grid
  12-optimize-gnome.sh        Optimización de memoria (GC, Tracker, systemd limits)
  13-configure-gnome-theme.sh Tema transparente Adwaita-Transparent
  20-install-multimedia.sh    VLC, Fooyin, Spotify, códecs
  21-install-fonts.sh         Nerd Fonts, Microsoft, sistema
  22-configure-wireless.sh    WiFi + Bluetooth + WirePlumber codecs + UPower
  23-install-development.sh   Rust + dev tools (VSCode, Node, n8n, Docker tools, etc.)
  24-configure-gaming.sh      Steam, Heroic, drivers GPU, MangoHud, kernel PsyCachy
  25-install-extras.sh        OnlyOffice, OBS, Obsidian, Mullvad, extras
  ...
  91-generate-report.sh       Informe de instalación
  92-backup-config.sh         Respaldo de configuración
files/
  99-ubuntu-advanced-install.gschema.override   Defaults GNOME
tools/
  post-install-test.sh        Tests post-instalación
