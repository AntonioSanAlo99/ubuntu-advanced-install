...
El sistema ahora incluye **configuración interactiva paso a paso** con menús numerados.
...
# o desde el menú:
sudo ./install.sh
# Opción 3) Configuración interactiva guiada
```
...
[1/8] Versión de Ubuntu
  ...
  4) Ubuntu 24.10 (Oracular Oriole)
  5) Ubuntu 25.04 (Plucky Puffin) - Desarrollo

Selecciona versión (1-5) [1]: 
```

**Preguntas guiadas:**
1. **Versión Ubuntu** (5 opciones)
2. **Hostname** (texto libre)
3. **Usuario** (texto libre)
4. **Tipo hardware** (Laptop/Desktop)
...
Al final muestra **resumen completo** y pregunta si guardar.

#### **2. Configuración manual (archivo)**
```bash
# Editar directamente
nano config.env
...
# O desde el menú
sudo ./install.sh
# Opción 4) Editar config.env manualmente
```

#### **3. Usar configuración existente**
Si `config.env` existe:
```bash
sudo ./install.sh

✓ Configuración encontrada: config.env

¿Usar configuración existente? (s/n/e=editar) [s]:
  s = usar tal cual
  n = configuración interactiva
  e = editar con nano
```

### Flujos de trabajo:

#### **A. Primera vez (sin config.env)**
```bash
sudo ./install.sh

No se encontró archivo de configuración

Opciones:
  1) Configuración interactiva (recomendado)
  2) Crear config.env por defecto

Selecciona opción (1-2) [1]: 1

# → Configuración guiada paso a paso
```

#### **B. Con configuración existente**
```bash
sudo ./install.sh --auto
# Usa config.env directamente

# O interactivo:
sudo ./install.sh --interactive
# Pregunta en cada paso
```

#### **C. Solo configurar (sin instalar)**
```bash
sudo ./install.sh --config
# Solo genera config.env
```

### Ejemplos de uso:

**Escenario 1: Usuario nuevo (quiere ayuda)**
```bash
cd ubuntu-advanced-install
sudo ./install.sh

# Elige: 1) Configuración interactiva
# Responde preguntas guiadas
# Al final: ¿Proceder con instalación? s
# → Instalación automática completa
```

**Escenario 2: Usuario avanzado (sabe lo que quiere)**
```bash
nano config.env
# Edita manualmente
...
**Escenario 3: Debugging (probar configuración)**
```bash
sudo ./install.sh --config
# Solo genera config.env
...
sudo ./install.sh --module 01-prepare-disk
# Probar solo particionamiento
```

---

## 📝 Resumen de opciones de línea de comandos

```bash
sudo ./install.sh                  # Menú interactivo
sudo ./install.sh --auto           # Instalación automática
sudo ./install.sh --interactive    # Instalación paso a paso
sudo ./install.sh --config         # Solo configuración guiada
sudo ./install.sh --module 11-*    # Ejecutar módulo específico
sudo ./install.sh --list           # Listar módulos
sudo ./install.sh --help           # Ayuda
```
...
El sistema configura **automáticamente** todos los componentes de Ubuntu:

```bash
# En debootstrap (módulo 02)
--components=main,restricted,universe,multiverse
```

#### **main**
- Software libre oficial
- Soportado por Canonical
- Actualizaciones de seguridad garantizadas

#### **restricted**
- Drivers propietarios necesarios
- NVIDIA, AMD, Intel wireless
- Firmware de hardware

#### **universe**
- Software libre mantenido por comunidad
- Miles de paquetes adicionales
- Soporte limitado

#### **multiverse**
- Software con restricciones de copyright
- Códecs multimedia propietarios
- Software con licencias especiales

### sources.list generado:
...
# Backports (opcional, comentado por defecto)
...
sudo nano /mnt/ubuntu/etc/apt/sources.list
# Descomentar línea de backports
...
# Ver qué componentes están activos
apt-cache policy

# Buscar paquete en todos los repositorios
apt-cache search <paquete>
```

### Ejemplos de paquetes por componente:

**main:**
- firefox, libreoffice, vim, gcc, python3

**restricted:**
- nvidia-driver-*, firmware-*, bcmwl-kernel-source

**universe:**
- vlc, gimp, inkscape, steam, discord

**multiverse:**
- ubuntu-restricted-extras, rar, lame

---

## 🎯 ¿Qué paquetes estarán disponibles?

Con **main + restricted + universe + multiverse** tienes acceso a:

- ✅ **80,000+ paquetes** (vs ~8,000 solo con main)
- ✅ Drivers propietarios (NVIDIA, WiFi, etc)
- ✅ Códecs multimedia completos
- ✅ Software de comunidad (Steam, Discord, etc)
- ✅ Herramientas de desarrollo modernas

**El sistema está configurado para tener acceso completo al ecosistema Ubuntu.** 📦✨
