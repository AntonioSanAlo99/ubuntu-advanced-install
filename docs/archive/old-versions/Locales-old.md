...
## ⚠️ Warnings de Perl (Normales)

Durante `locale-gen` puedes ver:
...
**Esto es NORMAL y esperado.** Perl intenta usar el locale mientras se está generando.

### Cuándo aparece (correctamente)

✅ **Una vez durante `locale-gen`** - Normal, el locale se está generando
❌ **NO después** - Si aparece en `apt update` o `apt install`, hay un problema de orden

### Orden correcto para evitar warnings repetidos

```bash
# 1. Editar /etc/locale.gen (sin configurar LANG)
sed -i 's/^# *es_ES.UTF-8/es_ES.UTF-8/' /etc/locale.gen

# 2. Generar locale (puede mostrar warning - OK)
locale-gen

# 3. Configurar LANG (DESPUÉS de generar)
cat > /etc/default/locale << EOF
LANG=es_ES.UTF-8
LANGUAGE=es_ES:es
EOF

# 4. Usar apt (sin warnings)
apt update
apt install language-pack-es
```

Si se configura LANG antes de generar el locale, apt mostrará warnings en cada ejecución.
