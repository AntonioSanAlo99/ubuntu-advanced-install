...
- **Gradia**: eliminado del proyecto completo. Variable `INSTALL_GRADIA`, bloque de compilación (~170 líneas), keybindings dconf, campo YAML y entrada en app grid eliminados.
- **gnome-keyring / PAM**: eliminada la configuración manual de PAM (common-auth, common-session, gdm-password, gdm-autologin), directorio keyrings, autostart unlock-keyring. Paquetes `gnome-keyring` y `libpam-gnome-keyring` eliminados del bloque core.
- **KDiskMark**: eliminado del bloque de utilidades.
- **update-manager**: eliminado (obsoleto para estándares GNOME).

### Extensiones GNOME
- **Caffeine**: reescrita la instalación. Antes hacía `git clone` del repo (estructura inestable); ahora descarga desde `extensions.gnome.org` API con la versión de GNOME Shell correcta, igual que Arc Menu.
- **V-Shell**: añadidas keys dconf `module-dash-enabled=false` y `module-panel-enabled=false` para desactivar los módulos Dash y Panel, evitando conflictos con Dash to Panel / Ubuntu Dock.

### Compatibilidad GNOME 49 / 50
- **experimental-features condicional**: `xwayland-native-scaling` solo se aplica en GNOME < 50. GNOME 50 lo activa por defecto junto con VRR.
- **VRR condicional** (módulo 24): el autostart de experimental-features solo se crea para GNOME 46-49. GNOME 50+ no necesita intervención.
- **screen-time-limits** y **sort-directories-first** ya condicionados desde v5.4.0.

### Sesión y audio
- **rtkit** (RealtimeKit): añadido al bloque core. PipeWire obtiene scheduling FIFO realtime para el thread de audio, eliminando latencia y xruns.
- **dbus-user-session**: añadido. Necesario para que systemd-logind gestione la sesión gráfica y los portales XDG funcionen.
- **polkitd**: añadido. Necesario para acciones privilegiadas (suspend, reboot, mount) sin pedir root.
- **pipewire-pulse + wireplumber**: añadidos explícitamente. En debootstrap pueden no instalarse por defecto.
- **WirePlumber bluetooth** (módulo 22): configuración `/etc/wireplumber/bluetooth.lua.d/` con prioridad de codecs (LDAC > AAC > aptX > SBC), autoconnect y hardware volume.
- **UPower** (módulo 22): umbrales de batería configurados (low=15%, critical=5%, action=3%).

### Optimización de memoria (módulo 12)
- **GJS GC**: intervalo de garbage collector reducido de ~30s a 10s (`GJS_GARBAGE_COLLECTOR_PERIOD`). Libera objetos JavaScript muertos de GNOME Shell más rápido.
- **systemd user slice**: `MemoryMax=85%`, `MemoryHigh=80%`. Previene que la sesión gráfica consuma toda la RAM, trabajando con systemd-oomd.

---

### Fixes
- **Salto de línea Mullvad**: eliminado `echo ""` entre Obsidian y Mullvad en `_ask_extras()`.
- **Cuadro del instalador**: reescrito con 58 caracteres `═` consistentes. Forma cerrada sin aberturas.
- **edge-tiling**: añadido `edge-tiling=true` al gschema override (arrastrar ventanas a bordes).
- **Comentarios obsoletos**: eliminadas referencias a locks, 07-install-rust, Gradia.
