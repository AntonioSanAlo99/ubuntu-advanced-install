...
if grep -q "Intel" /proc/cpuinfo; then
    ...
    echo "✓ CPU Intel detectada: $CPU_MODEL"
elif grep -q "AMD" /proc/cpuinfo; then
    CPU_VENDOR="AMD"
    CPU_MODEL=$(grep "model name" /proc/cpuinfo | head -1 | cut -d: -f2 | xargs)
    echo "✓ CPU AMD detectada: $CPU_MODEL"
else
    echo "⚠ CPU no reconocida"
    CPU_VENDOR="Unknown"
fi
...
if command -v dmidecode >/dev/null 2>&1; then
    ...
    echo "✓ Laptop: $LAPTOP_VENDOR $LAPTOP_MODEL"
    ...
    if [ "$SETUP_UNDERVOLT" = "s" ] || [ "$SETUP_UNDERVOLT" = "S" ]; then
        ...
        case $UV_LEVEL in
            1)  # CONSERVADOR - MUY SEGURO
                ...
                UV_IO=-30
                echo ""
                echo "✓ Nivel CONSERVADOR seleccionado:"
            ...
            2)  # MODERADO - SEGURO
                ...
                UV_IO=-40
                echo ""
                echo "✓ Nivel MODERADO seleccionado:"
            ...
            3)  # AVANZADO - PERSONALIZADO
                ...
                echo ""
                echo "⚠️  Validación activa: Se rechazarán valores peligrosos (>-150mV)"
                echo ""
                ...
                while true; do
                    ...
                    # Validar rango
                    if [ "$UV_CPU" -gt 0 ]; then
                        echo "⚠️  Error: Debe ser negativo"
                        continue
                    fi
                    if [ "$UV_CPU" -lt -150 ]; then
                        echo "⚠️  Error: Demasiado agresivo (máximo -150mV)"
                ...
                while true; do
                    ...
                    if [ "$UV_GPU" -gt 0 ] || [ "$UV_GPU" -lt -150 ]; then
                        echo "⚠️  Error: Rango válido: 0 a -150mV"
                        continue
                    fi
                    break
                done
                ...
                ;;
            *)
                echo "⚠️  Opción inválida, usando valores conservadores"
                UV_CPU=-50
                UV_GPU=-40
                UV_CACHE=-50
                UV_SA=-30
...
SENSEOF
        ...
        *)
            echo "⚠️  No se pudo determinar método de control de ventiladores"
    ...
    if [ "$FAN_METHOD" = "thinkfan" ]; then
        ...
        echo ""
        echo "⚠️  Requiere configuración manual:"
    ...
    echo "✓ Control de ventiladores omitido"
fi
...
echo "✓ Thermald instalado y habilitado"
THERMALEOF
        echo "✓ Thermald configurado"
    else
        echo "✓ Thermald omitido"
    fi
fi
...
if [ "$SETUP_FANS" = "s" ] || [ "$SETUP_FANS" = "S" ]; then
    echo "✓ Control ventiladores: $FAN_METHOD"
fi
