...
NODESOURCE_EOF
    ...
    apt update
    apt install -y $APT_FLAGS nodejs
...
# Actualiza APT, Snap, Flatpak, Cargo, pip, npm, firmware, etc.
# Método preferido: .deb de GitHub releases (sin dependencias de Rust)
# Fallback: cargo install topgrade (si Rust está instalado)
# ============================================================================
...
if [[ "\$INSTALL_TOPGRADE" =~ ^[SsYy]$ ]]; then
    ...
    # Método 1: Descargar .deb de GitHub releases (preferido)
    TOPGRADE_DEB_URL=\$(curl --max-time 30 --retry 3 -s https://api.github.com/repos/topgrade-rs/topgrade/releases/latest \
        | grep "browser_download_url.*amd64\.deb\"" | cut -d '"' -f 4 | head -1)

    if [ -n "\$TOPGRADE_DEB_URL" ]; then
        echo "  Descargando .deb desde GitHub releases..."
        if wget --timeout=30 --tries=3 -q "\$TOPGRADE_DEB_URL" -O /tmp/topgrade.deb; then
            if dpkg -i /tmp/topgrade.deb 2>/dev/null; then
                TOPGRADE_INSTALLED=true
                echo "  ✓ Topgrade instalado vía .deb"
            else
                apt install -f -y 2>/dev/null
                if dpkg -i /tmp/topgrade.deb 2>/dev/null; then
                    TOPGRADE_INSTALLED=true
                    echo "  ✓ Topgrade instalado vía .deb (tras resolver deps)"
                fi
            fi
            rm -f /tmp/topgrade.deb
        else
            echo "  ⚠ Error descargando .deb de Topgrade"
        fi
    fi

    # Método 2: cargo install (si Rust está disponible y .deb falló)
    if [ "\$TOPGRADE_INSTALLED" = "false" ]; then
        if [ -n "\$USERNAME" ] && [ -d "/home/\$USERNAME/.cargo/bin" ]; then
            echo "  .deb no disponible — instalando vía cargo..."
            su - "\$USERNAME" -c "source ~/.cargo/env && cargo install topgrade 2>&1" && {
                TOPGRADE_INSTALLED=true
                echo "  ✓ Topgrade instalado vía cargo en ~/.cargo/bin/"
            } || {
                echo "  ⚠ Error compilando topgrade con cargo"
            }
        elif command -v cargo >/dev/null 2>&1; then
            echo "  .deb no disponible — instalando vía cargo (sistema)..."
            cargo install topgrade 2>&1 && {
                TOPGRADE_INSTALLED=true
                echo "  ✓ Topgrade instalado vía cargo"
            } || {
                echo "  ⚠ Error compilando topgrade con cargo"
            }
        fi
    ...
    if [ "\$TOPGRADE_INSTALLED" = "false" ]; then
        echo "  ⚠ Topgrade no se pudo instalar"
        echo "  Puedes instalarlo después:"
        echo "    • Descargar .deb: https://github.com/topgrade-rs/topgrade/releases"
        echo "    • Con cargo:      cargo install topgrade"
    else
        # Crear configuración básica para el usuario
        if [ -n "\$USERNAME" ] && id "\$USERNAME" &>/dev/null; then
            TOPGRADE_CONFIG_DIR="/home/\$USERNAME/.config"
            # .config ya existe con el propietario correcto: useradd -m
            # lo creó copiando /etc/skel (que incluye .config desde 03-configure-base)
            if [ ! -f "\$TOPGRADE_CONFIG_DIR/topgrade.toml" ]; then
                cat > "\$TOPGRADE_CONFIG_DIR/topgrade.toml" << 'TOPGRADECONF'
# Topgrade configuration
# Docs: https://github.com/topgrade-rs/topgrade/wiki/Step-list

# Preguntar antes de actualizar cada sección
#assume_yes = false

# Limpiar paquetes obsoletos después de actualizar
#cleanup = true

# Desactivar secciones que no uses:
#[linux]
#disable = ["snap", "flatpak"]
TOPGRADECONF
                chown "\$(id -u "\$USERNAME"):\$(id -g "\$USERNAME")" "\$TOPGRADE_CONFIG_DIR/topgrade.toml"
            fi
            echo ""
            echo "  Uso: topgrade          (actualizar todo)"
            echo "       topgrade -n       (dry-run, solo muestra qué haría)"
            echo "       topgrade --only apt flatpak  (solo fuentes específicas)"
            echo "  Config: ~/.config/topgrade.toml"
        fi
    fi
else
    echo "⊘ Topgrade no instalado"
    echo "  Puedes instalarlo después: https://github.com/topgrade-rs/topgrade/releases"
fi
