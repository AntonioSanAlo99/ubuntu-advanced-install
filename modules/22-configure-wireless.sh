...
# REQUIERE: TARGET, HAS_WIFI, HAS_BLUETOOTH
# PRODUCE:  WiFi + Bluetooth configurados
...
HAS_WIFI="${HAS_WIFI:-true}"
HAS_BLUETOOTH="${HAS_BLUETOOTH:-true}"
...
if [ "\$HAS_BLUETOOTH" = "true" ]; then
    ...
    # ── WirePlumber: perfil bluetooth de alta calidad ─────────────────────────
    # WirePlumber (PipeWire session manager) gestiona bluetooth A2DP/HFP.
    # Por defecto Ubuntu ya configura PipeWire + WirePlumber, pero sin
    # preferencia de codec. Esta configuración:
    #   - Prioriza codecs de alta calidad (AAC, LDAC, aptX HD) sobre SBC
    #   - Autoconecta dispositivos bluetooth conocidos al arrancar
    #   - Evita conflictos con bluez usando el backend nativo de WirePlumber
    mkdir -p /etc/wireplumber/bluetooth.lua.d
    cat > /etc/wireplumber/bluetooth.lua.d/51-ubuntu-advanced.lua << 'WPEOF'
-- Priorizar codecs de alta calidad para audio bluetooth
bluez_monitor.properties = {
  -- Usar backend nativo de WirePlumber (evita conflictos con ofono/hsphfpd)
  ["bluez5.headset-roles"] = "[ hsp_hs hsp_ag hfp_hf hfp_ag ]",
  -- Autoconectar dispositivos conocidos
  ["bluez5.auto-connect"] = "[ hfp_hf hsp_hs a2dp_sink ]",
  -- Codecs preferidos en orden de prioridad (mejores primero)
  ["bluez5.codecs"] = "[ ldac aac aptx aptx_hd sbc_xq sbc ]",
  -- Hardware volume (control de volumen directo en el dispositivo)
  ["bluez5.enable-hw-volume"] = true,
}
WPEOF
    echo "✓  WirePlumber: perfil bluetooth configurado (LDAC > AAC > aptX > SBC)"

    # ── UPower: política de bluetooth en batería ─────────────────────────────
    # En portátiles, configurar UPower para no desactivar bluetooth al cambiar
    # a batería (comportamiento molesto por defecto en algunos entornos).
    if [ -f /etc/UPower/UPower.conf ]; then
        # IgnoreLid=false es el default correcto (no tocar)
        # PercentageLow/Critical/Action configurados con valores razonables
        sed -i 's/^#\?PercentageLow=.*/PercentageLow=15/' /etc/UPower/UPower.conf
        sed -i 's/^#\?PercentageCritical=.*/PercentageCritical=5/' /etc/UPower/UPower.conf
        sed -i 's/^#\?PercentageAction=.*/PercentageAction=3/' /etc/UPower/UPower.conf
        echo "✓  UPower: umbrales de batería configurados (low=15%, critical=5%, action=3%)"
    fi

    echo "✓  Bluetooth configurado (bluez + WirePlumber + UPower)"
else
    echo ""
