...
# Estos keys también están en gschema.override, pero se duplican aquí
# para garantizar que aplican incluso si el usuario ya tenía un valor
# previo en su ~/.config/dconf/user (system-db no puede sobreescribir
# el user-db, pero sí establece un default claro para nuevos usuarios).
[org/gnome/desktop/privacy]
remember-app-usage=false
remember-recent-files=false

# ── xwayland-native-scaling — escalado correcto para apps X11 en Wayland ─────
# Necesario para Spotify y otras apps forzadas a X11 en monitores con
# escalado fraccional. Se incluye como default en el system-db; si el
# usuario habilita VRR/HDR en gaming (16-configure-gaming.sh), ese
# autostart añadirá esas features preservando esta.
[org/gnome/mutter]
...
echo "✓  dconf system-db configurado (perfil user→local, welcome-dialog, experimental-features)"
DCONF_SYSTEM
