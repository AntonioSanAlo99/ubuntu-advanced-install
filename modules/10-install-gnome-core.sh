...
apt install -y kdiskmark 2>/dev/null \
...
# ── No overview at start-up (git clone → system-wide) ───────────────────────
...
if git clone --depth 1 -q https://github.com/fthx/no-overview.git \
    ...
    # Inyectar la versión de GNOME instalada en shell-version del metadata.json
    # para garantizar compatibilidad (upstream puede tardar en actualizar).
    ...
    # Desactivar validación de versión de extensiones (por si acaso)
...
if [ "\$GDM_AUTOLOGIN_ENABLED" = "true" ]; then
    # Autologin: pre-crear keyring con contraseña vacía
    if [ -n "\$USERNAME" ]; then
    ...
    echo "✓  GNOME Keyring: login.keyring con contraseña vacía (autologin)"
else
    # Login con contraseña: solo preparar directorio, PAM crea el keyring
    if [ -n "\$USERNAME" ]; then
        KEYRING_DIR="/home/\$USERNAME/.local/share/keyrings"
    ...
    echo "✓  GNOME Keyring: PAM desbloqueará automáticamente con contraseña del login"
fi
...
# sigma=4 (casi transparente), brightness=0.85 (cristal claro).
# Overview: sin blur (se ve el wallpaper nítido a través del cristal).
# App folders: blur suave (sigma=8) para legibilidad del texto sobre fondos.
