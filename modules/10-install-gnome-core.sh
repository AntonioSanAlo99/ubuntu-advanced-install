...
apt-get install -y \
    git meson ninja-build blueprint-compiler gettext \
    python3 python3-gi python3-gi-cairo python3-pil gir1.2-gtk-4.0 \
    gir1.2-adw-1 gir1.2-gtksource-5 gir1.2-xdpgtk4-1.0 \
...
if git clone --depth 1 https://github.com/AlexanderVanhee/Gradia.git "\$GRADIA_BUILD/gradia" 2>/dev/null; then
    ...
    if meson setup builddir --prefix=/usr 2>&1 | tail -3; then
        if ninja -C builddir 2>&1 | tail -3; then
            ninja -C builddir install 2>/dev/null
            GRADIA_OK=true
        fi
    fi
    cd /
...
if [ "\$GRADIA_OK" = "true" ]; then
    ...
    # Configurar como herramienta de screenshot por defecto en GNOME
    # GNOME usa org.gnome.Shell.Screenshot internamente, pero el atajo
    # Print Screen puede redirigirse a Gradia via custom keybinding.
    # Método: desactivar el screenshot nativo de GNOME Shell y asignar
    # Print Screen a Gradia directamente.

    # Nota: la integración con el portal se hace automáticamente por Gradia
    # al usar org.freedesktop.portal.Screenshot — no necesita config extra.
...
# gnome-power-statistics muestra detalles avanzados: ciclos, salud, tasa de
# descarga, historial de carga. Solo se instala si IS_LAPTOP=true.
IS_LAPTOP_VAR="$IS_LAPTOP"
if [ "\$IS_LAPTOP_VAR" = "true" ]; then
    echo ""
    echo "Instalando monitor de batería..."
    apt-get install -y gnome-power-manager 2>/dev/null \
        || apt-get install -y power-profiles-daemon 2>/dev/null || true
    echo "✓  Monitor de batería instalado (detalles en Configuración → Energía)"
fi
