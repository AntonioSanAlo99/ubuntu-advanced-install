...
# gnome-keyring necesita PAM correctamente configurado para:
#   1. Login con contraseña (gdm-password): pam_gnome_keyring desbloquea el
#      keyring "login" usando la misma contraseña del usuario.
#   2. Autologin (gdm-autologin): no hay contraseña → el keyring debe
#      crearse/existir sin contraseña para que se desbloquee automáticamente.
#
# pam-auth-update en chroot a veces falla (sin D-Bus). Se fuerza y luego
# se verifica manualmente que las líneas estén en los ficheros PAM.
# ============================================================================

DEBIAN_FRONTEND=noninteractive pam-auth-update --enable gnome-keyring 2>/dev/null || true

# Asegurar que las líneas de gnome-keyring están en gdm-password, gdm-autologin y login
for pam_file in /etc/pam.d/gdm-password /etc/pam.d/gdm-autologin /etc/pam.d/login; do
    [ ! -f "\$pam_file" ] && continue

    # auth: pam_gnome_keyring.so — desbloquea el keyring con la contraseña del login
    ...
    # session: pam_gnome_keyring.so auto_start — arranca el daemon del keyring
...
# ── Autologin: keyring sin contraseña ─────────────────────────────────────────
# Con autologin no hay prompt de contraseña → pam_gnome_keyring no recibe
# credenciales → si el keyring "login" tiene contraseña, apps como Chrome
# piden desbloqueo manual al iniciar.
#
# Solución: pre-crear el keyring "login" vacío y sin contraseña.
# gnome-keyring-daemon usa archivos binarios en ~/.local/share/keyrings/.
# El fichero "login.keyring" se inicializa sin contraseña creando un
# "default-keyring" apuntando a "login" — el daemon lo recrea en el
# primer arranque si no existe, pero con contraseña vacía solo si PAM
# no le pasó ninguna contraseña (caso autologin).
#
# Para garantizarlo: crear el directorio y el fichero "default" indicando
# que el keyring por defecto es "login". El daemon lo creará vacío.
