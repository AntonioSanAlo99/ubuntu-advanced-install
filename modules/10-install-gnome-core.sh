...
# ── Keyring "login" sin contraseña ────────────────────────────────────────────
# Pre-crear ~/.local/share/keyrings/ con "default" apuntando a "login" y SIN
# fichero login.keyring → gnome-keyring-daemon lo crea en el primer arranque
# con contraseña vacía. Así nunca aparece el emergente de desbloqueo, ni con
# autologin ni con login normal.
#
# Ref: ArchWiki GNOME/Keyring — "set a blank password for the keyring"
# Ref: help.gnome.org/users/seahorse/stable/keyring-create — "leave both
#      fields blank if you want your keyring to remain unlocked at all times"
if [ -n "\$USERNAME" ]; then
...
# /etc/skel para futuros usuarios
mkdir -p /etc/skel/.local/share/keyrings
echo "login" > /etc/skel/.local/share/keyrings/default
echo "✓  GNOME Keyring: sin contraseña (nunca pedirá desbloqueo)"
