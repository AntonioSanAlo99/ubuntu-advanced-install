...
# ── Sesión: dbus-user-session + pipewire-pulse ──────────────────────────────
# dbus-user-session: permite a systemd-logind gestionar la sesión gráfica.
# Sin él, loginctl no ve la sesión y los portales XDG (screenshot, file
# chooser) pueden fallar silenciosamente.
...
echo "✓  Sesión: dbus-user-session + pipewire-pulse + wireplumber"
...
# ── GNOME Keyring — sin encriptación (contraseña vacía) ──────────────────────
# Instalar gnome-keyring y configurar el keyring "login" con contraseña vacía.
# Esto evita el popup "Unlock default keyring" al abrir Chrome/apps que usan
# el Secret Service API. El contenido del keyring se almacena sin cifrar.
# Ref: https://wiki.archlinux.org/title/GNOME/Keyring#With_a_blank_password
apt-get install -y gnome-keyring libpam-gnome-keyring 2>/dev/null || true

if [ -n "\$USERNAME" ] && [ -d "/home/\$USERNAME" ]; then
    KEYRING_DIR="/home/\$USERNAME/.local/share/keyrings"
    mkdir -p "\$KEYRING_DIR"

    # Crear keyring "login" con contraseña vacía
    # El formato del fichero es: primera línea = header, segunda = contraseña vacía (salt)
    # gnome-keyring-daemon interpreta un fichero vacío + default como sin contraseña
    echo -n "" | gnome-keyring-daemon --unlock 2>/dev/null || true

    # Escribir un keyring default vacío sin contraseña
    cat > "\$KEYRING_DIR/default" << 'DKEOF'
login
DKEOF

    chown -R "\$USERNAME:\$USERNAME" "/home/\$USERNAME/.local"
    echo "✓  GNOME Keyring: configurado sin encriptación (contraseña vacía)"
fi

# Skel para futuros usuarios
mkdir -p /etc/skel/.local/share/keyrings
cat > /etc/skel/.local/share/keyrings/default << 'SKELKEOF'
login
SKELKEOF
...
# Desactivar módulos Dash y Panel de V-Shell para evitar conflictos
# con Dash to Panel / Ubuntu Dock que gestionan su propia barra y dock.
module-dash-enabled=false
module-panel-enabled=false
# Overview Background — wallpaper con blur suave en vez de gris sólido
