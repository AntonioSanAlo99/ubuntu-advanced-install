...
# ============================================================================
# APPIMAGE THUMBNAILER (compilado desde fuente — kem-a)
...
apt install -y --no-install-recommends \
    ...
    libglib2.0-dev libgdk-pixbuf-2.0-dev librsvg2-dev libcairo2-dev 2>/dev/null

THUMBNAILER_OK=false
if command -v meson >/dev/null 2>&1; then
    cd /tmp
    rm -rf appimage-thumbnailer
    if git clone --depth 1 https://github.com/kem-a/appimage-thumbnailer.git 2>/dev/null; then
        cd appimage-thumbnailer
        if meson setup build --prefix=/usr/local 2>/dev/null && ninja -C build 2>/dev/null; then
            ninja -C build install 2>/dev/null
            THUMBNAILER_OK=true
            echo "✓  AppImage thumbnailer compilado e instalado"
        else
            echo "  ⚠ Error compilando thumbnailer"
        fi
        cd /tmp
        rm -rf appimage-thumbnailer
    else
        echo "  ⚠ No se pudo clonar repositorio del thumbnailer"
    fi
else
    echo "  ⚠ meson no disponible, no se puede compilar thumbnailer"
fi

if [ "\$THUMBNAILER_OK" = false ]; then
    echo "  Las miniaturas de AppImages no se mostrarán en Nautilus"
    echo "  Instalar manualmente: https://github.com/kem-a/appimage-thumbnailer"
fi
...
if [ -n "\$APPMANAGER_URL" ]; then
    ...
    if wget --timeout=30 --tries=3 -q --show-progress "\$APPMANAGER_URL" -O /tmp/AppManager.AppImage 2>&1; then
        ...
        if [ -s /tmp/AppManager.AppImage ]; then
            # Instalar en /opt/AppImages
...
[Desktop Entry]
Name=AppManager
Comment=Gestor de AppImages (instalar, actualizar, eliminar)
Comment[es]=Gestor de AppImages (instalar, actualizar, eliminar)
Exec=/opt/AppImages/AppManager.AppImage %U
Icon=application-vnd.appimage
Terminal=false
Type=Application
Categories=System;PackageManager;
Keywords=appimage;apps;manager;install;update;
StartupNotify=true
MimeType=application/vnd.appimage;
APPMANAGER_DESKTOP
            
            # Crear symlink para acceso CLI
...
MIME_XML
            ...
            # Instalar libfuse2 para AppImages que lo necesiten (legacy)
            ...
            echo "  Doble-click en .AppImage → se abre con AppManager"
        else
            echo "  ⚠ Descarga vacía o corrupta"
            rm -f /tmp/AppManager.AppImage
        fi
    else
