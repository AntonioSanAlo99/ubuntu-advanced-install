...
step " Gestión de software instalada"
...
if apt-cache show systemd-oomd &>/dev/null; then
    ...
    step " systemd-oomd instalado y habilitado"
else
    warn " systemd-oomd no disponible en esta versión"
fi
...
step " Elementary icon theme instalado"
...
step " Temas GTK instalados (Adwaita, Adwaita-dark, HighContrast)"
...
step " GDM habilitado como display manager"
...
DESKTOP
    ...
    step " AppManager instalado"
else
    warn " No se pudo descargar AppManager (no crítico)"
fi
    ...
    if wget --timeout=30 --tries=3 --quiet --show-progress "\$THUMBNAILER_LATEST" -O /tmp/appimage-thumbnailer.deb 2>&1; then
        ...
        if [ -f /tmp/appimage-thumbnailer.deb ] && [ -s /tmp/appimage-thumbnailer.deb ]; then
            if file /tmp/appimage-thumbnailer.deb | grep -q "Debian binary package"; then
                ...
                step " AppImage thumbnailer instalado (.deb)"
            else
                warn " Archivo descargado no es un .deb válido"
                rm /tmp/appimage-thumbnailer.deb
            fi
        else
            warn " Descarga vacía o corrupta"
            rm -f /tmp/appimage-thumbnailer.deb
        fi
    else
        warn " Fallo en descarga del thumbnailer"
    fi
else
    warn " No se pudo obtener URL del thumbnailer desde GitHub API"
...
if [ -n "$USERNAME" ]; then
    ...
    if [ "$FIX_WORKSPACES" = "s" ] || [ "$FIX_WORKSPACES" = "S" ]; then
        ...
        step " Workspaces fijos: 1 área de trabajo"
    else
        sudo -u $USERNAME dbus-launch gsettings set org.gnome.mutter dynamic-workspaces true
        step " Workspaces dinámicos (comportamiento predeterminado de GNOME)"
    fi
    ...
    if [ "$DISABLE_USAGE" = "s" ] || [ "$DISABLE_USAGE" = "S" ]; then
        ...
        step " Tiempo de pantalla desactivado"
        ...
        step " Tiempo de pantalla habilitado (comportamiento predeterminado)"
    fi
...
EOF
    ...
    echo ""
    step " Configuración guardada en ~/.config/gnome-custom-config.txt"
    
else
    warn " Usuario no encontrado, configuraciones de GNOME omitidas"
fi
