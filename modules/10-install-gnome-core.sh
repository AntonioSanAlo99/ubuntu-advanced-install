...
# REQUIERE: TARGET, USERNAME, GNOME_DOCK, GDM_AUTOLOGIN, IS_LAPTOP, PERF_OOMD_AGGRESSIVE, UBUNTU_VERSION
...
# ── rtkit: prioridad realtime para PipeWire ─────────────────────────────────
# rtkit (RealtimeKit) permite a PipeWire obtener scheduling FIFO realtime
# para el thread de audio sin necesidad de permisos root. Sin rtkit,
# PipeWire funciona con SCHED_OTHER y latencia más alta (posibles xruns).
# El daemon se activa automáticamente via D-Bus cuando PipeWire lo solicita.
systemctl enable rtkit-daemon 2>/dev/null || true
echo "✓  rtkit habilitado (realtime scheduling para PipeWire)"

# pipewire-pulse: reemplazo drop-in de PulseAudio. En debootstrap puede
# no instalarse por defecto. wireplumber es el session manager de PipeWire.
echo "✓  Sesión: pipewire-pulse + wireplumber"
...
# ── Caffeine (extensions.gnome.org → system-wide) ────────────────────────────
...
CAFF_INSTALLED=false

GNOME_MAJOR_CAFF=\$(gnome-shell --version 2>/dev/null | grep -oP '[0-9]+' | head -1 || echo "")

if [ -n "\$GNOME_MAJOR_CAFF" ]; then
    CAFF_INFO=\$(curl --max-time 15 -s \
        "https://extensions.gnome.org/extension-info/?uuid=\${CAFF_UUID}&shell_version=\${GNOME_MAJOR_CAFF}" \
        2>/dev/null || echo "")
    CAFF_DL=\$(echo "\$CAFF_INFO" | grep -oP '"download_url"\s*:\s*"\K[^"]+' | head -1)

    if [ -n "\$CAFF_DL" ]; then
        wget --timeout=30 -q "https://extensions.gnome.org\${CAFF_DL}" -O /tmp/caffeine.zip 2>/dev/null
        if [ -s /tmp/caffeine.zip ]; then
            mkdir -p "\$CAFF_DIR"
            unzip -qo /tmp/caffeine.zip -d "\$CAFF_DIR"
            rm -f /tmp/caffeine.zip
            CAFF_INSTALLED=true
            echo "  ✓ Descargado desde extensions.gnome.org (GNOME \$GNOME_MAJOR_CAFF)"
        fi
    fi
fi

if [ "\$CAFF_INSTALLED" = "false" ]; then
...
# ── Wallpaper custom (incluido en el proyecto) ───────────────────────────────
WALLPAPER_SRC="$(dirname "$0")/../files/wallpaper.jpg"
WALLPAPER_DEST="/usr/share/backgrounds/ubuntu-advanced-install.jpg"
if [ -f "$WALLPAPER_SRC" ]; then
    cp "$WALLPAPER_SRC" "${TARGET}${WALLPAPER_DEST}"
    chmod 644 "${TARGET}${WALLPAPER_DEST}"
    echo "✓  Wallpaper custom instalado"
else
    echo "⚠  Wallpaper custom no encontrado en files/ — se usará el de Ubuntu"
fi
...
# ── GNOME Keyring ────────────────────────────────────────────────────────────
# gnome-keyring usa un formato binario interno no documentado. No se puede
# crear login.keyring "a mano" — solo gnome-keyring-daemon genera ficheros
# válidos. En chroot no hay D-Bus, pero dbus-run-session crea uno temporal.
#
# CON LUKS: GDM puede desbloquear el keyring automáticamente usando la misma
# contraseña LUKS (como Fedora). PAM pasa la contraseña del login al keyring.
# El keyring se crea con la contraseña del usuario → cifrado real.
#
# SIN LUKS + AUTOLOGIN: PAM no pasa contraseña → se crea keyring con
# contraseña vacía via dbus-run-session. Chrome/apps lo usan sin popup.
# Warning "gkr-pam: couldn't unlock" en journalctl es cosmético.
#
# Ref: https://wiki.archlinux.org/title/GNOME/Keyring
apt-get install -y gnome-keyring libpam-gnome-keyring dbus-user-session 2>/dev/null || true

# PAM: registrar gnome-keyring para desbloqueo en login con contraseña
DEBIAN_FRONTEND=noninteractive pam-auth-update --enable gnome-keyring 2>/dev/null || true

LUKS_ACTIVE="$USE_LUKS"
...
if [ -n "\$USERNAME" ]; then
    ...
    # Borrar keyrings previos para empezar limpio
    rm -f "\$KEYRING_DIR"/*.keyring "\$KEYRING_DIR"/default "\$KEYRING_DIR"/default-keyring 2>/dev/null

    if [ "\$LUKS_ACTIVE" = "true" ]; then
        # CON LUKS: no crear keyring — PAM lo crea con la contraseña del usuario
        # en el primer login real. GDM pasa la contraseña a pam_gnome_keyring.so
        # y el keyring se desbloquea automáticamente (modelo Fedora).
        echo "✓  GNOME Keyring: LUKS activo — PAM desbloqueará con contraseña del login"
    else
        # SIN LUKS: crear keyring con contraseña vacía via gnome-keyring-daemon
        # dbus-run-session provee un bus D-Bus temporal dentro del chroot
        sudo -u "\$USERNAME" dbus-run-session -- sh -c \
            'echo -n "" | gnome-keyring-daemon --unlock' 2>/dev/null || true

        # Marcar "login" como keyring por defecto (fichero de texto plano)
        echo "login" > "\$KEYRING_DIR/default-keyring"
        chown "\$USERNAME:\$USERNAME" "\$KEYRING_DIR/default-keyring"

        if [ -f "\$KEYRING_DIR/login.keyring" ]; then
            echo "✓  GNOME Keyring: login.keyring creado sin contraseña"
        else
            echo "⚠  GNOME Keyring: login.keyring no se pudo crear (se creará en primer login)"
        fi
    fi
fi
fi

# Skel vacío para futuros usuarios (keyring se crea en su primer login)
mkdir -p /etc/skel/.local/share/keyrings
chmod 700 /etc/skel/.local/share/keyrings
...
if [ -f "$OVERRIDE_SRC" ]; then
    ...
    if [ "${GNOME_DOCK:-ubuntu-dock}" = "dash-to-panel" ]; then
        ...
        # Desde "# ── Dock (ubuntu-dock" hasta la línea antes de la siguiente sección "[".
        # sed puro, sin dependencia de python3.
        awk '
            /^# ── Dock \(ubuntu-dock/ { skip=1; next }
            skip && /^\[/ { skip=0 }
            !skip
...
STLEOF
    ...
    arch-chroot "$TARGET" glib-compile-schemas /usr/share/glib-2.0/schemas/ 2>/dev/null || {
        echo "⚠  glib-compile-schemas falló — reintentando sin --strict"
        arch-chroot "$TARGET" glib-compile-schemas /usr/share/glib-2.0/schemas/ --allow-any 2>/dev/null || true
    }
...
arch-chroot "$TARGET" /bin/bash << 'DCONF_SYSTEM' || echo "⚠  dconf system-db: errores no críticos (se ignoran)"
...
# ── Mutter — workspaces estáticos (1 workspace) ──────────────────────────────
[org/gnome/mutter]
DCONF_EOF

# ── experimental-features: solo GNOME < 50 ──────────────────────────────────
# GNOME 50 activa VRR y xwayland-native-scaling por defecto.
# En versiones anteriores hay que forzarlos via experimental-features.
GNOME_MAJOR=$(echo "$GNOME_VER" | cut -d. -f1)
if [ "${GNOME_MAJOR:-0}" -lt 50 ]; then
    cat >> /etc/dconf/db/local.d/00-ubuntu-advanced-install << 'EXPEOF'

[org/gnome/mutter]
experimental-features=['xwayland-native-scaling']
EXPEOF
    echo "  experimental-features: xwayland-native-scaling (GNOME $GNOME_VER < 50)"
else
    echo "  experimental-features: no necesario (GNOME $GNOME_VER >= 50)"
fi

cat >> /etc/dconf/db/local.d/00-ubuntu-advanced-install << 'DCONF_EOF2'
...
overview-mode=0
# Desactivar módulos que conflictan con Dash to Panel / Ubuntu Dock
module-dash-enabled=false
module-panel-enabled=false
module-layout-enabled=false
# Overview Background — wallpaper visible con brightness ajustado
overview-bg-visible=true
overview-bg-blur-sigma=0
overview-bg-brightness=75
app-grid-bg-blur-sigma=0
app-grid-bg-brightness=75
...
dconf update 2>/dev/null || echo "⚠  dconf update falló (no crítico)"

echo "✓  dconf system-db configurado (welcome-dialog, extensiones, privacidad)"
DCONF_SYSTEM
