...
# ── GNOME Keyring — integración PAM completa ────────────────────────────────
# gnome-keyring y libpam-gnome-keyring ya se instalaron en el bloque de paquetes.
#
# Para que el keyring se desbloquee automáticamente al login, PAM debe pasar
# la contraseña del usuario a gnome-keyring-daemon. Esto requiere:
#   1. pam_gnome_keyring.so en auth (captura la contraseña)
#   2. pam_gnome_keyring.so auto_start en session (arranca el daemon)
#
# pam-auth-update configura common-auth y common-session, pero NO configura
# gdm-password ni gdm-autologin. Los ficheros de GDM se configuran manualmente.
# ============================================================================

# 1. pam-auth-update — configura common-auth + common-session
...
# 2. common-auth — auth optional pam_gnome_keyring.so
if ! grep -q 'pam_gnome_keyring' /etc/pam.d/common-auth 2>/dev/null; then
    ...
    echo "  ✓  PAM common-auth: gnome-keyring añadido"
fi

# 3. common-session — session optional pam_gnome_keyring.so auto_start
if ! grep -q 'pam_gnome_keyring' /etc/pam.d/common-session 2>/dev/null; then
    ...
    echo "  ✓  PAM common-session: gnome-keyring añadido"
fi

# 4. gdm-password — para login con contraseña
if [ -f /etc/pam.d/gdm-password ]; then
    if ! grep -q 'pam_gnome_keyring' /etc/pam.d/gdm-password; then
        ...
        echo "  ✓  PAM gdm-password: gnome-keyring añadido"
    fi
fi

# 5. gdm-autologin — para autologin sin contraseña
...
# Solo crear el directorio. gnome-keyring-daemon crea login.keyring y default
# automáticamente en el primer login. La contraseña del keyring coincidirá con
# la del usuario si PAM está bien configurado (puntos 1-5 arriba).
if [ -n "\$USERNAME" ]; then
...
# Safety net: autostart que desbloquea el keyring con contraseña vacía.
# gnome-keyring-daemon --unlock con stdin vacío = keyring sin contraseña.
GDM_AUTOLOGIN_ENABLED="$GDM_AUTOLOGIN"

if [ "\$GDM_AUTOLOGIN_ENABLED" = "true" ]; then
    # Borrar keyring existente para forzar recreación con contraseña vacía
    if [ -n "\$USERNAME" ] && [ -d "/home/\$USERNAME" ]; then
        rm -f "/home/\$USERNAME/.local/share/keyrings/login.keyring" 2>/dev/null
    fi

    # Autostart safety net
...
UNLOCKEOF
    ...
    echo "✓  GNOME Keyring: configurado para autologin (PAM + autostart safety net)"
else
...
if [ "${INSTALL_GRADIA:-false}" = "true" ]; then
    arch-chroot "$TARGET" /bin/bash << 'GRADIA_DCONF' || echo "⚠  Gradia dconf: errores no críticos"
