...
if [ "\$GNOME_DOCK_CHOICE" = "dash-to-panel" ]; then
    ...
    # Protocolo estándar de GNOME: descargar zip por UUID desde extensions.gnome.org
    if wget -q --timeout=30 \
        "https://extensions.gnome.org/extension-download/\${EXT_UUID}.shell-extension.zip" \
        -O "\$EXT_ZIP" 2>/dev/null && [ -s "\$EXT_ZIP" ]; then
