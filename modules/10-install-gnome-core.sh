...
# ── GNOME Keyring — desbloqueo automático sin encriptación ───────────────────
# Configura PAM para que pase la contraseña del login a gnome-keyring-daemon.
# En el primer login, gnome-keyring-daemon creará el keyring "login" con la
# contraseña del usuario. Con autologin (sin contraseña), el keyring se crea
# vacío (sin encriptación) — no pedirá popup de "Unlock default keyring".
#
# El fichero "default" apunta a "login" como keyring por defecto.
# Borrar login.keyring existente fuerza la recreación limpia.
#
# Ref: https://wiki.archlinux.org/title/GNOME/Keyring
apt-get install -y gnome-keyring libpam-gnome-keyring 2>/dev/null || true

# PAM: pam-auth-update registra gnome-keyring en common-auth y common-session
...
if [ -n "\$USERNAME" ] && [ -d "/home/\$USERNAME" ]; then
    ...
    # Borrar keyring previo para forzar recreación limpia en primer login
...
echo "✓  GNOME Keyring: PAM configurado (keyring se creará en primer login)"
