...
# paquetes. pam-auth-update integra las líneas en common-auth,
# common-session y common-password automáticamente.
#
# ADEMÁS: para autologin con GDM, hay que parchear gdm-autologin
# manualmente porque pam-auth-update NO lo toca y sin ello el keyring
# queda bloqueado (GDM no pasa contraseña a PAM).
# ============================================================================
...
# ── PAM gdm-autologin — parchear para gnome-keyring ─────────────────────────
# El stack gdm-autologin no incluye pam_gnome_keyring.so por defecto.
# Añadimos auth optional + session optional para que el daemon arranque
# y pueda recibir el unlock desde el autostart.
GDM_AUTOLOGIN_PAM="/etc/pam.d/gdm-autologin"
if [ -f "\$GDM_AUTOLOGIN_PAM" ]; then
    if ! grep -q "pam_gnome_keyring.so" "\$GDM_AUTOLOGIN_PAM"; then
        # auth: al final del bloque auth
        sed -i '/^auth.*pam_succeed_if.*uid/a auth    optional        pam_gnome_keyring.so' "\$GDM_AUTOLOGIN_PAM" 2>/dev/null || true
        # session: al final
        echo "session optional        pam_gnome_keyring.so auto_start" >> "\$GDM_AUTOLOGIN_PAM"
        echo "✓  PAM: gdm-autologin parcheado con pam_gnome_keyring.so"
    else
        echo "✓  PAM: gdm-autologin ya tiene pam_gnome_keyring.so"
    fi
fi

# ── Keyring "login" — preparar directorio ────────────────────────────────────
# Crear solo el directorio y el fichero "default" que indica qué keyring
# es el principal. NO pre-crear login.keyring binario — el formato egg es
# frágil y un fichero corrupto causa "failed to allocate" y crash del daemon.
...
if [ -n "\$USERNAME" ]; then
    ...
    _prepare_keyring_dir "\$KEYRING_DIR"
    chown -R "\$USERNAME:\$USERNAME" "/home/\$USERNAME/.local"
fi
_prepare_keyring_dir "/etc/skel/.local/share/keyrings"

# ── Autologin: desbloqueo automático del keyring ─────────────────────────────
# Con autologin, GDM no pasa contraseña a PAM → el keyring no se desbloquea.
# Solución: init-keyring.desktop que inicializa el keyring con contraseña
# vacía en el primer arranque y luego se autodestruye.
# En arranques posteriores, el PAM parcheado + unlock-keyring.desktop
# mantienen el keyring desbloqueado.
GDM_AUTOLOGIN_ENABLED="$GDM_AUTOLOGIN"

if [ "\$GDM_AUTOLOGIN_ENABLED" = "true" ]; then
    mkdir -p /etc/skel/.config/autostart

    # unlock-keyring.desktop — se ejecuta en cada login
    cat > /etc/skel/.config/autostart/unlock-keyring.desktop << 'UNLOCKEOF'
[Desktop Entry]
Type=Application
Name=Unlock Keyring
Exec=/bin/sh -c 'echo -n "" | gnome-keyring-daemon --unlock 2>/dev/null'
Hidden=false
NoDisplay=true
X-GNOME-Autostart-enabled=true
UNLOCKEOF

    # init-keyring.desktop — solo primer arranque, luego se autodestruye
    cat > /etc/skel/.config/autostart/init-keyring.desktop << 'INITEOF'
[Desktop Entry]
Type=Application
Name=Initialize Keyring
Comment=Crea el keyring login con contraseña vacía (solo primer arranque)
Exec=/bin/sh -c 'if [ ! -f "$HOME/.local/share/keyrings/login.keyring" ]; then echo -n "" | gnome-keyring-daemon --replace --unlock --components=secrets 2>/dev/null; sleep 2; fi; rm -f "$HOME/.config/autostart/init-keyring.desktop"'
Hidden=false
NoDisplay=true
X-GNOME-Autostart-enabled=true
INITEOF

    # Copiar también al usuario principal si ya existe
    if [ -n "\$USERNAME" ] && [ -d "/home/\$USERNAME" ]; then
        mkdir -p "/home/\$USERNAME/.config/autostart"
        cp /etc/skel/.config/autostart/unlock-keyring.desktop \
           "/home/\$USERNAME/.config/autostart/"
        cp /etc/skel/.config/autostart/init-keyring.desktop \
           "/home/\$USERNAME/.config/autostart/"
        chown -R "\$USERNAME:\$USERNAME" "/home/\$USERNAME/.config"
    fi
    echo "✓  GNOME Keyring: desbloqueo automático configurado (autologin)"
    echo "   → init-keyring.desktop (primer boot) + unlock-keyring.desktop (cada login)"
else
...
# Dash to Panel no necesita disable-overview-on-startup: V-Shell gestiona
# el startup-state para evitar el overview al iniciar sesión.
