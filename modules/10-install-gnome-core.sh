...
#
# Comportamiento según modo de login:
#   - Sin autologin: PAM pasa la contraseña al daemon, que crea login.keyring
#     automáticamente en el primer login con esa contraseña. Funciona perfecto.
#   - Con autologin: PAM no pasa contraseña. gnome-keyring-daemon arranca
#     pero el keyring queda bloqueado hasta que el usuario lo desbloquea
#     manualmente o se configura desbloqueo automático (ver abajo).
...
# Solución oficial: usar gnome-keyring-daemon --unlock con stdin vacío.
# Esto crea/desbloquea un keyring con contraseña vacía en el primer arranque.
# Ref: ArchWiki GNOME/Keyring#Using_the_keyring_outside_GNOME
GDM_AUTOLOGIN_ENABLED="$GDM_AUTOLOGIN"

if [ "\$GDM_AUTOLOGIN_ENABLED" = "true" ]; then
    # Autostart que desbloquea el keyring con contraseña vacía al iniciar sesión
