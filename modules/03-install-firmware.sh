...
if [ ${#FIRMWARE_NEEDED[@]} -eq 0 ]; then
    step " No se requiere firmware adicional"
    echo ""
    exit 0
fi
...
if [ -f /etc/apt/sources.list.d/ubuntu.sources ]; then
    if ! grep -q "multiverse" /etc/apt/sources.list.d/ubuntu.sources; then
        warn " Habilitando multiverse para firmware..."
        ...
        step " Multiverse habilitado"
    fi
else
    warn " ubuntu.sources no encontrado, verificando sources.list legacy..."
...
echo ""
echo "════════════════════════════════════════════════════════════════"
step " FIRMWARE INSTALADO CORRECTAMENTE"
echo "════════════════════════════════════════════════════════════════"
echo ""
