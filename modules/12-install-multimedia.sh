...
step " Thumbnailers instalados (audio requiere Totem hasta que GNOME libere paquete dedicado)"
...
case "\$UBUNTU_CODENAME" in
    ...
    *)
        ...
        warn " Fooyin no disponible para \$UBUNTU_CODENAME"
...
if [ "\$FOOYIN_SUPPORTED" = "true" ]; then
    ...
    if [ -z "\$FOOYIN_VERSION" ]; then
        warn " No se pudo obtener versión desde GitHub, usando v0.9.2"
    ...
    apt-get install -y \$APT_FLAGS \$QT6_DEPS || {
        warn " Algunas dependencias Qt6 fallaron, intentando alternativas..."
    ...
    if [ -n "\$LIBAV_PACKAGES" ]; then
        apt-get install -y \$APT_FLAGS \
            ...
            libpipewire-0.3-0 \
            || true
        step " Dependencias multimedia instaladas"
    else
        warn " No se detectaron paquetes libav, instalando solo otros..."
        apt-get install -y \$APT_FLAGS libtag1v5 libpipewire-0.3-0 || true
    fi
    ...
    if wget --timeout=30 --tries=3 -q --show-progress "\$FOOYIN_URL" -O fooyin.deb 2>&1; then
        step " Descarga completada"
        
        # Instalar con dpkg
        echo "Instalando Fooyin..."
        if dpkg -i fooyin.deb 2>&1; then
            step " Fooyin instalado correctamente"
            FOOYIN_INSTALLED=true
        else
            warn " dpkg reportó errores, intentando reparar..."
            apt-get --fix-broken install -y
            
            # Verificar si se instaló después de la reparación
            if dpkg -l | grep -q "^ii.*fooyin"; then
                step " Fooyin instalado después de reparar dependencias"
                FOOYIN_INSTALLED=true
            else
...
if ! command -v fooyin &>/dev/null; then
    error " fooyin no encontrado en PATH"
    exit 1
fi

# Intentar ejecutar fooyin con --version
if fooyin --version &>/dev/null; then
    step " Fooyin funciona correctamente"
    fooyin --version
    exit 0
else
    warn " Fooyin instalado pero falla al ejecutar"
    echo ""
