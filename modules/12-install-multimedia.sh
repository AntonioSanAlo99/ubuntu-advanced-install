...
# ============================================================================
# CINE — reproductor de vídeo GTK4 + libadwaita + mpv
# ============================================================================
# Cine es un reproductor de vídeo minimalista para GNOME escrito en Python,
# con GTK4 + libadwaita como interfaz y mpv como backend de reproducción.
# No está en los repos de Ubuntu — se instala desde fuente con meson.
#
# Dependencias de sistema: libmpv-dev, python3-mpv, python3-gi (PyGObject),
# python3-gi-cairo, gir1.2-gtk-4.0, gir1.2-adw-1.
# Dependencias de build: meson, ninja-build, gettext, desktop-file-utils,
# appstream, sassc, gobject-introspection (para fallback de libadwaita).
#
# Ubuntu 24.04 tiene libadwaita 1.5 — Cine requiere >= 1.6.
# Se usa --force-fallback-for=libadwaita igual que en ProtonPlus.
# ============================================================================

if [ "${INSTALL_CINE:-false}" = "true" ]; then
    echo ""
    echo "Instalando Cine (reproductor de vídeo)..."

    arch-chroot "$TARGET" /bin/bash << 'CINEEOF'
set -e
export DEBIAN_FRONTEND=noninteractive

# ── Dependencias de runtime ───────────────────────────────────────────────────
# python3-mpv: binding ctypes a libmpv — disponible en repos Ubuntu 24.04
# python3-gi: PyGObject (GTK4, Adwaita, etc.)
# libmpv2: runtime de mpv (dependencia de python3-mpv, se instala sola)
...
# ── Clonar último tag estable ─────────────────────────────────────────────────
BUILD_DIR="$(mktemp -d /tmp/cine-build.XXXXXX)"
cd "$BUILD_DIR"

git clone --filter=blob:none https://github.com/diegopvlk/Cine.git
cd Cine
...
MESON_EXTRA=""
if [ "$(echo "$ADW_VERSION" | cut -d. -f1)" -lt 1 ] || \
   { [ "$(echo "$ADW_VERSION" | cut -d. -f1)" -eq 1 ] && [ "$ADW_MINOR" -lt 6 ]; }; then
    echo "  libadwaita < 1.6 — compilando como subproyecto meson"
    MESON_EXTRA="--force-fallback-for=libadwaita"
fi

# ── Compilar e instalar ───────────────────────────────────────────────────────
# Cine es Python puro — meson no compila nada, solo instala los .py como
# datos del sistema y genera el script lanzador en /usr/local/bin/cine.
...
# ── ldconfig si libadwaita se compiló como subproyecto ───────────────────────
if [ -n "$MESON_EXTRA" ]; then
    echo "/usr/local/lib/x86_64-linux-gnu" >> /etc/ld.so.conf.d/cine-local.conf 2>/dev/null || \
    echo "/usr/local/lib/aarch64-linux-gnu" >> /etc/ld.so.conf.d/cine-local.conf 2>/dev/null || true
    ldconfig
fi
...
echo "✓  Cine instalado en /usr/local/bin/cine"
CINEEOF
...
if arch-chroot "$TARGET" test -x /usr/local/bin/cine 2>/dev/null; then
    echo "  ✓ Cine instalado (reproductor GTK4 + mpv)"
elif [ "${INSTALL_CINE:-false}" = "true" ]; then
    echo "  ⚠ Cine: instalación fallida (ver log)"
fi
echo "  ✓ VLC instalado"
echo ""
