...
if [ "\$FOOYIN_SUPPORTED" = "true" ]; then
    ...
    QT6_DEPS="
        ...
        qgnomeplatform-qt6
   "
    # qgnomeplatform-qt6: integra Qt6 con el tema de iconos y GTK de GNOME.
    # Sin él, Fooyin (y otras apps Qt6) no heredan el icon-theme del sistema
    # y muestran la interfaz sin iconos. Instala la plataforma Qt6-GNOME
    # y configura QT_QPA_PLATFORMTHEME=gnome en /etc/environment.
...
# ── QT_QPA_PLATFORMTHEME — integración Qt6 con GNOME ─────────────────────────
# Sin esta variable, las apps Qt6 (Fooyin, etc.) no heredan el icon-theme
# del sistema y muestran la interfaz sin iconos aunque qgnomeplatform-qt6
# esté instalado. Se escribe en /etc/environment para aplicar globalmente
# (Wayland, X11, sesiones con y sin autologin).
#
# Valor 'gnome' → usa qgnomeplatform-qt6, que lee el theme de GSettings.
# Valor alternativo 'gtk3' funciona igual en sistemas con GTK3/Qt5.
# Se usa 'gnome' porque es el valor estándar de Ubuntu desde 22.04.
if grep -q 'QT_QPA_PLATFORMTHEME' /etc/environment 2>/dev/null; then
    sed -i 's/^QT_QPA_PLATFORMTHEME=.*/QT_QPA_PLATFORMTHEME=gnome/' /etc/environment
else
    echo 'QT_QPA_PLATFORMTHEME=gnome' >> /etc/environment
fi
echo "✓  QT_QPA_PLATFORMTHEME=gnome configurado (iconos en apps Qt6)"
