...
# Añadir repositorio de Spotify con formato moderno (signed-by)
# Nota: El repositorio de Spotify tiene un problema conocido de validación
# Usamos [signed-by=...] para especificar la clave explícitamente
...
# Nota: Si falla la validación (error conocido de Spotify), permitir instalación igualmente
apt-get update -qq 2>&1 | grep -v "spotify" || true

# Instalar Spotify
# Si hay problemas de validación, usar --allow-unauthenticated (seguro en este caso específico)
if ! apt-get install -y spotify-client 2>/dev/null; then
    echo "  ⚠ Problema de validación del repositorio (error conocido de Spotify)"
    echo "  Instalando con --allow-unauthenticated..."
    apt-get install -y --allow-unauthenticated spotify-client
fi
