...
if [ -f /usr/share/applications/org.gnome.Totem.desktop ]; then
    ...
    echo "NoDisplay=true" >> /usr/share/applications/org.gnome.Totem.desktop
...
if [ "$INSTALL_SPOTIFY" = "s" ] || [ "$INSTALL_SPOTIFY" = "S" ]; then
    ...
    chroot "$TARGET" /bin/bash << 'SPOTIFY_EOF'
...
# Forzar X11 backend — evita problemas con Wayland en Electron/CEF
...
if [ -f "\$SPOTIFY_DESKTOP" ]; then
    ...
    echo "✓  Spotify: --ozone-platform=x11 aplicado"
fi
...
if chroot "$TARGET" dpkg -l 2>/dev/null | grep -q "^ii.*fooyin"; then
...
if chroot "$TARGET" dpkg -l 2>/dev/null | grep -q "^ii.*spotify-client"; then
    echo "  ✓ Spotify instalado"
else
    echo "  ⚠ Spotify no instalado (se omitió)"
fi
