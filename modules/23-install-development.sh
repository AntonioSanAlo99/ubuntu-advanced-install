...
# ============================================================================
# NEOVIM + LAZYVIM (configuración estilo Omarchy)
# ============================================================================
# LazyVim: distribución de plugins para Neovim lista para usar.
# Ref: https://github.com/LazyVim/LazyVim
# Ref: https://github.com/basecamp/omarchy (instala LazyVim como config por defecto)

echo ""
echo "Instalando Neovim..."
apt install -y neovim ripgrep fd-find lazygit

if [ -n "\$USERNAME" ] && id "\$USERNAME" &>/dev/null; then
    NVIM_DIR="/home/\$USERNAME/.config/nvim"
    if [ ! -d "\$NVIM_DIR" ]; then
        echo "Configurando LazyVim para \$USERNAME..."
        su - "\$USERNAME" -c "git clone https://github.com/LazyVim/starter ~/.config/nvim && rm -rf ~/.config/nvim/.git"
        echo "  ✓ LazyVim configurado (plugins se instalarán en primer arranque de nvim)"
    else
        echo "  ⊘ ~/.config/nvim ya existe — se respeta configuración existente"
    fi
fi
echo "✓  Neovim + LazyVim instalado"
...
GHOSTTY_DEB_URL=\$(curl --max-time 15 -s https://api.github.com/repos/mkasberg/ghostty-ubuntu/releases/latest \
    | grep "browser_download_url.*amd64\.deb\"" | cut -d '"' -f 4 | head -1)

if [ -n "\$GHOSTTY_DEB_URL" ] \
   && wget --timeout=30 --tries=2 -q "\$GHOSTTY_DEB_URL" -O /tmp/ghostty.deb; then
    dpkg -i /tmp/ghostty.deb 2>/dev/null || true
    ...
    if command -v ghostty >/dev/null 2>&1; then
        ...
        echo "  ⚠ Ghostty: instalación falló"
    fi
else
    echo "  ⚠ Ghostty: descarga falló — https://github.com/mkasberg/ghostty-ubuntu/releases"
fi
