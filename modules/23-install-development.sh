...
if [ -z "$INSTALL_NETTOOLS" ]; then
    read -p "¿Herramientas de red y cloud (Wireshark, nmap, AWS CLI, httpie, jq)? (s/n) [n]: " INSTALL_NETTOOLS
...
if [[ "\$INSTALL_NETTOOLS_VAR" =~ ^[SsYy]$ ]]; then
    echo ""
...
if [[ "${INSTALL_NETTOOLS:-n}" =~ ^[SsYy]$ ]]; then
