...
# REQUIERE: TARGET, USERNAME, INSTALL_VSCODE, NODEJS_OPTION, INSTALL_TOPGRADE, INSTALL_BOXES, INSTALL_LAZY_TOOLS, INSTALL_BUN, INSTALL_DENO, INSTALL_DOCKER_TOOLS, INSTALL_MELD, INSTALL_POSTMAN, INSTALL_NETTOOLS
...
[ -z "${INSTALL_BUN:-}" ] && INSTALL_BUN=$(_yaml_val "development" "bun" "false")
[ -z "${INSTALL_DENO:-}" ] && INSTALL_DENO=$(_yaml_val "development" "deno" "false")
...
INSTALL_NETTOOLS="${INSTALL_NETTOOLS:-n}"
...
# ============================================================================
# DOCKER ENGINE (docker-ce + containerd + compose v2 + buildx)
# ============================================================================
# Se instala si INSTALL_DOCKER_TOOLS o INSTALL_LAZY_TOOLS están activos,
# ya que lazydocker y docker-pussh necesitan el daemon.
NEED_DOCKER="false"
INSTALL_DOCKER_TOOLS_CHECK="$INSTALL_DOCKER_TOOLS"
INSTALL_LAZY_CHECK="$INSTALL_LAZY_TOOLS"
if [ "\$INSTALL_DOCKER_TOOLS_CHECK" = "true" ] || [ "\$INSTALL_LAZY_CHECK" = "true" ]; then
    NEED_DOCKER="true"
fi

if [ "\$NEED_DOCKER" = "true" ]; then
    echo ""
    echo "Instalando Docker Engine (docker-ce)..."
    ...
    USERNAME_DOCKER="$USERNAME"
    if [ -n "\$USERNAME_DOCKER" ]; then
        usermod -aG docker "\$USERNAME_DOCKER" 2>/dev/null || true
    fi
    ...
    echo "✓  Docker Engine instalado (docker-ce + compose v2 + buildx)"
else
    echo "⊘ Docker Engine no instalado (no requerido)"
fi
...
if [ "\$INSTALL_NETTOOLS_VAR" = "true" ]; then
    ...
    # No está en repos apt. Se descarga el zip oficial y se instala en /usr/local.
...
if [ "\$INSTALL_LAZY_VAR" = "true" ]; then
    ...
    if command -v nvim >/dev/null 2>&1; then
        ...
        # ── nvim.desktop — acceso desde App Grid ─────────────────────────────
        cat > /usr/share/applications/nvim.desktop << 'NVIMEOF'
[Desktop Entry]
Name=Neovim
Comment=Neovim — editor de texto modal (LazyVim IDE)
Exec=ghostty -e nvim %F
Icon=nvim
Terminal=false
Type=Application
Categories=Development;TextEditor;
MimeType=text/plain;text/x-csrc;text/x-python;application/json;
Keywords=Text;Editor;vim;nvim;neovim;
NVIMEOF
        chmod 644 /usr/share/applications/nvim.desktop
        echo "  ✓ nvim.desktop creado (abre en Ghostty)"
...
if [ "${INSTALL_DENO:-false}" = "true" ]; then
    echo "  ✓ Deno runtime"
fi
