...
# Método 1: PPA de mkasberg (auto-updates via apt)
if add-apt-repository -y ppa:mkasberg/ghostty 2>/dev/null; then
    apt-get update -qq || true
    if apt-get install -y ghostty; then
        GHOSTTY_OK=true
        echo "  ✓ Ghostty instalado vía PPA mkasberg"
    else
        add-apt-repository -ry ppa:mkasberg/ghostty 2>/dev/null || true
    fi
fi

# Método 2: fallback a .deb de GitHub releases
if [ "\$GHOSTTY_OK" = "false" ]; then
