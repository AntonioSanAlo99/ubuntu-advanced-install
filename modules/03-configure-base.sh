...
# REQUIERE: TARGET, HOSTNAME, USERNAME, USER_PASSWORD, ROOT_PASSWORD
# PRODUCE:  Locale, timezone, teclado, usuario
...
# ── Repositorios ──────────────────────────────────────────────────────────────
# Los repos ya están configurados por módulo 02 (debootstrap).
# Solo refrescamos el índice de paquetes para instalar actualizaciones.
apt-get update
...
# Teclado para Wayland (localectl) — método moderno vía systemd
# localectl escribe en /etc/vconsole.conf (TTY) y configura XKB para
# compositors Wayland vía logind. No necesita /etc/X11/xorg.conf.d.
localectl set-x11-keymap es pc105 2>/dev/null || true

echo "✓  Teclado: Español (es) — TTY y Wayland (localectl)"
...
# ── Sesión systemd — piezas que debootstrap NO instala ───────────────────────
# En una instalación estándar de Ubuntu estos paquetes vienen preinstalados,
# pero debootstrap instala un sistema mínimo sin ellos. Sin estas piezas:
#   - No hay session-*.scope al login (GDM no crea sesión systemd)
#   - systemd --user no arranca (no hay app-*.scope para apps GNOME)
#   - systemd-oomd no puede matar apps individuales
#   - user-.slice con MemoryMax no se aplica
#   - Portales XDG (screenshot, file chooser) pueden fallar
#
# libpam-systemd: provee pam_systemd.so que crea la sesión systemd en PAM.
#   Cuando el usuario hace login, pam_systemd registra la sesión con logind,
#   crea el session-*.scope, arranca systemd --user y monta /run/user/UID.
# systemd: paquete completo (debootstrap solo trae systemd-sysv mínimo).
# dbus-user-session: bus D-Bus del usuario para portales y systemd --user.
apt-get install -y \
    systemd \
    libpam-systemd \
    dbus-user-session
echo "✓  Sesión systemd: libpam-systemd + dbus-user-session (scopes, logind, portales)"
...
# ── bash-completion ──────────────────────────────────────────────────────────
# Autocompletado de comandos, rutas y argumentos con Tab.
# Ubuntu lo incluye como dependencia pero no siempre se instala en debootstrap.
apt-get install -y bash-completion

# Configurar para el usuario y futuros usuarios (via skel)
# complete -cf sudo: permite autocompletar después de sudo
cat >> /etc/skel/.bashrc << 'BASHRC_EXTRA'

# ── bash-completion (ubuntu-advanced-install) ──
...
echo "✓  bash-completion instalado + aliases configurados"

# ── systemd-logind: gestión correcta de sesiones y scopes ────────────────────
# KillUserProcesses=yes: al cerrar sesión, systemd mata todos los app-*.scope
# del usuario. Sin esto, procesos huérfanos pueden quedar en RAM.
# UserTasksMax=infinity: no limitar el número de tareas por usuario (el límite
# lo gestiona el user-.slice con MemoryMax en módulo 12).
mkdir -p /etc/systemd/logind.conf.d
cat > /etc/systemd/logind.conf.d/50-session-scope.conf << 'LOGINDEOF'
[Login]
KillUserProcesses=yes
UserTasksMax=infinity
LOGINDEOF
echo "✓  logind: KillUserProcesses=yes (limpieza de scopes al logout)"
