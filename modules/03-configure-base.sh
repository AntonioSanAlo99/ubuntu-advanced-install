...
if [ -n "${USER_PASSWORD_HASH:-}" ] && [ -n "${ROOT_PASSWORD_HASH:-}" ]; then
    # ── Hash SHA-512 (método preferido, como Subiquity autoinstall) ──────────
    arch-chroot "$TARGET" /bin/bash << USEREOF
useradd -m -G sudo,adm,audio,video,plugdev -s /bin/bash "$USERNAME"
USEREOF
    printf '%s:%s\n' "$USERNAME" "$USER_PASSWORD_HASH" | arch-chroot "$TARGET" chpasswd -e
    printf '%s:%s\n' "root"     "$ROOT_PASSWORD_HASH"  | arch-chroot "$TARGET" chpasswd -e
    echo -e "${C_OK}✓${C_RESET}  Usuario: $USERNAME (credenciales SHA-512)"
elif [ -n "${USER_PASSWORD:-}" ] && [ -n "${ROOT_PASSWORD:-}" ]; then
    # ── Plaintext fallback (si openssl no estaba disponible al configurar) ───
    arch-chroot "$TARGET" /bin/bash << USEREOF
useradd -m -G sudo,adm,audio,video,plugdev -s /bin/bash "$USERNAME"
USEREOF
