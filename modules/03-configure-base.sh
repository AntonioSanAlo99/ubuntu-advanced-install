...
#      chroot lo hereda → apt no genera warnings "Cannot set LC_*"
...
chroot "$TARGET" /bin/bash << 'LOCALE_FIRST'
...
chroot "$TARGET" /bin/bash << 'BASE_CONFIG'
export DEBIAN_FRONTEND=noninteractive
...
if [ -n "${USER_PASSWORD:-}" ] && [ -n "${ROOT_PASSWORD:-}" ]; then
    # Crear usuario (sin contraseña todavía)
    chroot "$TARGET" /bin/bash << USEREOF
useradd -m -G sudo,adm,audio,video,plugdev -s /bin/bash "$USERNAME"
USEREOF
    # Establecer contraseñas via stdin — nunca via variables de entorno
    # para evitar exposición en `ps aux` y /proc/*/environ
    printf '%s:%s\n' "$USERNAME" "$USER_PASSWORD" | chroot "$TARGET" chpasswd
