...
# ============================================================================
# LOCALE: GENERAR PRIMERO, antes de cualquier apt
# ============================================================================
echo "Generando locales..."

# Activar es_ES.UTF-8 en /etc/locale.gen (debootstrap ya creó el archivo)
sed -i 's/^# *es_ES.UTF-8/es_ES.UTF-8/' /etc/locale.gen
grep -q "es_ES.UTF-8" /etc/locale.gen || echo "es_ES.UTF-8 UTF-8" >> /etc/locale.gen

# Generar locale AHORA (antes de apt)
locale-gen

# Configurar como predeterminado
echo 'LANG=es_ES.UTF-8' > /etc/default/locale

# Configurar consola virtual (vconsole.conf - estilo Arch)
cat > /etc/vconsole.conf << 'VCONSOLE_EOF'
KEYMAP=es
FONT=eurlatgr
VCONSOLE_EOF

echo "✓ Locale generado: es_ES.UTF-8"
echo "✓ Consola virtual configurada: teclado español"
