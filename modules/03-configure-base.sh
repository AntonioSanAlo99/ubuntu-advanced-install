...
# ============================================================================
# LOCALE-GEN: paso obligatorio antes de cualquier apt install
# debootstrap no genera locales. Perl (usado por dpkg internamente)
# falla en TODOS los apt install siguientes si no se ejecuta locale-gen
# AQUÍ, en el primer chroot, antes de instalar nada más.
# ============================================================================
echo "Generando locales..."

# Instalar el paquete locales si no está (puede no estar en debootstrap mínimo)
if ! dpkg -l locales 2>/dev/null | grep -q "^ii"; then
    apt install -y locales
fi

# Activar es_ES.UTF-8 en /etc/locale.gen
...
# Establecer como locale del sistema (para aplicaciones)
update-locale LANG=es_ES.UTF-8 LC_ALL=es_ES.UTF-8 LANGUAGE=es_ES

# ============================================================================
# CONFIGURACIÓN ADICIONAL PARA MENSAJES DE SISTEMA EN ESPAÑOL
# ============================================================================

# 1. /etc/default/locale - Para aplicaciones en general
cat > /etc/default/locale << 'LOC_EOF'
LANG=es_ES.UTF-8
LANGUAGE=es_ES:es
LC_ALL=es_ES.UTF-8
...
# 2. /etc/locale.conf - Para systemd
cat > /etc/locale.conf << 'LOC_CONF_EOF'
LANG=es_ES.UTF-8
LC_MESSAGES=es_ES.UTF-8
LOC_CONF_EOF

# 3. Configurar en /etc/environment (leído por PAM y servicios del sistema)
...
# 4. Configurar systemd para usar español
localectl set-locale LANG=es_ES.UTF-8 LC_MESSAGES=es_ES.UTF-8

echo "✓ Locales generados correctamente"
echo "✓ Mensajes de sistema configurados en español"
locale -a | grep -i es_ES || true
...
cat > /etc/systemd/system/getty@.service.d/locale.conf << 'GETTY_EOF'
[Service]
Environment="LANG=es_ES.UTF-8"
Environment="LC_MESSAGES=es_ES.UTF-8"
GETTY_EOF

echo "✓ TTY configuradas para mostrar mensajes en español"
echo "✓ Páginas de manual en español instaladas"
...
echo "✓ Teclado configurado: Español de España"
