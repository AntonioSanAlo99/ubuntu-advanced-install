...
#
# Se usa partición swap (no swapfile) porque:
#   - Compatible con hibernación (suspend-to-disk)
#   - Rendimiento ligeramente superior
#   - Consistente con instaladores de referencia
...
echo "  (0 = sin partición swap; se puede crear swapfile manualmente después)"
echo ""
read -p "  Tamaño de swap en GiB [${SWAP_GIB}]: " user_swap
...
if [ "$install_mode" = "1" ]; then
    ...
    if [ "$FIRMWARE" = "UEFI" ]; then
        ...
        if [ "$SWAP_GIB" -gt 0 ]; then
            SWAP_END=$(( START_MIB + SWAP_GIB * 1024 ))
            parted -s "$TARGET_DISK" mkpart "swap" linux-swap "${START_MIB}MiB" "${SWAP_END}MiB"
            parted -s "$TARGET_DISK" set $NEXT_PART swap on 2>/dev/null || true
            SWAP_PART=$(part_name "$TARGET_DISK" $NEXT_PART)
            NEXT_PART=$(( NEXT_PART + 1 ))
            START_MIB=$SWAP_END
        fi
        ...
        if [ "$SWAP_GIB" -gt 0 ]; then
            SWAP_END=$(( START_MIB + SWAP_GIB * 1024 ))
            parted -s "$TARGET_DISK" mkpart primary linux-swap "${START_MIB}MiB" "${SWAP_END}MiB"
            SWAP_PART=$(part_name "$TARGET_DISK" $NEXT_PART)
            NEXT_PART=$(( NEXT_PART + 1 ))
            START_MIB=$SWAP_END
        fi
    ...
    if [ -n "$SWAP_PART" ]; then
        ...
        echo "  ✓  Swap: $SWAP_PART → linux-swap (${SWAP_GIB} GiB)"
    fi
...
# ============================================================================
    ...
    TOTAL_NEEDED=$(( 25 + SWAP_GIB + 1 ))   # mínimo root + swap + margen

    echo "  Espacio libre al final del disco : ${FREE_GIB} GiB"
    echo "  Mínimo necesario para Ubuntu     : ${TOTAL_NEEDED} GiB  (25 sistema + ${SWAP_GIB} swap + 1 margen)"
    echo ""
    ...
    TOTAL_NEEDED_FINAL=$(( ubuntu_gib + SWAP_GIB + 1 ))
    ...
    [ "$SWAP_GIB" -gt 0 ]   && echo "  ║  Swap : $(part_name $TARGET_DISK $NEXT_NUM)  (nueva, ${SWAP_GIB} GiB)"
    ROOT_NUM=$(( NEXT_NUM + ( SWAP_GIB > 0 ? 1 : 0 ) ))
    ...
    if [ "$SWAP_GIB" -gt 0 ]; then
        if [ "$PTTYPE" = "gpt" ]; then
            parted -s "$TARGET_DISK" mkpart "swap" linux-swap -- "-$(( ubuntu_gib * 1024 + SWAP_GIB * 1024 ))MiB" "-${ubuntu_gib}GiB"
        else
            parted -s "$TARGET_DISK" mkpart primary linux-swap -- "-$(( ubuntu_gib * 1024 + SWAP_GIB * 1024 ))MiB" "-${ubuntu_gib}GiB"
        fi
        SWAP_PART=$(part_name "$TARGET_DISK" $NEXT_NUM)
        NEXT_NUM=$(( NEXT_NUM + 1 ))
    fi
