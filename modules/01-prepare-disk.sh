...
if [ "$install_mode" = "1" ]; then
    ...
    # Root: LUKS + ext4 o ext4 directo
    if [ "${USE_LUKS:-false}" = "true" ] && [ -n "${LUKS_PASSWORD:-}" ]; then
        echo "  Configurando LUKS en $ROOT_PART..."
        echo -n "$LUKS_PASSWORD" | cryptsetup luksFormat --batch-mode \
            --type luks2 --cipher aes-xts-plain64 --key-size 512 \
            --hash sha256 --iter-time 2000 "$ROOT_PART" -
        echo -n "$LUKS_PASSWORD" | cryptsetup open "$ROOT_PART" crypt-root -
        LUKS_ENABLED="true"
        LUKS_UUID=$(blkid -s UUID -o value "$ROOT_PART")
        ROOT_DEVICE="/dev/mapper/crypt-root"
        mkfs.ext4 -F \
            -L "ubuntu-root" \
            -O "has_journal,extent,huge_file,flex_bg,metadata_csum,64bit,dir_nlink,extra_isize" \
            "$ROOT_DEVICE"
        echo "  ✓  Root: $ROOT_PART → LUKS2 → ext4 ($ROOT_DEVICE)"
    else
        LUKS_ENABLED="false"
        LUKS_UUID=""
...
# ============================================================================
    ...
    TOTAL_NEEDED=$(( 25 + 1 ))   # mínimo root + margen

    echo "  Espacio libre al final del disco : ${FREE_GIB} GiB"
    echo "  Mínimo necesario para Ubuntu     : ${TOTAL_NEEDED} GiB  (25 sistema + 1 margen)"
    echo ""
    ...
    # Sugerir tamaño: espacio libre menos 2 GiB de margen
    ...
    if [ "${USE_LUKS:-false}" = "true" ] && [ -n "${LUKS_PASSWORD:-}" ]; then
        ...
        mkfs.ext4 -F \
            ...
            -O "has_journal,extent,huge_file,flex_bg,metadata_csum,64bit,dir_nlink,extra_isize" \
            "$ROOT_DEVICE"
        echo "  ✓  Root: $ROOT_PART → LUKS2 → ext4 (${ubuntu_gib} GiB)"
    else
        LUKS_ENABLED="false"
        LUKS_UUID=""
        ROOT_DEVICE="$ROOT_PART"
...
ROOT_PART="$ROOT_PART"
ROOT_DEVICE="${ROOT_DEVICE:-$ROOT_PART}"
EFI_PART="$EFI_PART"
SWAP_PART="$SWAP_PART"
FIRMWARE="$FIRMWARE"
...
echo "  Root   : ${ROOT_DEVICE:-$ROOT_PART}"
[ "${LUKS_ENABLED:-false}" = "true" ] && echo "  LUKS   : $ROOT_PART (UUID: $LUKS_UUID)"
echo ""
