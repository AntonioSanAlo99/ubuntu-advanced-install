...
echo "[DEBUG] Verificando comandos necesarios..."
...
if ! command -v parted &> /dev/null; then
    ...
    if apt update && apt install -y parted; then
        ...
        echo "❌ Error instalando parted. Instálalo manualmente:"
...
if ! command -v lsblk &> /dev/null; then
    echo "❌ lsblk no encontrado. Instala util-linux:"
    echo "   sudo apt install -y util-linux"
    exit 1
fi

echo "✓ Comandos verificados"
echo ""
...
while IFS= read -r line; do
    ...
    if [ "$disk_type" = "disk" ]; then
        DISKS+=("$disk_name")
        echo "[DEBUG] Disco encontrado: $disk_name"
    fi
...
if [ ${#DISKS[@]} -eq 0 ]; then
    ...
    lsblk -a 2>/dev/null || echo "[DEBUG] lsblk -a falló"
    exit 1
fi
...
for disk in "${DISKS[@]}"; do
    ...
    while IFS= read -r part; do
        ...
        echo "[DEBUG] Analizando partición: $part"
        ...
        if [[ "$fstype" == "ntfs" ]]; then
            ...
            if [[ "$label" =~ ^(Windows|System|OS|WINRE).*$ ]]; then
                ...
                echo "[DEBUG] Windows detectado en $part"
            fi
            
            # Check blkid for windows
            if blkid "$part" 2>/dev/null | grep -qi windows; then
                DISK_HAS_WINDOWS[$disk]="yes"
                echo "[DEBUG] Windows detectado en $part (blkid)"
            fi
            ...
            if mkdir -p /tmp/check_win 2>/dev/null; then
                if mount -r "$part" /tmp/check_win 2>/dev/null; then
                    if [ -d /tmp/check_win/Windows ] || [ -d /tmp/check_win/windows ]; then
                        DISK_HAS_WINDOWS[$disk]="yes"
                        echo "[DEBUG] Windows detectado en $part (directorio)"
                    fi
        ...
        if [[ "$fstype" == "vfat" ]] && [[ "$label" =~ ^(EFI|SYSTEM).*$ ]]; then
            ...
            echo "[DEBUG] EFI detectado en $part"
        fi
        ...
        if [[ "$parttype" == "c12a7328-f81f-11d2-ba4b-00a0c93ec93b" ]]; then
            DISK_HAS_EFI[$disk]="yes"
            echo "[DEBUG] EFI detectado en $part (GPT)"
        fi
    done <<< "$PARTITIONS"
done
