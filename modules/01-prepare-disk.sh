...
source "$SCRIPT_DIR/../lib/debug-functions.sh" 2>/dev/null || {
    # Funciones básicas si no se puede cargar el archivo
    debug() { echo "[DEBUG] $*"; }
    step() { echo "✓ $*"; }
    error() { echo "✗ $*"; }
    separator() { echo "════════════════════════════════════════════════════════════════"; }
    module_header() { separator; echo "  $1"; separator; echo ""; }
}

# Variables se pasan desde install.sh via environment
# source "$(dirname "$0")/../config.env"

module_header "PREPARACIÓN DE DISCO - DETECCIÓN AUTOMÁTICA"

debug "Verificando comandos necesarios..."

# Verificar parted
if ! command -v parted &> /dev/null; then
    warn "parted no encontrado, instalando..."
    if apt update && apt install -y parted; then
        step "parted instalado"
    else
        error "Error instalando parted. Instálalo manualmente:"
        echo "   sudo apt update && sudo apt install -y parted"
        exit 1
    fi
    echo ""
fi
...
if ! command -v lsblk &> /dev/null; then
    error "lsblk no encontrado. Instala util-linux:"
    echo "   sudo apt install -y util-linux"
    exit 1
fi

step "Comandos verificados"
echo ""

# Detect firmware
debug "Detectando firmware..."
...
debug "Total discos encontrados: ${#DISKS[@]}"

if [ ${#DISKS[@]} -eq 0 ]; then
    error "No se detectaron discos"
    debug "Intentando listar todos los dispositivos de bloque:"
...
for disk in "${DISKS[@]}"; do
    ...
    debug "Verificando particiones en $disk..."
...
if [ "$mode" = "1" ] && [ "$HAS_WINDOWS" = "yes" ]; then
    ...
    # Calcular default inteligente basado en espacio libre
    if [ $FREE_GB -ge 100 ]; then
        ...
        DEFAULT_SIZE=$FREE_GB
    fi
    
    echo "Sugerencias según espacio libre (${FREE_GB}GB):"
    echo "  • Mínimo recomendado: 20GB"
    echo "  • Instalación normal: 30-50GB"
    echo "  • Con desarrollo/gaming: 100GB+"
    echo ""
    
    read -p "¿Cuánto espacio asignar a Ubuntu? (GB) [${DEFAULT_SIZE}]: " ubuntu_size
    ubuntu_size=${ubuntu_size:-$DEFAULT_SIZE}
    
    # Validar que no exceda el espacio libre
    if [ $ubuntu_size -gt $FREE_GB ]; then
        echo "⚠️  No hay suficiente espacio libre (disponible: ${FREE_GB}GB)"
        echo "    Usando todo el espacio disponible: ${FREE_GB}GB"
        ubuntu_size=$FREE_GB
    fi
    
    # Validar mínimo sensato
    if [ $ubuntu_size -lt 20 ]; then
        echo "⚠️  Espacio muy reducido. Mínimo recomendado: 20GB"
        read -p "    ¿Continuar con ${ubuntu_size}GB? (s/n): " confirm
        [[ ! $confirm =~ ^[SsYy]$ ]] && exit 1
    fi
    ...
    PARTED_OUTPUT=$(parted -s "$TARGET_DISK" print 2>/dev/null)
    LAST_PART=$(echo "$PARTED_OUTPUT" | grep "^ " | tail -1 | awk '{print $1}')
    ...
    if [ "$FIRMWARE" = "UEFI" ]; then
        if [ "$HAS_EFI" = "yes" ]; then
            ...
            LSBLK_OUTPUT=$(lsblk -n -p -o NAME,FSTYPE "$TARGET_DISK" 2>/dev/null)
            EFI_PART=$(echo "$LSBLK_OUTPUT" | grep vfat | head -1 | awk '{print $1}')
