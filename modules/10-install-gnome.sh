...
# Instalar tema de iconos elementary
apt install -y \$APT_FLAGS elementary-icon-theme
echo "✓ Tema de iconos elementary instalado"
...
echo "════════════════════════════════════════════════════════════════"
echo ""
echo "Configuración de transparencias (opacidad 15%) para:"
echo "  • App Grid, Panel, Calendario, Quick Settings"
echo "  • Workspaces, Dock, GDM"
echo ""

read -p "¿Aplicar transparencias? (s/n) [s]: " APPLY_TRANSPARENCY
if [[ ${APPLY_TRANSPARENCY:-s} =~ ^[SsYy]$ ]]; then
    echo ""
    echo "Configurando transparencias..."
    bash "$(dirname "$0")/10c-gnome-transparency.sh"
else
    echo "✓ Transparencias omitidas (GNOME estándar)"
fi
