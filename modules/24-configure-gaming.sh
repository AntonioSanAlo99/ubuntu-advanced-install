...
# ============================================================================
# STEAM — tarball oficial de Valve (repo.steampowered.com)
# ============================================================================
# Referencia: GLFS (linuxfromscratch.org/glfs/view/stable/steam/steam.html)
#
# El tarball contiene:
#   - bin_steam.sh          — launcher que descomprime el bootstrap y arranca Steam
#   - bootstraplinux_ubuntu12_32.tar.xz — runtime 32-bit (se auto-extrae en 1er boot)
#   - steamdeps             — script Python que chequea deps via python-apt (inútil aquí)
#   - Makefile              — instala todo en /usr/bin, /usr/lib/steam, /usr/share/...
#   - icons/, .desktop      — integración con el escritorio
#
# steamdeps se anula con ln -sf /usr/bin/true (igual que GLFS y Arch).
# make install coloca bin_steam.sh como /usr/bin/steam y el bootstrap en /usr/lib/steam/.

echo ""
echo "Instalando Steam (tarball oficial Valve)..."

# make es necesario para el Makefile del tarball.
# En un sistema debootstrap sin build-essential, make no está presente.
if ! command -v make >/dev/null 2>&1; then
    echo "  Instalando make (requerido por el Makefile de Steam)..."
    apt-get install -y make || { echo "⚠  No se pudo instalar make — omitiendo Steam"; }
fi
...
if command -v make >/dev/null 2>&1 \
   && wget --timeout=30 --tries=2 -q "\$STEAM_URL" -O "\$STEAM_TAR" 2>/dev/null \
   && [ -s "\$STEAM_TAR" ]; then

    # Extraer y obtener el directorio raíz del tarball.
    # Según la versión/URL, puede ser 'steam-launcher/' o 'steam/'.
    # tar -tf lee la primera entrada sin extraer — más limpio que find post-extracción.
    STEAM_TOPDIR=\$(tar tf "\$STEAM_TAR" 2>/dev/null | head -1 | cut -d/ -f1)

    mkdir -p /tmp/steam-build
    tar xf "\$STEAM_TAR" -C /tmp/steam-build

    if [ -n "\$STEAM_TOPDIR" ] && [ -f "/tmp/steam-build/\$STEAM_TOPDIR/Makefile" ]; then
        cd "/tmp/steam-build/\$STEAM_TOPDIR"

        # steamdeps usa python-apt (paquete Debian/Ubuntu) para resolver dependencias.
        # En un sistema que gestiona deps manualmente, se reemplaza por /usr/bin/true
        # para que `make install` no falle al invocarlo.
        ln -sf /usr/bin/true steamdeps

        if make install; then
            echo "✓  Steam instalado desde tarball \${STEAM_VER}"
        else
            echo "⚠  Steam: make install falló (exit \$?)"
        fi
        cd /
    else
        echo "⚠  Steam: estructura inesperada en el tarball (Makefile no encontrado en '\$STEAM_TOPDIR')"
    fi

    rm -rf /tmp/steam-build "\$STEAM_TAR"
else
    echo "⚠  Steam: no se pudo descargar el tarball (o make no disponible)"
fi

# ── Dependencias de runtime de Steam ──────────────────────────────────────
# Fuentes:
#   - GLFS required:     alsa-plugins, curl, dbus, libglvnd, ca-certificates
#   - GLFS recommended:  vulkan, pulseaudio, zenity, lsof, xdg-user-dirs, libgpg-error
#   - Arch PKGBUILD:     depends + depends_x86_64, mapeados a nombres de paquete Ubuntu
#
# Se instalan siempre (incluso si el tarball falló) porque el usuario puede
# instalar Steam manualmente después y necesitará estas libs.
safe_apt \
    ...
    zenity lsof lsb-release usbutils xdg-user-dirs \
...
# Se usa `install -d -o` en vez de sudo/mkdir porque sudo no está en debootstrap.
if [ -n "\$USERNAME" ]; then
    ...
    chown -h "\$USERNAME:\$USERNAME" "\$STEAM_HOME/.steam/steam" "\$STEAM_HOME/.steam/root" 2>/dev/null || true
fi
...
meson setup build --prefix=/usr/local --buildtype=plain $MESON_EXTRA_OPTS
