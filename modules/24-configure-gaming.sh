...
# ============================================================================
# STEAM — .deb oficial de Valve + dependencias completas
# ============================================================================
...
if wget --timeout=30 --tries=2 -q "\$STEAM_URL" -O "\$STEAM_DEB" 2>/dev/null \
   && [ -s "\$STEAM_DEB" ]; then
    ...
    # Pre-aceptar licencia de Steam
    echo "steam steam/question select I AGREE" | debconf-set-selections
    echo "steam steam/license note"            | debconf-set-selections

    # --install-recommends: el sistema tiene Install-Recommends=false global,
    # pero Steam necesita sus Recommends o muestra "Package Install" al abrir.
    dpkg -i "\$STEAM_DEB" 2>/dev/null || true
    apt-get install -f -y --install-recommends 2>/dev/null || true
    rm -f "\$STEAM_DEB"

    # Dependencias de runtime (lista explícita de steamdeps)
