...
# ============================================================================
# STEAM — .deb oficial de Valve + bootstrap headless del cliente
# ============================================================================
# Paso 1: Instalar el .deb de Akamai (bootstrapper + dependencias i386)
# Paso 2: Ejecutar el bootstrap como $USERNAME dentro del chroot para que
#          descargue el cliente completo a ~/.local/share/Steam/
#          Así en el primer arranque real solo necesita login, no descarga.
#
# El bootstrap de Steam necesita red (wget/curl a client-download.steampowered.com)
# y acceso al tarball /usr/lib/steam/bootstraplinux_ubuntu12_32.tar.xz.
# No necesita GPU ni display — solo extrae archivos y descarga binarios.
# ============================================================================
...
if wget --timeout=30 --tries=2 -q "\$STEAM_URL" -O "\$STEAM_DEB" 2>/dev/null \
   && [ -s "\$STEAM_DEB" ]; then
    ...
    # ── Dependencias de runtime de Steam ──────────────────────────────────
    # Con Install-Recommends=false, apt no instala steam-libs ni las libs
    # GL/EGL/GBM i386 que Steam necesita para arrancar sin el diálogo
    # "Package Install" del primer inicio. Se instalan explícitamente.
    apt-get install -y \
        steam-libs-amd64 \
        steam-libs-i386 \
    ...
    echo "✓  Steam .deb + dependencias de runtime instalados"

    # ── Bootstrap headless: pre-descargar el cliente completo ──────────
    # El script /usr/bin/steam extrae bootstraplinux_ubuntu12_32.tar.xz
    # a ~/.local/share/Steam/ y luego el updater descarga el cliente
    # desde client-download.steampowered.com/client/steam_client_ubuntu12.
    #
    # Ejecutamos como $USERNAME con HOME apuntando a su home real.
    # timeout mata el proceso tras la descarga (el cliente intenta abrir
    # GUI después del update, que fallará sin display — eso está OK).
    # El exit code != 0 es esperado (no hay display), lo ignoramos.

    STEAM_HOME="/home/\$USERNAME"
    if [ -f /usr/lib/steam/bootstraplinux_ubuntu12_32.tar.xz ]; then
        echo "  Ejecutando bootstrap headless de Steam (descargando cliente)..."

        # Extraer bootstrap manualmente si /usr/bin/steam no coopera sin TTY
        sudo -u "\$USERNAME" mkdir -p "\$STEAM_HOME/.local/share/Steam"
        sudo -u "\$USERNAME" tar xf /usr/lib/steam/bootstraplinux_ubuntu12_32.tar.xz \
            -C "\$STEAM_HOME/.local/share/Steam/" 2>/dev/null || true

        # Crear symlinks que Steam espera
        sudo -u "\$USERNAME" mkdir -p "\$STEAM_HOME/.steam"
        ln -sfn "\$STEAM_HOME/.local/share/Steam" "\$STEAM_HOME/.steam/steam" 2>/dev/null || true
        ln -sfn "\$STEAM_HOME/.local/share/Steam" "\$STEAM_HOME/.steam/root"  2>/dev/null || true

        # Ejecutar el updater para descargar el cliente completo
        # steam.sh con -noreactlogin evita intentar abrir CEF/browser
        if [ -x "\$STEAM_HOME/.local/share/Steam/steam.sh" ]; then
            timeout 120 sudo -u "\$USERNAME" \
                env HOME="\$STEAM_HOME" \
                    STEAM_RUNTIME=1 \
                    DISPLAY="" \
                    DBUS_SESSION_BUS_ADDRESS="" \
                "\$STEAM_HOME/.local/share/Steam/steam.sh" -noreactlogin 2>/dev/null || true
            # Matar cualquier proceso steam residual
            pkill -u "\$USERNAME" -f steam 2>/dev/null || true
            sleep 2
            pkill -9 -u "\$USERNAME" -f steam 2>/dev/null || true
        fi

        # Verificar que el cliente se descargó
        if [ -d "\$STEAM_HOME/.local/share/Steam/ubuntu12_32" ] \
           && [ -f "\$STEAM_HOME/.local/share/Steam/steam.sh" ]; then
            echo "✓  Steam: cliente completo pre-descargado (solo necesita login en primer arranque)"
        else
            echo "⚠  Steam: bootstrap instalado pero descarga del cliente incompleta"
            echo "   El cliente se descargará automáticamente en el primer arranque"
        fi
        ...
        echo "⚠  Steam: bootstrap tarball no encontrado, el cliente se descargará en primer arranque"
    fi
else
...
# ── Actualizar caché de iconos (meson no lo ejecuta en chroot) ────────────────
