...
install_nvidia() {
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NVIDIA WAYLAND — configuración profesional completa
    # ══════════════════════════════════════════════════════════════════════════
    # Ref: ArchWiki NVIDIA, Hyprland Wiki NVIDIA, CachyOS-Settings
    # Estos ajustes garantizan la mejor experiencia Wayland con NVIDIA:
    #   1. DRM KMS (modeset + fbdev) — obligatorio para Wayland
    #   2. Early-load en initramfs — evita flicker y acelera arranque
    #   3. Suspend/resume services — evita pantalla negra al despertar
    #   4. EGL Wayland libraries — compatibilidad EGL + Wayland protocol
    #   5. GBM backend — GNOME/Mutter usa GBM por defecto (no EGLStreams)
    #   6. VA-API via NVIDIA — aceleración de vídeo por hardware
    #   7. GSK renderer fix — evita glitches GTK4 con Vulkan
    #   8. GDM Wayland override — asegurar que GDM ofrece sesión Wayland
    # ══════════════════════════════════════════════════════════════════════════

    # ── 1. DRM KMS: modeset + fbdev (modprobe.d) ─────────────────────────────
    # modeset=1: habilita DRM kernel mode setting (obligatorio Wayland)
    # fbdev=1: framebuffer nativo NVIDIA (mejor que efifb/simpledrm)
    # NVreg_PreserveVideoMemoryAllocations=1: preserva VRAM en suspend/hibernate
    # NVreg_UsePageAttributeTable=1: mejor rendimiento de memoria NVIDIA
...
NVIDIAMOD
    ...
    # ── 2. Early-load en initramfs ────────────────────────────────────────────
    # Cargar nvidia,nvidia_modeset,nvidia_uvm,nvidia_drm ANTES del display manager.
    # Evita: flicker al arrancar, fallback a nouveau, GDM sin Wayland.
    ...
    echo "  ✓ initramfs: nvidia modules early-load"

    # ── 3. Suspend/resume/hibernate services ──────────────────────────────────
    # Sin estos, la GPU pierde estado al suspender → pantalla negra al despertar.
    # Trabajan con NVreg_PreserveVideoMemoryAllocations=1 (punto 1).
    ...
    echo "  ✓ nvidia-suspend/resume/hibernate services habilitados"

    # ── 4. EGL Wayland libraries ──────────────────────────────────────────────
    # Compatibilidad entre EGL API y Wayland protocol.
    # libnvidia-egl-wayland1: EGL external platform para Wayland
    # libnvidia-egl-gbm1: EGL external platform para GBM
    safe_apt libnvidia-egl-wayland1 libnvidia-egl-gbm1
    echo "  ✓ EGL Wayland + GBM libraries instaladas"

    # ── 5. Variables de entorno para Wayland + NVIDIA ─────────────────────────
    # GBM_BACKEND=nvidia-drm: fuerza GBM (no EGLStreams) — estándar desde 2021
    # __GLX_VENDOR_LIBRARY_NAME=nvidia: asegura GLX via NVIDIA
    # LIBVA_DRIVER_NAME=nvidia: VA-API via nvidia-vaapi-driver
    # GSK_RENDERER=ngl: GTK4 usa OpenGL NGL en vez de Vulkan (evita glitches)
    # MOZ_ENABLE_WAYLAND=1: Firefox en Wayland nativo
    # ELECTRON_OZONE_PLATFORM_HINT=auto: Electron apps usan Wayland si disponible
    for envvar in \
        ...
        "ELECTRON_OZONE_PLATFORM_HINT=auto"; do
        envkey="\${envvar%%=*}"
        grep -q "^\$envkey=" /etc/environment 2>/dev/null || echo "\$envvar" >> /etc/environment
    done
    echo "  ✓ /etc/environment: GBM, GLX, VA-API, GSK, MOZ, Electron"

    # ── 6. VA-API via NVIDIA ──────────────────────────────────────────────────
    # nvidia-vaapi-driver traduce VA-API → NVDEC/NVENC
    safe_apt nvidia-vaapi-driver libva2 libva-drm2 libva-x11-2 libva-wayland2 vainfo
    echo "  ✓ VA-API: nvidia-vaapi-driver instalado"

    # ── 7. GDM: asegurar sesión Wayland con NVIDIA ────────────────────────────
    # GDM 42+ habilita Wayland con NVIDIA por defecto, pero la regla udev
    # 61-gdm.rules puede bloquearlo en algunos casos. La anulamos.
    # Ref: ArchWiki GDM#Wayland_and_the_proprietary_NVIDIA_driver
    if [ -f /usr/lib/udev/rules.d/61-gdm.rules ]; then
        ln -sf /dev/null /etc/udev/rules.d/61-gdm.rules
        echo "  ✓ GDM: regla 61-gdm.rules anulada (forzar Wayland)"
    fi

    # Asegurar WaylandEnable=true en GDM
...
# ============================================================================
# Descarga kernel mainline estable de kernel.org, aplica parches CachyOS desde
# github.com/CachyOS/kernel-patches, y configura scheduler con sed sobre .config.
#
# Schedulers disponibles:
#   1) BORE             — EEVDF + Burst-Oriented Response Enhancer (firelzrd)
#                         Mejora la respuesta interactiva bajo carga priorizando
#                         ráfagas de CPU cortas. Ideal gaming + escritorio.
#   2) BORE + sched-ext — BORE + framework para schedulers BPF en userspace.
#                         Permite cambiar scheduler en caliente (scx_bpfland
#                         modo gaming, scx_rusty, etc.) sin recompilar kernel.
#   3) EEVDF            — Earliest Eligible Virtual Deadline First (stock tuneado).
#                         Reemplazó a CFS en kernel 6.6+. Equilibrio fairness/
#                         rendimiento. Buenos allround sin parches adicionales.
#   4) CFS compat       — En kernels ≥6.6, EEVDF es el scheduler "stock".
#                         Esta opción desactiva BORE y sched-ext, y cambia a
#                         PREEMPT_VOLUNTARY para máxima estabilidad/compatibilidad.
#                         Útil para servidores o hardware problemático.
#
# Parches CachyOS aplicados (de kernel-patches):
#   0002-bbr3.patch     — Google BBRv3 congestion control
#   0003-cachy.patch    — CONFIG_CACHY, HZ tweaks, interactivity
#   0004-fixes.patch    — Fixes recogidos del mailing list
#   sched/0001-bore*.patch — Parche BORE (solo opciones 1 y 2)
#   + otros parches menores (amd-isp4, hdmi, etc.)
#
# Se compila como .deb con make deb-pkg.
# ============================================================================

if [ "${INSTALL_CACHYOS_KERNEL:-false}" = "true" ]; then
    echo ""
    SCHED_OPT="${CACHYOS_SCHEDULER:-1}"
    case "$SCHED_OPT" in
        1) SCHED_LABEL="BORE" ;;
        2) SCHED_LABEL="BORE + sched-ext" ;;
        3) SCHED_LABEL="EEVDF" ;;
        4) SCHED_LABEL="CFS compat" ;;
        *) SCHED_LABEL="BORE"; SCHED_OPT="1" ;;
    esac
    echo "Compilando kernel CachyOS — scheduler: $SCHED_LABEL"
...
apt-get install -y \
    ...
    libelf-dev libssl-dev dwarves bison lz4 zstd wget curl \
    debhelper rsync python3
...
# ── Obtener última versión estable del kernel desde kernel.org ───────────────
# API JSON (método preferido) → fallback a parseo HTML
KVER=\$(curl -s https://www.kernel.org/releases.json 2>/dev/null \
    | grep -oP '"version"\s*:\s*"\K[0-9]+\.[0-9]+\.[0-9]+' | head -1)

if [ -z "\$KVER" ]; then
    KVER=\$(curl -s https://www.kernel.org/ 2>/dev/null \
        | grep -oP 'linux-\K[0-9]+\.[0-9]+\.[0-9]+(?=\.tar)' | head -1)
fi

if [ -z "\$KVER" ]; then
    echo "  ⚠ No se pudo obtener versión del kernel — abortando"
    cd /; rm -rf "\$BUILD_DIR"
    exit 0
fi
...
wget -q "https://cdn.kernel.org/pub/linux/kernel/v\${KMAJOR}.x/linux-\${KVER}.tar.xz" \
    -O "linux-\${KVER}.tar.xz" 2>/dev/null \
    || wget -q "https://mirrors.edge.kernel.org/pub/linux/kernel/v\${KMAJOR}.x/linux-\${KVER}.tar.xz" \
        -O "linux-\${KVER}.tar.xz" 2>/dev/null

if [ ! -s "linux-\${KVER}.tar.xz" ]; then
    echo "  ⚠ Descarga fallida — abortando"
    cd /; rm -rf "\$BUILD_DIR"
    exit 0
fi
...
# ── Clonar parches CachyOS ──────────────────────────────────────────────────
# Estructura del repo: kernel-patches/<major>.<minor>/ con parches sueltos
# y subdirectorios sched/ (BORE), sched-dev/, all/, misc/
echo "  Clonando parches CachyOS..."
git clone --depth 1 https://github.com/CachyOS/kernel-patches.git /tmp/cachyos-patches 2>/dev/null || true

# Buscar directorio de parches para nuestra versión de kernel
PATCH_DIR=""
for trydir in "/tmp/cachyos-patches/\${KMAJOR}.\${KMINOR}" "/tmp/cachyos-patches/\${KMAJOR}"; do
    [ -d "\$trydir" ] && PATCH_DIR="\$trydir" && break
done

if [ -n "\$PATCH_DIR" ]; then
    echo "  Aplicando parches desde \$(basename \$PATCH_DIR)/..."
    APPLIED=0

    # ── Parches base (raíz del directorio) ──
    # Típicos: 0002-bbr3, 0003-cachy, 0004-fixes, 0005-hdmi, etc.
    for p in "\$PATCH_DIR"/00*.patch; do
        [ ! -f "\$p" ] && continue
        PNAME=\$(basename "\$p")
        if patch -Np1 --dry-run -i "\$p" > /dev/null 2>&1; then
            patch -Np1 -i "\$p" > /dev/null 2>&1
            echo "    ✓ \$PNAME"
            APPLIED=\$((APPLIED + 1))
        else
            echo "    ✗ \$PNAME (conflicto — omitido)"
        fi
    done

    # ── Parches de scheduler (sched/) — solo para BORE (opciones 1 y 2) ──
    if [ "\$SCHED_OPT" = "1" ] || [ "\$SCHED_OPT" = "2" ]; then
        if [ -d "\$PATCH_DIR/sched" ]; then
            for p in "\$PATCH_DIR"/sched/*.patch; do
                [ ! -f "\$p" ] && continue
                PNAME=\$(basename "\$p")
                if patch -Np1 --dry-run -i "\$p" > /dev/null 2>&1; then
                    patch -Np1 -i "\$p" > /dev/null 2>&1
                    echo "    ✓ \$PNAME (sched/BORE)"
                    APPLIED=\$((APPLIED + 1))
                else
                    echo "    ✗ \$PNAME (conflicto — omitido)"
                fi
    ...
    # ── Parches all/ (comunes a todas las variantes) ──
    if [ -d "\$PATCH_DIR/all" ]; then
        for p in "\$PATCH_DIR"/all/*.patch; do
            [ ! -f "\$p" ] && continue
            PNAME=\$(basename "\$p")
            if patch -Np1 --dry-run -i "\$p" > /dev/null 2>&1; then
                patch -Np1 -i "\$p" > /dev/null 2>&1
    ...
    echo "  ✓ \$APPLIED parches aplicados"
else
    echo "  ⚠ No se encontraron parches para kernel \${KMAJOR}.\${KMINOR} — usando kernel vanilla"
fi
rm -rf /tmp/cachyos-patches

# ── Configuración base ───────────────────────────────────────────────────────
# Partir de la config del kernel actual como base (si existe)
if [ -f /boot/config-\$(uname -r) ]; then
    cp /boot/config-\$(uname -r) .config
    echo "  ✓ Config base copiada de kernel actual"
else
    make defconfig
    echo "  ✓ Config base: defconfig"
fi

# ── Ajustes CachyOS comunes (todos los schedulers) ──────────────────────────
# HZ=1000:      tick rate alto → menor latencia de escritorio
# PREEMPT:      preemption completa → mejor interactividad (override en CFS)
# BBR:          Google BBRv3 → mejor throughput TCP
# CONFIG_CACHY: activa tweaks CachyOS en el patchset
# BPF_SYSCALL:  necesario para sched-ext y herramientas BPF

# HZ 250 → 1000
sed -i '/^CONFIG_HZ_250=y/          s/.*/# CONFIG_HZ_250 is not set/' .config
sed -i '/^# CONFIG_HZ_1000 is not set/s/.*/CONFIG_HZ_1000=y/' .config
sed -i '/^CONFIG_HZ=250/            s/.*/CONFIG_HZ=1000/' .config
# Si HZ no estaba definido a 250, forzar
grep -q '^CONFIG_HZ=' .config || echo 'CONFIG_HZ=1000' >> .config
grep -q '^CONFIG_HZ_1000=' .config || echo 'CONFIG_HZ_1000=y' >> .config

# Preemption completa (por defecto; opción 4 lo revierte)
sed -i '/^CONFIG_PREEMPT_VOLUNTARY=y/     s/.*/# CONFIG_PREEMPT_VOLUNTARY is not set/' .config
sed -i '/^# CONFIG_PREEMPT is not set/    s/.*/CONFIG_PREEMPT=y/' .config
sed -i '/^CONFIG_PREEMPT_NONE=y/          s/.*/# CONFIG_PREEMPT_NONE is not set/' .config

# BBR TCP
sed -i '/^# CONFIG_TCP_CONG_BBR is not set/s/.*/CONFIG_TCP_CONG_BBR=y/' .config
grep -q '^CONFIG_TCP_CONG_BBR=' .config || echo 'CONFIG_TCP_CONG_BBR=y' >> .config
sed -i 's/^CONFIG_DEFAULT_TCP_CONG=.*/CONFIG_DEFAULT_TCP_CONG="bbr"/' .config

# CONFIG_CACHY (si el parche lo introduce)
grep -q '^# CONFIG_CACHY is not set' .config && \
    sed -i 's/^# CONFIG_CACHY is not set/CONFIG_CACHY=y/' .config

# BPF syscall (base para sched-ext)
sed -i '/^# CONFIG_BPF_SYSCALL is not set/s/.*/CONFIG_BPF_SYSCALL=y/' .config
grep -q '^CONFIG_BPF_SYSCALL=' .config || echo 'CONFIG_BPF_SYSCALL=y' >> .config

echo "  ✓ Ajustes CachyOS comunes aplicados (HZ=1000, PREEMPT, BBR, BPF)"

# ── Scheduler específico (via sed sobre .config) ────────────────────────────
case "\$SCHED_OPT" in
    1)
        # ── BORE ──────────────────────────────────────────────────────────
        # Burst-Oriented Response Enhancer sobre EEVDF (firelzrd).
        # Prioriza ráfagas cortas de CPU → excelente respuesta interactiva.
        # Ideal para gaming y escritorio con cargas mixtas.
        # Sin sched-ext: menor overhead, más simple.
        sed -i '/^# CONFIG_SCHED_BORE is not set/s/.*/CONFIG_SCHED_BORE=y/' .config
        grep -q '^CONFIG_SCHED_BORE=' .config || echo 'CONFIG_SCHED_BORE=y' >> .config
        # Desactivar sched-ext
        sed -i '/^CONFIG_SCHED_CLASS_EXT=y/s/.*/# CONFIG_SCHED_CLASS_EXT is not set/' .config
        echo "  ✓ Scheduler: BORE"
        SCHED_SUFFIX="bore"
        ;;
    2)
        # ── BORE + sched-ext ──────────────────────────────────────────────
        # BORE como scheduler base + framework sched-ext habilitado.
        # sched-ext permite cargar schedulers BPF en userspace en caliente:
        #   scx_bpfland (gaming)  — prioriza tareas latency-sensitive
        #   scx_rusty   (general) — numa-aware, buena distribución de carga
        #   scx_lavd    (latency) — minimiza cola de espera
        # Requiere scx-scheds userspace (paquete separado).
        sed -i '/^# CONFIG_SCHED_BORE is not set/s/.*/CONFIG_SCHED_BORE=y/' .config
        grep -q '^CONFIG_SCHED_BORE=' .config || echo 'CONFIG_SCHED_BORE=y' >> .config
        sed -i '/^# CONFIG_SCHED_CLASS_EXT is not set/s/.*/CONFIG_SCHED_CLASS_EXT=y/' .config
        grep -q '^CONFIG_SCHED_CLASS_EXT=' .config || echo 'CONFIG_SCHED_CLASS_EXT=y' >> .config
        echo "  ✓ Scheduler: BORE + sched-ext"
        SCHED_SUFFIX="bore-scx"
        ;;
    3)
        # ── EEVDF tuneado ─────────────────────────────────────────────────
        # Earliest Eligible Virtual Deadline First (Peter Zijlstra, kernel 6.6+).
        # Reemplazó a CFS como scheduler por defecto en mainline.
        # Buen equilibrio fairness/rendimiento sin parches de scheduler.
        # Los parches CachyOS base (cachy.patch) ya optimizan interactividad.
        sed -i '/^CONFIG_SCHED_BORE=y/s/.*/# CONFIG_SCHED_BORE is not set/' .config
        sed -i '/^CONFIG_SCHED_CLASS_EXT=y/s/.*/# CONFIG_SCHED_CLASS_EXT is not set/' .config
        echo "  ✓ Scheduler: EEVDF (stock con parches CachyOS)"
        SCHED_SUFFIX="eevdf"
        ;;
    4)
        # ── CFS compat (máxima estabilidad) ───────────────────────────────
        # En kernels ≥6.6 el scheduler stock es EEVDF (evolución de CFS).
        # Esta opción desactiva todo lo extra (BORE, sched-ext) y revierte
        # preemption a VOLUNTARY → menor overhead, máxima compatibilidad.
        # Recomendado para servidores, VMs, o hardware legacy.
        sed -i '/^CONFIG_SCHED_BORE=y/s/.*/# CONFIG_SCHED_BORE is not set/' .config
        sed -i '/^CONFIG_SCHED_CLASS_EXT=y/s/.*/# CONFIG_SCHED_CLASS_EXT is not set/' .config
        # Revertir preemption a voluntary
        sed -i '/^CONFIG_PREEMPT=y/s/.*/# CONFIG_PREEMPT is not set/' .config
        sed -i '/^# CONFIG_PREEMPT_VOLUNTARY is not set/s/.*/CONFIG_PREEMPT_VOLUNTARY=y/' .config
        grep -q '^CONFIG_PREEMPT_VOLUNTARY=' .config || echo 'CONFIG_PREEMPT_VOLUNTARY=y' >> .config
        echo "  ✓ Scheduler: EEVDF stock (modo compatibilidad, PREEMPT_VOLUNTARY)"
        SCHED_SUFFIX="cfs"
        ;;
    *)
        echo "  ⚠ Opción inválida (\$SCHED_OPT) — usando BORE por defecto"
        sed -i '/^# CONFIG_SCHED_BORE is not set/s/.*/CONFIG_SCHED_BORE=y/' .config
        grep -q '^CONFIG_SCHED_BORE=' .config || echo 'CONFIG_SCHED_BORE=y' >> .config
        SCHED_SUFFIX="bore"
        ;;
...
# ── Compilar como .deb ───────────────────────────────────────────────────────
NPROC=\$(nproc)
echo "  Compilando con \$NPROC hilos (20-60 min)..."
make -j"\$NPROC" deb-pkg LOCALVERSION="-cachyos-\$SCHED_SUFFIX" 2>&1 | tail -5
...
if [ -n "\$INSTALLED" ]; then
    ...
    echo "    Selecciona el kernel cachyos-\$SCHED_SUFFIX en GRUB tras reiniciar"
else
    echo "  ⚠ Kernel CachyOS: instalación no confirmada — revisa /tmp/cachyos-kernel.*/deb"
fi
