...
install_nvidia() {
    ...
    # ── NVIDIA Power Management (suspend/resume/hibernate) ────────────────────
    # Sin estos servicios, la GPU NVIDIA pierde estado al suspender y causa
    # pantalla negra, artefactos o crash al despertar.
    # Ref: ArchWiki NVIDIA#Preserve_video_memory, CachyOS-Settings
    ...
    # nvidia_drm.modeset=1: necesario para Wayland con NVIDIA
    # nvidia_drm.fbdev=1: framebuffer nativo NVIDIA (mejor que efifb)
    # NVreg_PreserveVideoMemoryAllocations=1: preserva VRAM en suspend
    mkdir -p /etc/modprobe.d
    cat > /etc/modprobe.d/nvidia-power.conf << 'NVIDIAMOD'
# NVIDIA power management — ubuntu-advanced-install
options nvidia NVreg_PreserveVideoMemoryAllocations=1
options nvidia_drm modeset=1 fbdev=1
NVIDIAMOD
    echo "  ✓ NVIDIA modprobe: modeset=1, fbdev=1, PreserveVideoMemory"

    # ── NVIDIA early-load en initramfs ────────────────────────────────────────
    # Cargar módulos NVIDIA antes del display manager para evitar flicker
    # y asegurar que Wayland detecta la GPU desde el inicio.
    ...
    echo "  ✓ NVIDIA modules añadidos a initramfs (early-load)"
}
