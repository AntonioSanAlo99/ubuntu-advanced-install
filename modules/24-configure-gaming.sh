...
# ============================================================================
# STEAM — tarball oficial de Valve
# ============================================================================
# Fuente: GLFS (linuxfromscratch.org/glfs/view/stable/steam/steam.html)
# y PKGBUILD de Arch Linux. El tarball contiene el launcher (bin_steam.sh),
# el bootstrap (bootstraplinux_ubuntu12_32.tar.xz), iconos, .desktop y
# udev rules. steamdeps se anula porque usa python-apt.
...
if wget --timeout=30 --tries=2 -q "\$STEAM_URL" -O "\$STEAM_TAR" 2>/dev/null \
   && [ -s "\$STEAM_TAR" ]; then
    ...
    # steamdeps usa python-apt — anular con true (igual que GLFS/Arch)
    ...
    # Required (GLFS): alsa-plugins, curl, dbus, display server, ca-certs, libglvnd
    # Recommended (GLFS): vulkan, pulseaudio, xdg-desktop-portal, zenity, lsof, libgpg-error
    # + deps x86_64 del PKGBUILD de Arch mapeadas a Ubuntu
    safe_apt \
    ...
    echo "✓  Steam instalado (se actualizará en primer arranque)"
else
