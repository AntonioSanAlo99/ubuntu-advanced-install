...
# ============================================================================
# Fuente: PKGBUILD de Arch Linux (gitlab.archlinux.org/archlinux/packaging/packages/steam)
# El tarball contiene el launcher, udev rules, iconos, .desktop y el bootstrap.
# Se instala con make DESTDIR=/ — las dependencias se resuelven manualmente
# porque steamdeps usa python-apt que no necesitamos.
...
STEAM_URL="https://repo.steampowered.com/steam/pool/steam/s/steam/steam_\${STEAM_VER}.tar.gz"

if wget --timeout=30 --tries=2 -q "\$STEAM_URL" -O "\$STEAM_TAR" 2>/dev/null \
   && [ -s "\$STEAM_TAR" ]; then

    # Extraer e instalar con make
    mkdir -p /tmp/steam-build
    tar xf "\$STEAM_TAR" -C /tmp/steam-build
    cd /tmp/steam-build/steam-launcher

    # steamdeps usa python-apt — lo anulamos (igual que LFS/Arch)
    ln -sf /usr/bin/true steamdeps
    make install
    cd /

    # Udev rules: permisos de dispositivos de input para Steam Input / VR
    if [ -f /usr/lib/steam/subprojects/steam-devices/60-steam-input.rules ]; then
        cp /usr/lib/steam/subprojects/steam-devices/60-steam-input.rules \
           /etc/udev/rules.d/ 2>/dev/null || true
    fi
    if [ -f /usr/lib/steam/subprojects/steam-devices/60-steam-vr.rules ]; then
        cp /usr/lib/steam/subprojects/steam-devices/60-steam-vr.rules \
           /etc/udev/rules.d/ 2>/dev/null || true
    fi
    ...
    # Mapeadas desde el PKGBUILD de Arch (depends + depends_x86_64) a Ubuntu.
    # --install-recommends: el sistema tiene Install-Recommends=false global.
    safe_apt desktop-file-utils diffutils hicolor-icon-theme curl dbus \
        libfreetype6 libgdk-pixbuf2.0-0 fonts-liberation zenity lsb-release \
        libnss3 usbutils x11-xserver-utils lsof python3 \
    ...
    # Preparar directorios y symlinks de Steam
    if [ -n "\$USERNAME" ]; then
        STEAM_HOME="/home/\$USERNAME"
        sudo -u "\$USERNAME" mkdir -p "\$STEAM_HOME/.local/share/Steam/compatibilitytools.d"
    ...
    echo "✓  Steam instalado (se actualizará automáticamente en primer arranque)"
else
    echo "⚠  Steam: no se pudo descargar el tarball"
