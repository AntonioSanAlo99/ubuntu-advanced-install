...
INSTALL_DISCORD="${INSTALL_DISCORD:-false}"

# ── Decidir i386 según método de Steam ─────────────────────────────────────
# Métodos 1 (GLFS) y 2 (.deb) siempre necesitan i386.
# Método 3 (SteamRT3) puede funcionar sin i386 (containerizado).
USE_I386="true"
if [ "${STEAM_METHOD:-2}" = "3" ]; then
    echo ""
    echo "╔══════════════════════════════════════════════════════════════════╗"
...
# ============================================================================
# STEAM — tres métodos de instalación
# ============================================================================
#   1) GLFS 13.0 — tarball + make install (estable, probado, necesita i386)
#   2) .deb oficial Valve — método estándar recomendado (necesita i386)
#   3) SteamRT3 Beta — .deb + contenedor 64-bit (experimental)
#
# El método se elige con STEAM_METHOD (1, 2 o 3) desde install.sh.
# ============================================================================

if [ "${STEAM_METHOD:-2}" = "3" ]; then
    # ── Método 3: SteamRT3 Beta (64-bit, containerizado) ─────────────────────
    ...
    if [ -f "\$STEAM_DEB" ] && [ -s "\$STEAM_DEB" ]; then
        ...
        ln -sf /usr/bin/true /usr/bin/steamdeps
        mkdir -p /etc/environment.d
        echo "STEAM_RUNTIME=1" > /etc/environment.d/50-steam.conf

        echo "✓  Steam instalado (.deb oficial Valve — SteamRT3)"
...
elif [ "${STEAM_METHOD:-2}" = "2" ]; then
    # ── Método 2: .deb oficial de Valve (recomendado) ─────────────────────────
    # El método más simple y compatible. Instala el .deb estándar de Valve
    # con todas las dependencias i386 necesarias.
    echo ""
    echo "Instalando Steam (.deb oficial Valve)..."

    safe_apt \
        curl ca-certificates wget zenity xdg-user-dirs desktop-file-utils \
        libgl1 libgl1:i386 libglvnd0 libglvnd0:i386 \
        libvulkan1 libvulkan1:i386 mesa-vulkan-drivers mesa-vulkan-drivers:i386 \
        libnss3 libnss3:i386 libgpg-error0 libgpg-error0:i386 \
    ...
    # ── Anular steamdeps definitivamente ─────────────────────────────────────
    # make install del tarball reemplaza steamdeps dentro de /tmp/steam-build,
    # pero el script /usr/bin/steam sigue intentando ejecutar steamdeps del PATH.
    # Nos aseguramos de que /usr/bin/steamdeps sea siempre un no-op.
    ln -sf /usr/bin/true /usr/bin/steamdeps

    # ── Pre-copiar el bootstrap para que el primer arranque sea instantáneo ──
    # Sin esto, steam.sh descarga bootstraplinux_ubuntu12_32.tar.xz (~400MB) la
    # primera vez. Al pre-copiarlo, Steam arranca directo al binario.
    if [ -f /tmp/steam-build/steam-launcher/bootstraplinux_ubuntu12_32.tar.xz ]; then
        # Hay que hacerlo para el usuario, el directorio ya se crea más abajo
        install -d -o "$USERNAME" -g "$USERNAME" "/home/$USERNAME/.local/share/Steam"
        cp /tmp/steam-build/steam-launcher/bootstraplinux_ubuntu12_32.tar.xz \
            "/home/$USERNAME/.local/share/Steam/bootstrap.tar.xz"
        chown "$USERNAME:$USERNAME" "/home/$USERNAME/.local/share/Steam/bootstrap.tar.xz"
        echo "  ✓  Bootstrap pre-copiado (primer arranque instantáneo)"
    fi

    # ── Configurar STEAM_RUNTIME=1 para saltar verificación de deps ──────────
    # Steam ya tiene todas las deps del host instaladas (ver safe_apt arriba).
    # Con STEAM_RUNTIME=1, steam.sh usa el runtime bundled sin verificar nada.
    mkdir -p /etc/environment.d
    echo "STEAM_RUNTIME=1" > /etc/environment.d/50-steam.conf
    echo "  ✓  STEAM_RUNTIME=1 configurado"
...
# ── Estructura de directorios y preconfiguración de Steam ──────────────────
# Común a ambos métodos. Crea toda la estructura que Steam espera encontrar
# para que el primer arranque vaya directo al login sin first-run wizards.
if [ -n "\$USERNAME" ]; then
    STEAM_HOME="/home/\$USERNAME"
    STEAM_DIR="\$STEAM_HOME/.local/share/Steam"
    STEAM_CONFIG="\$STEAM_DIR/config"

    # ── Directorios ──────────────────────────────────────────────────────────
    install -d -o "\$USERNAME" -g "\$USERNAME" \
        "\$STEAM_DIR/compatibilitytools.d" \
        "\$STEAM_DIR/steamapps/common" \
        "\$STEAM_DIR/steamapps/downloading" \
        "\$STEAM_DIR/steamapps/temp" \
        "\$STEAM_DIR/logs" \
        "\$STEAM_DIR/dumps" \
        "\$STEAM_CONFIG" \
        "\$STEAM_HOME/.steam"
    ...
    # Symlinks canónicos
    ln -sfn "\$STEAM_DIR" "\$STEAM_HOME/.steam/steam"
    ln -sfn "\$STEAM_DIR" "\$STEAM_HOME/.steam/root"
    chown -h "\$USERNAME:\$USERNAME" "\$STEAM_HOME/.steam/steam" "\$STEAM_HOME/.steam/root"

    # ── registry.vdf — evita rutina de "nueva instalación" ───────────────────
    cat > "\$STEAM_DIR/registry.vdf" << 'REGEOF'
"Registry"
{
    "HKCU"
    {
...
REGEOF
    ...
    # ── config.vdf — wizards y first-run completados ─────────────────────────
    cat > "\$STEAM_CONFIG/config.vdf" << CFGEOF
"InstallConfigStore"
{
    "Software"
    {
        "Valve"
        {
            "Steam"
            {
                ...
                "SteamUIFirstRunComplete"       "1"
                "SteamCmdUIFirstRunComplete"    "1"
                "FirstRunComplete"              "1"
                "DownloadThrottleWhilePlaying"  "0"
                "MaxDownloadRate"               "0"
                "AutoUpdateWindowEnabled"       "0"
                "StartupMode"                   "0"
                "BaseInstallFolder_1"           "\$STEAM_DIR/steamapps"
            }
        }
    }
}
CFGEOF
    ...
    # ── loginusers.vdf — vacío pero válido (evita flujo nueva instalación) ───
    cat > "\$STEAM_CONFIG/loginusers.vdf" << 'LUEOF'
"users"
{
}
LUEOF
    ...
    # ── Permisos ─────────────────────────────────────────────────────────────
    chown -R "\$USERNAME:\$USERNAME" "\$STEAM_DIR" "\$STEAM_HOME/.steam"
    echo "  ✓  Steam preconfigurado (registry.vdf + config.vdf + loginusers.vdf)"

    # ── .desktop optimizado ──────────────────────────────────────────────────
    # -silent:         sin splash screen
    # -nofriendsui:    UI de amigos carga bajo demanda
    # -nominidumps:    sin volcado de memoria en background
    # -nocrashmonitor: sin monitor de crashes en background
    # NO -no-browser ni -noreactlogin (rompe tienda y login moderno)
    APPS_DIR="\$STEAM_HOME/.local/share/applications"
    mkdir -p "\$APPS_DIR"
    cat > "\$APPS_DIR/steam.desktop" << 'DTEOF'
[Desktop Entry]
Name=Steam
Comment=Steam Client
Exec=/usr/bin/steam -silent -nofriendsui -nominidumps -nocrashmonitor %U
Icon=steam
Terminal=false
Type=Application
Categories=Game;
...
[Desktop Action Quit]
Name=Salir de Steam
Exec=/usr/bin/steam -shutdown
DTEOF
    chown "\$USERNAME:\$USERNAME" "\$APPS_DIR/steam.desktop"
    echo "  ✓  .desktop optimizado (silent + sin crash monitor)"
fi

# ── Pre-aceptar licencia Steam (evita popup de debconf) ──────────────────
echo "steam steam/question select I AGREE" | debconf-set-selections 2>/dev/null || true
echo "steam steam/license note"            | debconf-set-selections 2>/dev/null || true
...
# ============================================================================
# VRR + EXPERIMENTAL FEATURES (condicional según versión de GNOME)
# ============================================================================
# GNOME 46-47: VRR y xwayland-native-scaling son experimental-features
# GNOME 48-49: VRR se activa desde Ajustes → Pantallas; xwayland-scaling sigue
# GNOME 50+:   VRR activado por defecto, X11 session eliminada
# ============================================================================
...
if arch-chroot "$TARGET" command -v gnome-shell >/dev/null 2>&1; then
    ...
    if [ -n "$GNOME_VER" ] && [ "$GNOME_VER" -ge 46 ] && [ "$GNOME_VER" -lt 50 ]; then
        echo ""
        echo "Configurando experimental-features para GNOME $GNOME_VER..."

        arch-chroot "$TARGET" /bin/bash << VRREOF
GNOME_VER=$GNOME_VER

# GNOME 50+ no necesita esto — VRR y xwayland-native-scaling son defaults
...
VRREOF
    elif [ -n "$GNOME_VER" ] && [ "$GNOME_VER" -ge 50 ]; then
        echo "✓  GNOME $GNOME_VER: VRR y xwayland-native-scaling activos por defecto"
    fi
fi
...
echo "  GPU: $GPU_CONFIG"
STEAM_LABEL="GLFS"
[ "${STEAM_METHOD:-2}" = "2" ] && STEAM_LABEL=".deb Valve"
[ "${STEAM_METHOD:-2}" = "3" ] && STEAM_LABEL="SteamRT3 Beta"
echo "  Steam ($STEAM_LABEL) + Heroic + Faugus"
