...
cd ..
dpkg -i linux-headers-*.deb linux-image-*.deb linux-libc-dev*.deb 2>/dev/null || true
...
# ============================================================================
# PARÁMETROS GRUB GAMING — opcionales (heredoc independiente)
# ============================================================================
# Parámetros del kernel que mejoran rendimiento a costa de seguridad.
# mitigations=off: desactiva protecciones Spectre/Meltdown (~5-10% más FPS)
# nowatchdog: elimina interrupciones del watchdog
# preempt=full: preemption completa para baja latencia
# tsc=reliable: evita calibración TSC innecesaria
# split_lock_detect=off: evita penalización de split locks
# ============================================================================

if [ "${GRUB_GAMING_PARAMS:-false}" = "true" ]; then
    echo ""
    echo "Configurando parámetros GRUB gaming..."

    arch-chroot "$TARGET" /bin/bash << 'GRUBEOF'
GRUB_FILE="/etc/default/grub"

if [ -f "$GRUB_FILE" ]; then
    # Parámetros gaming a añadir
    GAMING_PARAMS="mitigations=off nowatchdog preempt=full tsc=reliable split_lock_detect=off"
    ...
    echo "✓  Parámetros GRUB gaming aplicados: $GAMING_PARAMS"
    echo "   ⚠ AVISO: mitigations=off desactiva protecciones Spectre/Meltdown"
else
    echo "⚠  /etc/default/grub no encontrado — se omite"
fi
GRUBEOF
...
# Instalar zig (snap o binario)
ZIG_OK=false

# Método 1: snap (si snapd está disponible)
if command -v snap >/dev/null 2>&1; then
    snap install zig --classic 2>/dev/null && ZIG_OK=true
fi

# Método 2: binario estático desde ziglang.org
...
apt-get install -y power-profiles-daemon 2>/dev/null || true

BUILD_DIR="$(mktemp -d /tmp/falcond-build.XXXXXX)"
cd "$BUILD_DIR"

git clone --depth 1 https://github.com/PikaOS-Linux/falcond.git
cd falcond/falcond

# Compilar
/opt/zig/zig build -Doptimize=ReleaseFast 2>/dev/null \
...
SVCEOF
    ...
    # Config
    mkdir -p /etc/falcond
    if [ -f ../falcond.conf ] || [ -f debian/falcond.conf ]; then
        cp ../falcond.conf /etc/falcond/config.conf 2>/dev/null \
            || cp debian/falcond.conf /etc/falcond/config.conf 2>/dev/null || true
    fi
    ...
    # Crear grupo falcond y añadir usuario
    groupadd -f falcond 2>/dev/null || true
    usermod -aG falcond "$USERNAME" 2>/dev/null || true

    systemctl enable falcond 2>/dev/null || true
    echo "✓  Falcond instalado y habilitado"
else
    echo "⚠  Falcond: compilación fallida"
fi

cd /
rm -rf "$BUILD_DIR"

# Limpiar zig si se instaló como binario local
[ -d /opt/zig ] && rm -rf /opt/zig
FALCONDEOF
...
if git clone --depth 1 https://github.com/ind4skylivey/0ptiscaler4linux.git \
    ...
    chmod +x /opt/optiscaler/scripts/*.sh 2>/dev/null || true
...
[ "${GRUB_GAMING_PARAMS:-false}" = "true" ] && echo "  Parámetros GRUB gaming (mitigations=off, preempt=full)"
