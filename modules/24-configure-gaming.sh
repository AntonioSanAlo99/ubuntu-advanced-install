...
if [ "${STEAM_METHOD:-1}" = "2" ]; then
    ...
    if [ -f "\$STEAM_DEB" ] && [ -s "\$STEAM_DEB" ]; then
        ...
        # Anular steamdeps (el .deb de Valve instala su propio /usr/bin/steamdeps)
        ln -sf /usr/bin/true /usr/bin/steamdeps

        # STEAM_RUNTIME=1: saltar verificación de deps en steam.sh
        mkdir -p /etc/environment.d
        echo "STEAM_RUNTIME=1" > /etc/environment.d/50-steam.conf

        echo "✓  Steam instalado (.deb oficial Valve)"
        echo "  ✓  steamdeps anulado, STEAM_RUNTIME=1"
