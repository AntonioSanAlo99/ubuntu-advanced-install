...
apt-get install -y icoextract 2>/dev/null \
...
# ============================================================================
# dpkg -i + apt-get -f install resuelve automáticamente todas las dependencias
# (Qt6, libav*, libtag, libebur128, etc.) sin necesidad de detectarlas a mano.
    ...
    noble|plucky|questing|oracular)
        ...
        if wget --timeout=30 --tries=2 -q "\$FOOYIN_URL" -O /tmp/fooyin.deb 2>/dev/null \
           && [ -s /tmp/fooyin.deb ]; then
            dpkg -i /tmp/fooyin.deb 2>/dev/null || true
            apt-get install -f -y
            rm -f /tmp/fooyin.deb
