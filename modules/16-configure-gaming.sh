...
step " GameMode y MangoHud instalados"
...
if [ -f /tmp/steam.deb ]; then
    ...
    step " Steam instalado (.deb)"
else
    warn " No se pudo descargar Steam, instalando desde repos..."
...
if [ -n "\$FAUGUS_LATEST" ]; then
    ...
    if [ -f /tmp/faugus.deb ]; then
        ...
        rm /tmp/faugus.deb
        step " Faugus Launcher instalado (.deb)"
...
NoDisplay=true
IMAGEMAGICK_EOF
            step " ImageMagick ocultado del menú (dependencia de Faugus)"
        fi
    else
        warn " No se pudo descargar Faugus Launcher"
    fi
else
    warn " No se pudo obtener última versión de Faugus Launcher"
fi
...
if [ -n "\$HEROIC_LATEST" ]; then
    ...
    if [ -f /tmp/heroic.deb ]; then
        ...
        rm /tmp/heroic.deb
        step " Heroic Games Launcher instalado (.deb)"
    else
        warn " No se pudo descargar Heroic"
    fi
else
    warn " No se pudo obtener última versión de Heroic"
fi
...
if [ -n "\$PROTONUP_LATEST" ]; then
    ...
    if [ -f /tmp/protonup-qt.deb ]; then
        ...
        step " ProtonUp-Qt instalado (.deb)"
    else
        warn " No se pudo descargar ProtonUp-Qt"
    fi
else
    warn " No se pudo obtener última versión de ProtonUp-Qt"
fi
...
step " Estructura compartida configurada"
...
if [ -n "\$CACHYOS_LATEST" ]; then
    ...
    if [ -f /tmp/proton-cachyos.tar.gz ]; then
        ...
        step " Proton-Cachyos instalado"
        echo "  Gestionar con ProtonUp-Qt para actualizar"
    else
        warn " No se pudo descargar Proton-Cachyos"
    fi
else
    warn " No se pudo obtener última versión de Proton-Cachyos"
...
step " Variables de entorno configuradas"
...
step " Optimizaciones sysctl aplicadas"
...
if [ -n "$USERNAME" ]; then
    ...
    step " VRR habilitado en GNOME Mutter"
    ...
    step " Características experimentales habilitadas (incluye base para HDR)"
    ...
    if [ -n "$GNOME_VERSION" ] && [ "$GNOME_VERSION" -ge 47 ]; then
        step " GNOME $GNOME_VERSION detectado - Soporte HDR disponible"
        echo "  HDR se habilitará automáticamente con monitores compatibles"
    else
        warn " GNOME $GNOME_VERSION - HDR completo requiere GNOME 47+"
...
if [ -n "$USERNAME" ]; then
    ...
    step " Animaciones deshabilitadas (menor latencia en gaming)"
...
ANIMEKEEP
        step " Animaciones mantenidas (experiencia visual completa)"
    fi
    
    
    echo ""
    echo "═══════════════════════════════════════════════════════════"
    step " CONFIGURACIÓN GNOME APLICADA"
    echo "═══════════════════════════════════════════════════════════"
    echo ""
...
echo ""
echo "═══════════════════════════════════════════════════════════"
step " CONFIGURACIÓN GAMING COMPLETADA"
echo "═══════════════════════════════════════════════════════════"
echo ""
