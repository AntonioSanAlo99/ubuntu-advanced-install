...
# ============================================================================
# ============================================================================
# PROTONPLUS (gestor gráfico de versiones de Proton)
# ============================================================================
# ProtonPlus se distribuye como .deb en GitHub releases (Vysp3r/ProtonPlus).
# Pacstall era un intermediario que descargaba ese mismo .deb — eliminado.
# Instalamos directamente desde GitHub API con el mismo patrón que Heroic/Faugus.
# ============================================================================

echo ""
echo "Instalando ProtonPlus..."

PROTONPLUS_API="https://api.github.com/repos/Vysp3r/ProtonPlus/releases/latest"
PROTONPLUS_URL=$(curl --max-time 30 --retry 3 -s "$PROTONPLUS_API" \
...
if [ -n "$PROTONPLUS_URL" ]; then
    echo "  Descargando ProtonPlus desde GitHub..."
    if wget --timeout=30 --tries=3 -q "$PROTONPLUS_URL" -O /tmp/protonplus.deb; then
        apt-get install -y /tmp/protonplus.deb
        rm -f /tmp/protonplus.deb
        echo "  ✓ ProtonPlus instalado"
    else
        echo "  ⚠ Error descargando ProtonPlus — omitido"
    fi
else
    echo "  ⚠ No se pudo obtener URL de ProtonPlus desde GitHub API — omitido"
    echo "  Instalar manualmente: https://github.com/Vysp3r/ProtonPlus/releases"
fi
