...
# ============================================================================
# STEAM (steam-installer desde repos Ubuntu + preconfiguración)
# ============================================================================

echo ""
echo "Instalando Steam..."

# Dependencias previas (i386 ya habilitado arriba)
apt install -y \$APT_FLAGS \
    libgl1-mesa-dri:i386 \
    libgl1:i386 \
    libc6:i386 \
...
# Aceptar licencia de Steam automáticamente (evita prompt interactivo)
...
# Instalar steam-installer (paquete de Ubuntu, añade repo de Valve y descarga Steam)
STEAM_OK=false
if apt install -y steam-installer 2>/dev/null; then
    STEAM_OK=true
    echo "✓  Steam instalado (steam-installer)"
else
    # Fallback: .deb directo de Valve
    echo "⚠  steam-installer falló, descargando .deb de Valve..."
    ...
    if [ -f /tmp/steam.deb ]; then
        ...
        rm -f /tmp/steam.deb
        if command -v steam &>/dev/null || [ -f /usr/bin/steam ] || [ -f /usr/games/steam ]; then
            STEAM_OK=true
...
# ── Preconfigurar Steam para evitar ventana de primer arranque ────────────────
# Steam al primer inicio lanza un wizard de configuración + se auto-actualiza.
# Pre-creamos la estructura de directorios y config básica para que Steam
# arranque silenciosamente cuando el usuario lo lance por primera vez.
if [ "\$STEAM_OK" = "true" ] && [ -n "\$USERNAME" ]; then
    STEAM_CFG="/home/\$USERNAME/.local/share/Steam"

    mkdir -p "\$STEAM_CFG/config"
    mkdir -p "\$STEAM_CFG/compatibilitytools.d"
    mkdir -p "\$STEAM_CFG/ubuntu12_32"

    # config.vdf — configuración base de Steam
    cat > "\$STEAM_CFG/config/config.vdf" << 'STEAMVDF'
"InstallConfigStore"
{
	"Software"
	{
...
STEAMVDF
    ...
    # steam.cfg — desactiva bootstrap updater popup
    cat > "\$STEAM_CFG/steam.cfg" << 'STEAMCFGEOF'
BootStrapperInhibitAll=enable
BootStrapperForceSelf=disable
STEAMCFGEOF
...
STEAMAUTOEOF
    ...
    chown -R \$(id -u \$USERNAME):\$(id -g \$USERNAME) \
        ...
        "\$AUTOSTART_DIR/steam.desktop"

    echo "✓  Steam preconfigurado (sin autostart, sin popup de primer arranque)"
fi
