...
# ============================================================================
# STEAM (repositorio oficial de Valve — repo.steampowered.com)
# ============================================================================

echo ""
echo "Instalando Steam..."

# Añadir clave GPG de Valve
apt-key adv --keyserver keyserver.ubuntu.com --recv-keys B05498B7 2>/dev/null || true

# Añadir repositorio oficial
echo "deb [arch=amd64,i386 signed-by=/usr/share/keyrings/steam.gpg] http://repo.steampowered.com/steam/ stable steam" \
    > /etc/apt/sources.list.d/steam.list

# Exportar clave al keyring si apt-key la importó en el legacy keyring
if ! [ -f /usr/share/keyrings/steam.gpg ]; then
    apt-key export B05498B7 2>/dev/null | gpg --dearmor -o /usr/share/keyrings/steam.gpg 2>/dev/null || true
fi

apt-get update 2>/dev/null || true
apt-get install -y steam 2>/dev/null && echo "✓  Steam instalado" || echo "⚠  Steam: instálalo manualmente tras el primer boot"
...
if command -v am >/dev/null 2>&1; then
    am -i protonup-qt 2>/dev/null && echo "✓  ProtonUp-Qt instalado vía AM" \
        || echo "⚠  ProtonUp-Qt: am falló — instalar después con 'am -i protonup-qt'"
else
    echo "⚠  ProtonUp-Qt: AM no disponible — instalar después con 'am -i protonup-qt'"
fi
