...
ANIMEKEEP
    ...
    arch-chroot "$TARGET" /bin/bash << 'FINALGAME'
USERNAME=$(grep "1000" /etc/passwd | cut -d: -f1)
if [ -n "$USERNAME" ]; then
...
## VRR (Variable Refresh Rate)
Estado: Habilitado
Comando: gsettings set org.gnome.mutter experimental-features "['variable-refresh-rate']"

## HDR (High Dynamic Range)
Estado: Características experimentales habilitadas
Requisitos:
  - GNOME 47 o superior
  - Monitor compatible con HDR
  - GPU con soporte HDR (NVIDIA RTX/AMD RX 5000+/Intel Arc)
  
Verificación HDR:
  - Settings → Displays → HDR (si disponible)
  - xrandr --prop | grep -i hdr

## Optimizaciones aplicadas
- VRR habilitado
- Compositor optimizado para menor latencia
...
EOF
    ...
    echo "⚠ Usuario no encontrado, configuración de GNOME omitida"
fi
