...
# Se instala el .deb oficial de Valve en lugar del paquete apt del PPA.
# El .deb de cdn.akamai contiene el cliente Steam completo con sus binarios
# incluidos — no requiere bootstrap al primer arranque.
# El PPA de Valve solo instala un launcher que descarga Steam en el primer uso.
...
if wget --timeout=30 --tries=2 -q "$STEAM_URL" -O "$STEAM_DEB" 2>/dev/null \
   && [ -s "$STEAM_DEB" ]; then
    ...
    rm -f "$STEAM_DEB"
    echo "✓  Steam instalado (cliente completo, sin bootstrap al primer arranque)"
else
