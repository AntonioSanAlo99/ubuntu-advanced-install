...
# ============================================================================
# STEAM (.deb oficial de Valve)
# ============================================================================

echo ""
echo "Instalando Steam..."

# Dependencias i386 previas
apt install -y \$APT_FLAGS \
    libgl1-mesa-dri:i386 \
    libgl1:i386 \
    libc6:i386 \
...
# Aceptar licencia de Steam (evita prompt interactivo)
...
wget --timeout=30 --tries=3 -q "https://cdn.akamai.steamstatic.com/client/installer/steam.deb" \
    -O /tmp/steam.deb 2>/dev/null || true

if [ -f /tmp/steam.deb ]; then
    dpkg -i /tmp/steam.deb 2>/dev/null || true
    apt --fix-broken install -y
    rm -f /tmp/steam.deb

    if [ -f /usr/bin/steam ] || [ -f /usr/games/steam ]; then
        STEAM_OK=true
        echo "✓  Steam instalado (.deb Valve)"
    fi
fi
...
if [ -n "\$FAUGUS_LATEST" ]; then
    ...
    if [ -f /tmp/faugus.deb ]; then
        ...
        # Usar override en home del usuario — no modificar /usr/share/applications/
        IM_DESKTOP=""
        ...
        if [ -n "\$IM_DESKTOP" ] && [ -n "\$USERNAME" ]; then
            IM_USER="/home/\$USERNAME/.local/share/applications/\$IM_DESKTOP"
            mkdir -p "/home/\$USERNAME/.local/share/applications"
            cp "/usr/share/applications/\$IM_DESKTOP" "\$IM_USER"
            sed -i '/^NoDisplay=/d' "\$IM_USER"
            echo "NoDisplay=true" >> "\$IM_USER"
            chown \$(id -u \$USERNAME):\$(id -g \$USERNAME) "\$IM_USER"
...
# ============================================================================
# PROTONUP-QT (gestor de versiones de Proton — AppImage)
# ============================================================================
# ProtonUp-Qt gestiona GE-Proton, Wine-GE, Luxtorpeda, etc. para Steam,
# Lutris y Heroic. Se instala usando AM/AppMan (instalado en módulo 10).
#   - am -i protonup-qt (instalación sistema con AM)
#   - appman -i protonup-qt (instalación local con AppMan)
#   - descarga .deb de GitHub como último fallback
...
PROTONUPQT_INSTALLED=false

# Método 1: Instalar vía AM (sistema) — método preferido
if command -v am >/dev/null 2>&1; then
    echo "  Usando AM para instalar ProtonUp-Qt..."
    if am -i protonup-qt 2>/dev/null; then
        PROTONUPQT_INSTALLED=true
        echo "  ✓ ProtonUp-Qt instalado vía AM"
    else
        echo "  ⚠ AM falló al instalar ProtonUp-Qt"
    fi
fi

# Método 2: Instalar vía AppMan (local) si AM no estaba o falló
if [ "\$PROTONUPQT_INSTALLED" = "false" ] && command -v appman >/dev/null 2>&1; then
    echo "  Usando AppMan para instalar ProtonUp-Qt..."
    if appman -i protonup-qt 2>/dev/null; then
        PROTONUPQT_INSTALLED=true
        echo "  ✓ ProtonUp-Qt instalado vía AppMan"
    else
        echo "  ⚠ AppMan falló al instalar ProtonUp-Qt"
    fi
fi

# Método 3: Descargar .deb desde GitHub releases como fallback
if [ "\$PROTONUPQT_INSTALLED" = "false" ]; then
    echo "  AM/AppMan no disponible — intentando .deb desde GitHub..."
    PROTONUPQT_DEB_URL=\$(curl --max-time 30 --retry 3 -s https://api.github.com/repos/DavidoTek/ProtonUp-Qt/releases/latest \
        | grep "browser_download_url.*\.deb\"" | cut -d '"' -f 4 | head -1)

    if [ -n "\$PROTONUPQT_DEB_URL" ]; then
        echo "  Descargando ProtonUp-Qt .deb..."
        if wget --timeout=30 --tries=3 -q "\$PROTONUPQT_DEB_URL" -O /tmp/protonup-qt.deb; then
            if dpkg -i /tmp/protonup-qt.deb 2>/dev/null; then
                apt install -f -y 2>/dev/null  # Resolver dependencias si faltaban
                PROTONUPQT_INSTALLED=true
                echo "  ✓ ProtonUp-Qt instalado vía .deb"
            else
                apt install -f -y 2>/dev/null
                if dpkg -i /tmp/protonup-qt.deb 2>/dev/null; then
                    PROTONUPQT_INSTALLED=true
                    echo "  ✓ ProtonUp-Qt instalado vía .deb (tras resolver deps)"
                else
                    echo "  ⚠ Error instalando .deb de ProtonUp-Qt"
                fi
            fi
            rm -f /tmp/protonup-qt.deb
        else
            echo "  ⚠ Error descargando ProtonUp-Qt .deb — omitido"
        fi
    else
        echo "  ⚠ No se encontró .deb en releases de ProtonUp-Qt — omitido"
    fi
fi

if [ "\$PROTONUPQT_INSTALLED" = "false" ]; then
    echo "  ⚠ ProtonUp-Qt no se pudo instalar"
    echo "  Puedes instalarlo después con: am -i protonup-qt"
fi
...
echo "  ✅ Steam (steam-installer)"
echo "  ✅ Heroic Games Launcher (.deb)"
echo "  ✅ Faugus Launcher (.deb)"
echo "  ✅ ProtonUp-Qt (gestor de Proton/Wine-GE)"
