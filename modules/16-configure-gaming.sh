...
# Steam (repo Valve) + Heroic + Faugus + ProtonUp-Qt + Proton-Cachyos + drivers GPU
...
# ============================================================================
# STEAM (.deb oficial de Valve — steam_latest.deb)
# ============================================================================
# El .deb oficial instala: steam-launcher (bootstrap que se autoactualiza),
# steam-devices (udev rules para mandos), y configura el repo de Valve para
# futuras actualizaciones via apt. dpkg -i + apt-get install -f resuelve
# todas las dependencias (Mesa i386, libgl, etc.) automáticamente.
# ============================================================================

echo ""
echo "Instalando Steam..."

# Habilitar arquitectura i386 (requerida por Steam y Proton)
dpkg --add-architecture i386 2>/dev/null || true
apt-get update 2>/dev/null || true

# Descargar .deb oficial de Valve
wget --timeout=30 --tries=3 -q "https://repo.steampowered.com/steam/archive/stable/steam_latest-stable.deb" \
    -O /tmp/steam_latest.deb 2>/dev/null || true

if [ -f /tmp/steam_latest.deb ] && file /tmp/steam_latest.deb | grep -q "Debian binary package"; then
    dpkg -i /tmp/steam_latest.deb 2>/dev/null || true
    apt-get install -f -y 2>/dev/null || true
    rm -f /tmp/steam_latest.deb
    echo "✓  Steam instalado (steam_latest.deb)"
else
    echo "⚠  Steam: descarga falló — instalar manualmente tras el primer boot"
    rm -f /tmp/steam_latest.deb
fi
...
HEROIC_URL=\$(curl --max-time 15 --retry 2 -sL \
...
if [ -n "\$HEROIC_URL" ]; then
    wget --timeout=30 --tries=2 -q "\$HEROIC_URL" -O /tmp/heroic.deb 2>/dev/null || true
    if [ -f /tmp/heroic.deb ]; then
        dpkg -i /tmp/heroic.deb 2>/dev/null || true
        apt-get install -f -y 2>/dev/null || true
        rm -f /tmp/heroic.deb
        echo "✓  Heroic Games Launcher instalado"
    else
        echo "⚠  Heroic: descarga falló"
    fi
else
    echo "⚠  Heroic: no se pudo consultar GitHub API"
fi
...
echo ""
echo "Configurando estructura de Proton..."

if [ -n "\$USERNAME" ]; then
    PROTON_DIR="/home/\$USERNAME/.local/share/Steam/compatibilitytools.d"
    mkdir -p "\$PROTON_DIR"

    # Symlinks para Faugus y Heroic
    mkdir -p "/home/\$USERNAME/.local/share/faugus-launcher"
    ln -sf "\$PROTON_DIR" "/home/\$USERNAME/.local/share/faugus-launcher/compatibilitytools.d" 2>/dev/null || true

    mkdir -p "/home/\$USERNAME/.config/heroic/tools/proton"
    ln -sf "\$PROTON_DIR" "/home/\$USERNAME/.config/heroic/tools/proton/Steam" 2>/dev/null || true
    ...
    if [ -n "\$PCACHYOS_URL" ]; then
        ...
        if [ -f /tmp/proton-cachyos.tar.gz ]; then
            ...
            echo "⚠  Proton-Cachyos: descarga falló"
        fi
    else
        echo "⚠  Proton-Cachyos: usar ProtonUp-Qt para instalarlo después"
    fi

    chown -R \$(id -u \$USERNAME):\$(id -g \$USERNAME) \
        "/home/\$USERNAME/.local/share/Steam" \
        "/home/\$USERNAME/.local/share/faugus-launcher" \
        "/home/\$USERNAME/.config/heroic" 2>/dev/null || true

    echo "✓  Estructura de Proton configurada"
fi
...
if chroot "$TARGET" command -v gnome-shell >/dev/null 2>&1; then
    GNOME_VER=$(chroot "$TARGET" gnome-shell --version 2>/dev/null | grep -oP '\d+' | head -1)
...
if [ -n "$CACHYOS_SCHED" ]; then
    ...
    chroot "$TARGET" /bin/bash << CACHYOS_EOF
export DEBIAN_FRONTEND=noninteractive
SCHED="$CACHYOS_SCHED"
...
echo ""
echo "  GPU: $GPU_CONFIG"
echo "  Steam, Heroic, Faugus, ProtonUp-Qt, Proton-Cachyos"
