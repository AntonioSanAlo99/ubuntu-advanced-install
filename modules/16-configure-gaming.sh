...
# Lutris y Heroic. Se instala usando el gestor de AppImages disponible:
#   - am/appman (descarga e integra desde su base de datos)
#   - app-manager CLI (requiere .AppImage ya descargado)
...
# Método 1: Instalar vía AM/AppMan (descarga e integra automáticamente)
...
# Método 2/3: Descargar AppImage y registrar con app-manager o dejar en ~/Applications
if [ "\$PROTONUPQT_INSTALLED" = "false" ]; then
    ...
    if [ -n "\$PROTONUPQT_URL" ]; then
        ...
        if wget --timeout=30 --tries=3 -q "\$PROTONUPQT_URL" -O "\$APPS_DIR/ProtonUp-Qt.AppImage"; then
            ...
            # Intentar registrar con app-manager CLI si está disponible
            if command -v app-manager >/dev/null 2>&1; then
                if app-manager install "\$APPS_DIR/ProtonUp-Qt.AppImage" 2>/dev/null; then
                    PROTONUPQT_INSTALLED=true
                    echo "  ✓ ProtonUp-Qt instalado vía AppManager CLI"
                fi
            fi

            # Si app-manager no estaba o falló, queda como descarga directa
            if [ "\$PROTONUPQT_INSTALLED" = "false" ]; then
                PROTONUPQT_INSTALLED=true
                echo "  ✓ ProtonUp-Qt descargado en ~/Applications/"
            fi
        else
