...
# ============================================================================
# VRR + HDR (GNOME — escrito en dconf de usuario directamente)
# ============================================================================
# experimental-features es un array que 10-user-config.sh ya configura con
# ['xwayland-native-scaling']. Aquí añadimos VRR (y HDR en GNOME 48+)
# al mismo array usando dconf load, sin autostart diferido ni D-Bus.
# Se ejecuta ANTES de 10-user-config.sh, que hará la sincronización final.
...
if arch-chroot "$TARGET" command -v gnome-shell >/dev/null 2>&1; then
    ...
    if [ -n "$GNOME_VER" ] && [ "$GNOME_VER" -ge 46 ]; then
        ...
        VRR_FEATURES="'xwayland-native-scaling', 'variable-refresh-rate'"
        [ "$GNOME_VER" -ge 48 ] && VRR_FEATURES="'xwayland-native-scaling', 'variable-refresh-rate', 'hdr'"

        arch-chroot "$TARGET" /bin/bash << VRREOF
USERNAME="$USERNAME"
VRR_FEATURES="$VRR_FEATURES"

if ! id "\$USERNAME" >/dev/null 2>&1; then exit 0; fi

# Escribir solo la clave experimental-features con el array completo.
# 10-user-config.sh (que corre después) también escribe esta clave, así que
# guardamos el valor en un archivo que user-config leerá e incorporará.
# Solución más simple: escribir aquí en dconf — user-config lo sobreescribirá
# con el mismo valor o superior. No hay conflicto porque escribimos el mismo path.
install -d -m 0700 -o "\$USERNAME" -g "\$USERNAME" "/home/\$USERNAME/.config/dconf"
su -s /bin/bash "\$USERNAME" -c "dconf write /org/gnome/mutter/experimental-features \"[\$VRR_FEATURES]\""

echo "✓  experimental-features: [\$VRR_FEATURES]"
VRREOF

        VRR_LABEL="VRR"
        [ "$GNOME_VER" -ge 48 ] && VRR_LABEL="VRR/HDR"
        echo "✓  $VRR_LABEL configurado en dconf (GNOME $GNOME_VER)"
    fi
fi
