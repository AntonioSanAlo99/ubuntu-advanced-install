...
# Verificar genfstab y arch-chroot (de arch-install-scripts)
if ! command -v genfstab &> /dev/null || ! command -v arch-chroot &> /dev/null; then
    echo "⚠  arch-install-scripts no encontrado (genfstab + arch-chroot)"
    REQUIRED_PACKAGES+=("arch-install-scripts")
else
    echo "✓  arch-install-scripts instalado (genfstab + arch-chroot)"
fi
...
# Verificar dosfstools (mkfs.fat para particiones EFI en sistemas UEFI)
if ! command -v mkfs.fat &> /dev/null; then
    echo "⚠  dosfstools no encontrado (necesario para UEFI)"
    REQUIRED_PACKAGES+=("dosfstools")
else
    echo "✓  dosfstools instalado"
fi
...
if [ ${#REQUIRED_PACKAGES[@]} -gt 0 ]; then
    ...
    echo ""
    apt update
    apt install -y --no-install-recommends "${REQUIRED_PACKAGES[@]}"
    echo ""
