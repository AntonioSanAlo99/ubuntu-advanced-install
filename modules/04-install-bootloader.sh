...
if [ "$FIRMWARE" = "UEFI" ]; then
    ...
    if [ -z "$TARGET_DISK" ]; then
        error " ERROR: TARGET_DISK no definido para sistema BIOS"
...
step " Parámetros de boot configurados"
...
if [ "${DUAL_BOOT_MODE:-false}" = "true" ]; then
    ...
    if [ -s /tmp/os-prober.out ]; then
        ...
        warn " No se detectaron otros sistemas operativos"
    fi
fi
...
if [ \$UPDATE_GRUB_EXIT -eq 0 ]; then
    step " grub.cfg generado correctamente"
else
    warn " update-grub exit code: \$UPDATE_GRUB_EXIT"
    echo "Salida:"
...
if ! ls "$TARGET/boot/vmlinuz-"* >/dev/null 2>&1; then
    error " CRÍTICO: No se encontró kernel en $TARGET/boot/"
    VALIDATION_FAILED=true
else
    KERNEL_VERSION=$(ls "$TARGET/boot/vmlinuz-"* | tail -1 | xargs basename | sed 's/vmlinuz-//')
    step " Kernel: $KERNEL_VERSION"
fi

# Verificar grub.cfg
if [ ! -f "$TARGET/boot/grub/grub.cfg" ]; then
    error " CRÍTICO: grub.cfg no existe"
    VALIDATION_FAILED=true
else
    GRUB_LINES=$(wc -l < "$TARGET/boot/grub/grub.cfg")
    if [ "$GRUB_LINES" -lt 10 ]; then
        error " CRÍTICO: grub.cfg demasiado pequeño ($GRUB_LINES líneas)"
        VALIDATION_FAILED=true
    else
        step " grub.cfg: $GRUB_LINES líneas"
    fi
fi

# Verificar entrada de Ubuntu en grub.cfg
if grep -q "menuentry.*Ubuntu" "$TARGET/boot/grub/grub.cfg"; then
    step " Entrada de Ubuntu encontrada en GRUB"
else
    warn " Advertencia: No se encontró entrada 'Ubuntu' en grub.cfg"
fi

# Verificar initramfs
if ls "$TARGET/boot/initrd.img-"* >/dev/null 2>&1; then
    step " initramfs presente"
else
    warn " Advertencia: initramfs no encontrado"
fi
...
if [ "$VALIDATION_FAILED" = true ]; then
    echo "════════════════════════════════════════════════════════════════"
    error " INSTALACIÓN DE BOOTLOADER FALLÓ"
    echo "════════════════════════════════════════════════════════════════"
    echo ""
...
if [ $CHROOT_EXIT -ne 0 ]; then
    echo "════════════════════════════════════════════════════════════════"
    warn " ADVERTENCIA: Exit code del chroot = $CHROOT_EXIT"
    echo "════════════════════════════════════════════════════════════════"
    echo ""
...
echo "════════════════════════════════════════════════════════════════"
step " BOOTLOADER INSTALADO CORRECTAMENTE"
echo "════════════════════════════════════════════════════════════════"
