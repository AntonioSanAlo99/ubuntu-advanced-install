...
# REQUIERE: TARGET, TARGET_DISK, ROOT_PART, EFI_PART, FIRMWARE, DUAL_BOOT_MODE, KERNEL_PARAMS_LEVEL
# PRODUCE:  Kernel + GRUB instalados
...
[ -z "${KERNEL_PARAMS_LEVEL:-}" ] && KERNEL_PARAMS_LEVEL=$(_yaml_val "system_tuning" "kernel_params" "1")
...
# ============================================================================
# Parámetros del kernel configurables en 3 niveles (KERNEL_PARAMS_LEVEL):
#
#   1) Estándar — escritorio + gaming optimizado (Clear Linux defaults)
#      quiet splash intel_pstate=active no_timer_check page_alloc.shuffle=1
#      rcupdate.rcu_expedited=1 nowatchdog nmi_watchdog=0
#      preempt=full tsc=reliable split_lock_detect=off
#
#   2) Estándar + mitigations=off — máximo rendimiento
#      Todo lo de (1) + mitigations=off (desactiva Spectre/Meltdown, +5-10% FPS)
...
PARAMS_STANDARD="quiet splash intel_pstate=active no_timer_check page_alloc.shuffle=1 rcupdate.rcu_expedited=1 nowatchdog nmi_watchdog=0 preempt=full tsc=reliable split_lock_detect=off"
PARAMS_UNSAFE="mitigations=off"

case "${KERNEL_PARAMS_LEVEL:-1}" in
    2) BOOT_PARAMS="\$PARAMS_STANDARD \$PARAMS_UNSAFE" ;;
    3) BOOT_PARAMS="quiet splash" ;;
    *) BOOT_PARAMS="\$PARAMS_STANDARD" ;;
esac
...
# ── LUKS: cryptdevice en GRUB + crypttab + initramfs ─────────────────────────
LUKS_ON="${LUKS_ENABLED:-false}"
LUKS_DEV_UUID="${LUKS_UUID:-}"

if [ "\$LUKS_ON" = "true" ] && [ -n "\$LUKS_DEV_UUID" ]; then
    echo "Configurando GRUB para LUKS..."

    # GRUB_CMDLINE_LINUX: añadir cryptdevice para que initramfs pregunte contraseña
    CRYPT_PARAM="cryptdevice=UUID=\$LUKS_DEV_UUID:crypt-root root=/dev/mapper/crypt-root"
    CURRENT_LINUX=\$(grep '^GRUB_CMDLINE_LINUX=' "\$GRUB_FILE" | cut -d'"' -f2 || echo "")
    if ! echo "\$CURRENT_LINUX" | grep -q "cryptdevice"; then
        NEW_LINUX="\$CURRENT_LINUX \$CRYPT_PARAM"
        NEW_LINUX=\$(echo "\$NEW_LINUX" | xargs)
        sed -i "s|^GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX=\"\$NEW_LINUX\"|" "\$GRUB_FILE"
    fi

    # GRUB_ENABLE_CRYPTODISK: necesario para que GRUB pueda leer /boot dentro de LUKS
    if ! grep -q '^GRUB_ENABLE_CRYPTODISK=' "\$GRUB_FILE"; then
        echo 'GRUB_ENABLE_CRYPTODISK=y' >> "\$GRUB_FILE"
    fi

    # /etc/crypttab: mapeo de LUKS para initramfs
    echo "crypt-root UUID=\$LUKS_DEV_UUID none luks,discard" > /etc/crypttab
    echo "✓  /etc/crypttab configurado"

    # Paquetes cryptsetup en initramfs
    apt-get install -y cryptsetup-initramfs 2>/dev/null || true

    # Regenerar initramfs con hook de cryptsetup
    update-initramfs -u -k all 2>/dev/null || true
    echo "✓  initramfs actualizado con soporte LUKS"
fi
