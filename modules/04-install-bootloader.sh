...
export GRUB_TARGET="$GRUB_TARGET"
export EFI_DIR="$EFI_DIR"
export APT_FLAGS="$APT_FLAGS"

# Preseed grub-pc para BIOS (evita prompt interactivo de debconf)
...
if [ $CHROOT_EXIT -ne 0 ]; then
    ...
    echo "Esto puede ocurrir en entornos chroot (acceso limitado a hardware)."
    echo ""
