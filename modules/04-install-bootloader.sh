...
apt-get install -y \$APT_FLAGS $GRUB_PKG
...
# ============================================================================
# Parámetros equivalentes a los usados por Ubuntu 24.04 y Clear Linux:
#   quiet splash      — boot gráfico (Plymouth)
#   intel_pstate      — scheduler de frecuencia Intel (mejor respuesta)
#   no_timer_check    — evita falsos positivos en timers en VMs
#   page_alloc.shuffle— aleatoriza orden de páginas libres (seguridad + perf)
#   rcupdate.rcu_expedited — RCU expedito en escritorio
#   nowatchdog        — desactiva softlockup detector (reduce latencia)
# ============================================================================

GRUB_FILE="/etc/default/grub"

# Parámetros de kernel para escritorio/gaming
# ima_disable_htable=1: deshabilita la tabla hash de IMA que en Ubuntu 24.04
# (CONFIG_IMA_DISABLE_HTABLE=n por defecto) rechaza entradas duplicadas y
# genera warnings en el arranque. El parámetro de cmdline lo sobreescribe
# sin necesidad de recompilar el kernel.
BOOT_PARAMS="quiet splash intel_pstate=active no_timer_check page_alloc.shuffle=1 rcupdate.rcu_expedited=1 nowatchdog nmi_watchdog=0 ima_disable_htable=1"
...
NEW_CMDLINE=\$(echo "\$NEW_CMDLINE" | xargs)  # normalizar espacios
