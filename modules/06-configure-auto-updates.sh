...
# Desactivar update-notifier y update-manager — redundantes con unattended-upgrades
# update-notifier: demonio que muestra el icono de actualizaciones en la barra
# update-manager: interfaz gráfica de actualización manual
mkdir -p /etc/xdg/autostart
if [ -f /etc/xdg/autostart/update-notifier.desktop ]; then
    sed -i '/^Hidden=/d; /^X-GNOME-Autostart-enabled=/d' /etc/xdg/autostart/update-notifier.desktop
...
echo "  ✓ update-notifier y update-manager desactivados"
ENABLE_AUTO_EOF
        ;;
    3)
        echo ""
...
# Respetar valor de config.env si está definido; si no, preguntar interactivamente
if [ -z "$NOTIFY_RELEASES" ]; then
    read -p "¿Activar notificación de nuevas versiones? [S/n]: " NOTIFY_RELEASES
    NOTIFY_RELEASES="${NOTIFY_RELEASES:-S}"
fi
...
# .config/autostart ya existe con el propietario correcto porque useradd -m
# lo creó copiando /etc/skel (que 03-configure-base preparó antes de useradd).
USERNAME=$(grep ":1000:" /etc/passwd | cut -d: -f1)
if [ -n "$USERNAME" ] && [ -d "/home/$USERNAME/.config/autostart" ]; then
    cp /etc/skel/.config/autostart/check-ubuntu-release.desktop \
       /home/$USERNAME/.config/autostart/
    chown "$USERNAME:$USERNAME" \
          "/home/$USERNAME/.config/autostart/check-ubuntu-release.desktop"
fi
