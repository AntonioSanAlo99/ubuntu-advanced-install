...
case $AUTO_UPDATE_CHOICE in
    1|2)
        ...
        chroot "$TARGET" /bin/bash << 'AUTO_UPDATE_EOF'
...
AUTO_UPDATE_EOF
        ...
        if [ "$AUTO_UPDATE_CHOICE" = "2" ]; then
            ...
            chroot "$TARGET" /bin/bash << 'STABLE_UPDATES_EOF'
...
STABLE_UPDATES_EOF
            ...
            chroot "$TARGET" /bin/bash << 'SECURITY_ONLY_EOF'
...
SECURITY_ONLY_EOF
        ...
        chroot "$TARGET" /bin/bash << 'ENABLE_AUTO_EOF'
...
if [ "$NOTIFY_RELEASES" != "n" ] && [ "$NOTIFY_RELEASES" != "N" ]; then
    ...
    chroot "$TARGET" /bin/bash << 'RELEASE_NOTIFY_EOF'
# ============================================================================
