...
if [ -z "$INSTALL_QBITTORRENT" ]; then
    echo ""
    read -p "¿Instalar qBittorrent (cliente torrent)? (s/n) [n]: " INSTALL_QBITTORRENT
...
if [[ "\$INSTALL_QBITTORRENT" =~ ^[SsYy]$ ]]; then
    echo ""
...
[[ "${INSTALL_QBITTORRENT:-n}" =~ ^[SsYy]$ ]] && echo "  ✓ qBittorrent"
