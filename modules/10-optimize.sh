...
if [ "$DISABLE_ANIMATIONS" = "s" ] || [ "$DISABLE_ANIMATIONS" = "S" ]; then
    ...
    # Usar dconf defaults de sistema (coherente con 10-user-config.sh)
    # profile.d se ejecuta en cualquier shell — incorrecto para gsettings
    mkdir -p /etc/dconf/db/local.d
    cat >> /etc/dconf/db/local.d/00-gnome-installer << 'ANIM'
...
ANIM
    ...
    echo "✓  Animaciones deshabilitadas via dconf (~30-50MB)"
fi
