...
# Activación: extensión user-theme activada en 10-user-config vía dconf load
#
# ══════════════════════════════════════════════════════════════════════════════
...
# El directorio .themes ya existe en skel (creado en 03-configure-base).
# Se escribe el CSS en skel. Al final del bloque se copia al home del
# usuario existente y se aplica un único chown -R.
...
/* ══════════════════════════════════════════════════════════════════════════════
   ...
   El escritorio se ve tal cual, sin capa gris superpuesta
   ══════════════════════════════════════════════════════════════════════════════ */
...
# Copiar desde skel al home del usuario (que ya existe porque 03 creó el usuario)
# cp -a preserva permisos de skel; --no-clobber no sobreescribe si ya hay algo
cp -a --no-clobber /etc/skel/.themes/. "\$USER_HOME/.themes/"
# Un único chown -R cubre todo el árbol de una sola vez
...
echo -e "${C_INFO}ℹ${C_RESET}  Tema escrito en skel y configurado en dconf — activo desde el primer boot"
