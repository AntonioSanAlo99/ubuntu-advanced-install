...
# Invocado por ~/.config/autostart/gnome-first-login.desktop
# Se autodestruye al completarse.

MARKER="$HOME/.config/.gnome-user-configured"
AUTOSTART_DESKTOP="$HOME/.config/autostart/gnome-first-login.desktop"

# Doble guardia: marker + verificar que estamos en GNOME
[ -f "$MARKER" ] && exit 0
[ "$XDG_CURRENT_DESKTOP" != "GNOME" ] && exit 0
[ -z "$DBUS_SESSION_BUS_ADDRESS" ] && exit 0

sleep 3  # Esperar a que GNOME Shell esté completamente cargado
...
# Reiniciar shell en X11 para aplicar extensiones inmediatamente
