...
# ── Extensiones ───────────────────────────────────────────────────────────────
# Solo habilitamos las 3 extensiones esenciales.
# No tocamos disabled-extensions ni disable-user-extensions para no
# interferir con la configuración que el usuario haga después.
EXTENSIONS=(
...
# Merge: leer lista actual y añadir las nuestras sin borrar lo que haya
...
# GNOME por defecto solo muestra el botón de cerrar.
# button-layout: izquierda:derecha — los botones a la derecha (estilo Ubuntu/Windows)
gsettings set org.gnome.desktop.wm.preferences button-layout ':minimize,maximize,close'

# ── Cursor — tema elementary ─────────────────────────────────────────────────
# El paquete elementary-icon-theme incluye cursores en /usr/share/icons/elementary/
# Se activa con el mismo nombre que los iconos: 'elementary'
gsettings set org.gnome.desktop.interface cursor-theme 'elementary'

# ── Workspaces ────────────────────────────────────────────────────────────────
# dconf de sistema (/etc/dconf/db/local.d/) solo establece *defaults* — si el
# usuario tiene un valor propio en ~/.config/dconf/user (por haber abierto
# Ajustes → Multitarea en algún momento), ese valor toma precedencia y los
# workspaces dinámicos vuelven a activarse ignorando el default de sistema.
# La única forma de garantizar el valor es escribirlo también con gsettings
# en el primer login, sobreescribiendo cualquier valor previo del perfil usuario.
gsettings set org.gnome.mutter dynamic-workspaces false
gsettings set org.gnome.mutter workspaces-only-on-primary true
gsettings set org.gnome.desktop.wm.preferences num-workspaces 1

# ── xwayland-native-scaling — escalado correcto para apps X11 en Wayland ──────
# Necesario para que Spotify (forzado a --ozone-platform=x11) no muestre texto
# borroso con escalado fraccional. Sin esta feature, XWayland escala con
# el método legacy (blur visible en monitores HiDPI o con scaling > 100%).
#
# Se añade a experimental-features sin pisar valores existentes (VRR/HDR
# pueden estar ya configurados por 16-configure-gaming.sh en su propio autostart).
# gsettings get devuelve el array actual; se comprueba si ya contiene la feature
# antes de añadirla para evitar duplicados.
CURRENT_FEATURES=$(gsettings get org.gnome.mutter experimental-features 2>/dev/null || echo "@as []")
if ! echo "$CURRENT_FEATURES" | grep -q "xwayland-native-scaling"; then
    # Insertar xwayland-native-scaling preservando cualquier valor existente
    if [ "$CURRENT_FEATURES" = "@as []" ] || [ "$CURRENT_FEATURES" = "[]" ]; then
        gsettings set org.gnome.mutter experimental-features "['xwayland-native-scaling']"
    else
        # Array ya tiene valores — añadir al principio
        NEW_FEATURES=$(echo "$CURRENT_FEATURES" | sed "s/^\[/['xwayland-native-scaling', /")
...
#
# NOTA: Si necesitas verificar los nombres exactos de los .desktop, ejecuta:
#   ls /usr/share/applications/ | grep -i "nombre"
#   o busca el .desktop exacto con: dpkg -L <paquete> | grep .desktop
#
HIDDEN_APPS=(
    ...
    # ImageMagick (display-im7.q16.desktop) se oculta en 16-configure-gaming.sh
    # tras instalar Faugus Launcher que lo arrastra como dependencia
)
...
for DESKTOP_NAME in "${HIDDEN_APPS[@]}"; do
    ...
    if [ ! -f "$USER_DESKTOP" ]; then
        if [ -f "$SYSTEM_DESKTOP" ]; then
            ...
            echo "⚠  $DESKTOP_NAME no encontrado en /usr/share/applications/ — omitido"
            continue
        fi
    fi

    # CRÍTICO: NoDisplay=true debe estar DENTRO del bloque [Desktop Entry],
    # no al final del archivo. Si el .desktop tiene secciones adicionales
    # ([Desktop Action Play], [NewWindow], etc.) un simple echo >> lo escribiría
    # fuera del grupo principal y GNOME lo ignoraría completamente.
    # La solución correcta es insertarlo en la primera línea tras [Desktop Entry].
    sed -i '/^NoDisplay=/d' "$USER_DESKTOP"
    sed -i '/^\[Desktop Entry\]/a NoDisplay=true' "$USER_DESKTOP"
done

# ── Carpetas del App Grid ─────────────────────────────────────────────────────
# Primero limpiar cualquier configuración previa
gsettings set org.gnome.desktop.app-folders folder-children "[]"

# ── Utilidades: herramientas de uso cotidiano ─────────────────────────────────
...
# ── Privacidad ────────────────────────────────────────────────────────────────
# remember-app-usage y remember-recent-files ya están en dconf (gnome-core)
...
# ── Overview y app grid sin fondo opaco ──────────────────────────────────────
# ~/.config/gnome-shell/gnome-shell.css se carga automáticamente por gnome-shell
# como override de usuario desde GNOME 44+ — sin extensión, sin tema externo.
# .workspace-background es el selector que controla el fondo del overview
# y del app grid (ambos comparten la misma capa en GNOME 46).
# Sin este override, GNOME muestra un recuadro gris oscuro opaco sobre el wallpaper.
...
/* El escritorio se ve tal cual, sin fondo opaco superpuesto    */
...
# ── Crear directorios XDG de usuario con permisos correctos ──────────────────
# Algunos módulos (gaming, auto-updates) escriben en ~/.config/autostart/
# antes del primer login. Si algún directorio padre fue creado como root,
# las apps fallan con "Permission denied". Se usan install -d nivel a nivel.
# Sincronizar skel → home del usuario una vez más al final de la configuración.
# Los módulos anteriores pueden haber añadido archivos a skel después de
# que useradd -m ya copió skel al home. cp -a --no-clobber añade solo lo
# que falta sin sobreescribir configuración ya existente del usuario.
# Un único chown -R al final garantiza que todo el home tiene el propietario correcto.
if [ -n "$USERNAME" ] && id "$USERNAME" >/dev/null 2>&1; then
    cp -a --no-clobber /etc/skel/. "/home/$USERNAME/"
    chown -R "$USERNAME":"$USERNAME" "/home/$USERNAME"
    echo "✓  skel sincronizado a /home/$USERNAME — permisos correctos"
fi
