...
# ── Pregunta: método de activación de extensiones ────────────────────────────
# La activación ocurre en el primer login (autostart), pero la elección se hace
# ahora y se guarda en un fichero que el script de primer login leerá.
echo ""
echo -e "${C_INFO}¿Cómo activar las extensiones preinstaladas (dock, appindicators, ding)?${C_RESET}"
echo ""
echo "  1) gnome-extensions enable  — activa una a una vía D-Bus (añade a la lista existente)"
echo "  2) gsettings set            — escribe la lista completa en dconf (reemplaza lista entera)"
echo "  3) No preactivar            — el usuario las activa manualmente desde Extension Manager"
echo ""
read -rp "Método de activación [1/2/3] (default: 1): " EXT_METHOD
EXT_METHOD="${EXT_METHOD:-1}"

case "$EXT_METHOD" in
    1|2|3) ;;
    *) echo "  Opción no válida, usando método 1"; EXT_METHOD="1" ;;
esac

echo -e "${C_OK}✓${C_RESET}  Método de activación de extensiones: ${EXT_METHOD}"

# Escribir elección al sistema destino para que el script de primer login la lea
mkdir -p "${TARGET}/usr/local/lib/ubuntu-advanced-install"
echo "$EXT_METHOD" > "${TARGET}/usr/local/lib/ubuntu-advanced-install/ext-activation-method"
...
# ── Extensiones ───────────────────────────────────────────────────────────────
# Leer método de activación elegido durante la instalación
EXT_METHOD_FILE="/usr/local/lib/ubuntu-advanced-install/ext-activation-method"
EXT_METHOD="1"  # default: gnome-extensions enable
[ -f "$EXT_METHOD_FILE" ] && EXT_METHOD="$(cat "$EXT_METHOD_FILE" 2>/dev/null | tr -d '[:space:]')"

# Construir lista de extensiones a habilitar
EXTENSIONS=(
    "ubuntu-appindicators@ubuntu.com"
    "ubuntu-dock@ubuntu.com"
)

if gnome-extensions list 2>/dev/null | grep -q "ding@rastersoft.com"; then
    EXTENSIONS+=("ding@rastersoft.com")
fi
...
case "$EXT_METHOD" in
    1)
        # Método 1: merge con gsettings — lee la lista actual y añade las nuestras
        # Más fiable que gnome-extensions enable (que modifica enabled + disabled
        # secuencialmente y puede dejar las listas en estado inconsistente).
        ...
        ;;
    2)
        # Método 2: gsettings set enabled-extensions — lista completa
        # Escribe la lista entera de golpe, reemplazando lo que hubiera
        EXTENSIONS_GSETTINGS=""
        for ext in "${EXTENSIONS[@]}"; do
            if [ -z "$EXTENSIONS_GSETTINGS" ]; then
                EXTENSIONS_GSETTINGS="'$ext'"
            else
                EXTENSIONS_GSETTINGS="$EXTENSIONS_GSETTINGS, '$ext'"
            fi
        done
        gsettings set org.gnome.shell enabled-extensions "[$EXTENSIONS_GSETTINGS]" 2>/dev/null || true
        ;;
    3)
        # Método 3: No preactivar — el usuario las activa desde Extension Manager
        ;;
esac

# Limpiar disabled-extensions para evitar residuos que confundan a
# Extension Manager (extensiones desinstaladas que siguen en la lista)
gsettings reset org.gnome.shell disabled-extensions 2>/dev/null || true

# Asegurar que las extensiones de usuario no estén deshabilitadas globalmente
