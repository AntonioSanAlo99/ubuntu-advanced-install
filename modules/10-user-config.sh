...
# Sustituir el placeholder del dock con el UUID real según la elección.
# Este bloque no lleva comillas para que $GNOME_DOCK se expanda desde el host.
arch-chroot "$TARGET" /bin/bash << DOCK_PATCH_EOF
...
sed -i "s/DOCK_EXTENSION_PLACEHOLDER/\${DOCK_UUID}/" \
    /usr/local/lib/ubuntu-advanced-install/gnome-first-login.sh
