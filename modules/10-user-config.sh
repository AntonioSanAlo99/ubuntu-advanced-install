...
if [ -d "$SNAPD_EXT" ]; then
    ...
    rm -rf "$SNAPD_EXT"
    step " Extensión snapd-prompting eliminada"
else
    step " Extensión snapd-prompting no encontrada (OK)"
fi
...
if [ -n "$DBUS_SESSION_BUS_ADDRESS" ] && [ "$XDG_CURRENT_DESKTOP" = "GNOME" ]; then
    ...
    if [ ! -f "$MARKER" ]; then
        ...
        step " GNOME configuración de usuario aplicada"
    fi
fi
USERCONF
...
step " Script de usuario creado: /etc/profile.d/10-gnome-user-config.sh"

CHROOTEOF

echo ""
echo "════════════════════════════════════════════════════════════════"
step " CONFIGURACIÓN DE USUARIO PREPARADA"
echo "════════════════════════════════════════════════════════════════"
echo ""
