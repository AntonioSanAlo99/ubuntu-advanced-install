...
# ── Carpeta Juegos — lista dinámica según apps instaladas ────────────────────
GAMING_APPS=()
for dt in \
    steam.desktop \
    com.heroicgameslauncher.hgl.desktop \
    io.github.faugus.faugus-launcher.desktop \
    com.vysp3r.ProtonPlus.desktop \
    io.github.radiolamp.mangojuice.desktop; do
    if [ -f "/usr/share/applications/$dt" ] || \
       [ -f "/usr/local/share/applications/$dt" ] || \
       [ -f "$HOME/.local/share/applications/$dt" ]; then
        GAMING_APPS+=("'$dt'")
    fi
done

if [ ${#GAMING_APPS[@]} -gt 0 ]; then
    GAMING_LIST=$(IFS=,; echo "[${GAMING_APPS[*]}]")
    # Normalizar: quitar espacios tras comas que IFS podría dejar
    GAMING_LIST=$(echo "$GAMING_LIST" | sed "s/, */,/g; s/,/, /g")
